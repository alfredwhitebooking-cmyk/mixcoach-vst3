...
void MessengerAudioProcessor::ensureSlotRegistered()
{
    ...
    try {
        ...
        slotIndex_ = registry.registerSlot(
...
void MessengerAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
    ...
    if (audioSignalDetected_.load(std::memory_order_relaxed) == SignalState::Waiting) {
        for (int ch = 0; ch < numChannels && ch < 2; ++ch) {
            ...
            if (audioSignalDetected_.load(std::memory_order_relaxed) == SignalState::Detected)
                break;
        }
    }
...
TrackType MessengerAudioProcessor::suggestTrackTypeFromName(const juce::String& rawName) noexcept
{
    ...
    // Kick 808
    if (kw.has808Kick || (kw.has808 && kw.hasKick))
    ...
    // 808 Bass
    if (kw.has808Bass || (kw.has808 && kw.hasBass))
...
{
    if (suggestedType == TrackType::None)
        return false;
    ...
    if (typeName.isEmpty() || typeName == "\xE2\x80\x94")
        return false;

    LogHelper::writeToLog("[Messenger-Identity-V11] Slot " + juce::String(slotIndex_)
                          + " auto-nombrado: \"" + trackName_ + "\" -> \""
                          + typeName + "\"");
    ...
    try {
        ...
        if (version == kStateVersion) {
            ...
            if (mis.getNumBytesRemaining() >= 4) {
                ...
                if (trackType_ != TrackType::None)
                    trackTypePinned_ = true; // Restaurar estado de pin del usuario
            }
            if (mis.getNumBytesRemaining() >= 4)
            ...
            if (mis.getNumBytesRemaining() >= 4)
                muted_.store(mis.readInt() != 0, std::memory_order_relaxed);
        } else {
