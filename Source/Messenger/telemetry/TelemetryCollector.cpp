...
void LoudnessMeter::prepare(double sampleRate)
{
    ...
    // ─── Coeficientes K-weighting (ITU-R BS.1770-4 / EBU R128) ──────────
    // Stage 1: Pre-filter shelving +4dB @ 1.5kHz (RBJ high-shelf 2nd order)
    {
        double gain = std::pow(10.0, 4.0 / 20.0); // +4 dB
        double w0 = 2.0 * juce::MathConstants<double>::pi * 1500.0 / sampleRate;
        double cosW0 = std::cos(w0);
        double sinW0 = std::sin(w0);
        double A = std::sqrt(gain); // 10^(dB/40)
        ...
        // ═══ FIX 2026-05-31: Pre-filter now computed FIRST (Stage 1) per BS.1770-4 ═══
        shB0_ = b0 / a0;
        shB1_ = b1 / a0;
        shB2_ = b2 / a0;
        shA1_ = a1 / a0;
    ...
    // Stage 2: RLB weighting — 2nd order High-pass @ 38Hz (Butterworth)
    // ═══ FIX 2026-05-31: RLB uses 38Hz per BS.1770-4 (was 20Hz) ═══
    // ═══ FIX 2026-05-31: Was incorrectly LOW-PASS (1-cosW0); now HIGH-PASS (1+cosW0) ═══
    {
        constexpr double kRlbFreq = 38.1; // RLB -3dB cutoff per ITU-R BS.1770-4
        double w0 = 2.0 * juce::MathConstants<double>::pi * kRlbFreq / sampleRate;
        double cosW0 = std::cos(w0);
        double sinW0 = std::sin(w0);
        double alpha = sinW0 / std::sqrt(2.0); // Q = 1/√2 (Butterworth)

        // HIGH-PASS biquad coefficients: (1+cosW0) with negated b1
...
{
    // Stage 1: Pre-filter (shelving +4dB @ 1.5kHz) — FIRST per BS.1770-4
    for (int i = 0; i < numSamples; ++i) {
        double in = input[i];
    ...
    // Stage 2: RLB weighting — High-pass 38Hz (Butterworth) — SECOND per BS.1770-4
    for (int i = 0; i < numSamples; ++i) {
        double in = output[i];
                                          ...
                                          float* outSpectrum, int numBins)
{
    ...
    // ─── Extraer magnitudes ─────────────────────────────────────────────
    // JUCE performRealOnlyForwardTransform usa formato empaquetado:
    //   fftInput[0]     = Re(0)      (DC)
    //   fftInput[1]     = Re(N/2)    (Nyquist - N = kFFTSize)
    //   fftInput[2*i]   = Re(i)      para i = 1, 2, ..., N/2-1
    //   fftInput[2*i+1] = Im(i)      para i = 1, 2, ..., N/2-1
    int usableBins = std::min(kFFTSize / 2, numBins);
    for (int i = 0; i < usableBins; ++i) {
        float real, imag;
        if (i == 0) {
            // DC: fftInput[0] = Re(0), imag = 0
            real = fftInput[0];
            imag = 0.0f;
        } else {
            // Bins 1..N/2-1: Re en fftInput[2*i], Im en fftInput[2*i+1]
            real = fftInput[i * 2];
            imag = fftInput[i * 2 + 1];
        }
        float mag = std::sqrt(real * real + imag * imag);
        // Normalizar por tamaño FFT
        float normalized = mag / (float)kFFTSize;
        // Escalar logarítmicamente: mapear -80dB..0dB → 0..1
        outSpectrum[i] = juce::jlimit(0.0f, 1.0f,
            (20.0f * std::log10(normalized + 1e-6f) + 80.0f) / 80.0f);
    }

    // Rellenar bins restantes con decaimiento suave
    // en lugar de 0 abrupto, para evitar bordes duros en el display
    if (usableBins < numBins) {
        float lastVal = outSpectrum[usableBins - 1];
        for (int i = usableBins; i < numBins; ++i) {
            float fade = 1.0f - (float)(i - usableBins) / (float)(numBins - usableBins);
            outSpectrum[i] = lastVal * fade * fade;
        }
    }
}
