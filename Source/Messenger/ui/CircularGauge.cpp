...
void CircularGauge::drawArc(juce::Graphics& g, juce::Rectangle<float> bounds)
{
    ...
    {
        juce::Path glowPath;
        glowPath.addArc(cx - outerR, cy - outerR, outerR * 2, outerR * 2,
                        kArcStartAngle, kArcEndAngle + juce::MathConstants<float>::twoPi, true);
    ...
    {
        juce::Path trackPath;
        trackPath.addArc(cx - outerR, cy - outerR, outerR * 2, outerR * 2,
                         kArcStartAngle, kArcEndAngle + juce::MathConstants<float>::twoPi, true);
    ...
    if (current > minDb_ + 0.5f) {
        ...
        if (endAngle < kArcStartAngle)
            endAngle += juce::MathConstants<float>::twoPi;
        ...
        activePath.addArc(cx - outerR, cy - outerR, outerR * 2, outerR * 2,
                          kArcStartAngle, endAngle, true);
