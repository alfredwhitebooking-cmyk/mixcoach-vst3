...
namespace mixcoach {
    ...
    static constexpr float kScaleValues[] = {
        6.0f, 0.0f, -6.0f, -12.0f, -18.0f, -24.0f, -30.0f, -36.0f, -42.0f, -48.0f, -60.0f};
    ...
    void StereoMeter::setLevels(float leftDb, float rightDb)
    {
        ...
        if (newL > leftLevel_) leftLevel_ = newL;
            ...
            leftLevel_ += (newL - leftLevel_) * 0.15f;

        // ─── Ballistics R ──────────────────────────────────────────────────
        if (newR > rightLevel_) rightLevel_ = newR;
        else
            rightLevel_ += (newR - rightLevel_) * 0.15f;
        ...
        if (newL > leftPeak_) {
            ...
            leftPeak_ += (-80.0f - leftPeak_) * 0.03f;
        ...
        if (newR > rightPeak_) {
            ...
            rightPeak_ += (-80.0f - rightPeak_) * 0.03f;
    ...
    void StereoMeter::paint(juce::Graphics& g)
    {
        ...
        if (leftPeak_ > -60.0f) {
            ...
            juce::Path tri;
            tri.addTriangle(
                peakZone.getX(), peakY, peakZone.getRight(), peakY - 3.0f, peakZone.getRight(), peakY + 3.0f);
        ...
        if (rightPeak_ > -60.0f) {
            ...
            juce::Path tri;
            tri.addTriangle(peakZone.getX() + 1.0f,
                            peakY + 0.5f,
                            peakZone.getRight() - 0.5f,
                            peakY - 2.0f,
                            peakZone.getRight() - 0.5f,
                            peakY + 3.0f);
        ...
        if (clipActive_) {
            ...
            g.drawText("CLIP",
                       juce::Rectangle<float>(
                           barAreaBounds.getX(), barAreaBounds.getY() + 2.0f, barAreaBounds.getWidth(), 12.0f),
    ...
    void StereoMeter::drawBar(juce::Graphics& g, juce::Rectangle<float> barBounds, float level)
    {
        ...
        if (norm > 0.005f) {
            ...
            juce::ColourGradient grad(juce::Colour(kRed),
