...
namespace mixcoach {
    ...
    void VUMeter::setLevel(float levelDb)
    {
        ...
        if (std::abs(newLevel - lastDrawnLevel_) > 0.05f || std::abs(peakLevel_ - lastDrawnPeak_) > 0.05f) {
            repaint();
    ...
    void VUMeter::paint(juce::Graphics& g)
    {
        ...
        if (norm > 0.005f) {
            ...
            juce::ColourGradient grad(juce::Colour(kRed), // top: rojo
    ...
    {
        if (levelDb > -6.0f) return juce::Colour(kRed).withAlpha(alpha);
