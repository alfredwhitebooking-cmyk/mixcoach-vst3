    ...
    public:
        ...
        static DetectedKeywords detectKeywords(const juce::String& rawName) noexcept
        {
            ...
            auto removeSuffix = [](const juce::String& n, const juce::String& suffix) -> juce::String {
                if (n.endsWith(suffix)) return n.substring(0, n.length() - suffix.length()).trim();
                return n;
            };
            ...
            kw.hasElectric = name.contains("electric")
                             || name.contains(
                                 "el\xC3"
                                 "\xA9"
                                 "ctrica")
                             || name.contains("electrica");
            kw.hasFinger   = name.contains("finger") || name.contains("dedo");
            kw.hasPick     = name.contains("pick")
                             || name.contains(
                                 "p\xC3"
                                 "\xBA"
                                 "a")
            ...
            kw.hasVozPrincipal = kw.hasVocal
                                 && (name.contains("principal") || name.contains("lead") || name.contains("main")
                                     || name.contains("voz"));
            kw.hasVozFondo     = kw.hasVocal
                                 && (name.contains("fondo") || name.contains("back") || name.contains("coro")
                                     || name.contains("backing"));
            kw.hasAdlib        = kw.hasVocal
                                 && (name.contains("adlib") || name.contains("double") || name.contains("doblaje"));
            kw.hasRhythm       = name.contains("rhythm") || name.contains("ritmica")
                                 || name.contains(
                                     "r\xC3"
                                     "\xAD"
                                     "tmica")
                                 || name.contains("ritmica");
            ...
            kw.hasRoomMic = kw.hasRoom
                            && (kw.hasDrum || kw.hasKick || kw.hasSnare || kw.hasHiHat || kw.hasTom
            ...
            kw.hasLeadVocal = kw.hasVocal
                              && (name.contains("principal") || name.contains("lead") || name.contains("main")
                                  || name.contains("voz"));
            kw.hasBackVocal = kw.hasVocal
                              && (name.contains("fondo") || name.contains("back") || name.contains("coro")
