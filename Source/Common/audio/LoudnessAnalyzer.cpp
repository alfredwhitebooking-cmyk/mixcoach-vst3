...
namespace mixcoach {
    ...
    void LoudnessAnalyzer::initFilters()
    {
        ...
        auto hpCoeffs          = juce::dsp::IIR::Coefficients<float>::makeHighPass(sampleRate_, 48.1f);
        hpFilter_.coefficients = hpCoeffs;

        // High-shelf: +4 dB gain at 1.5 kHz, shelf Q = 0.5
        auto shelfCoeffs = juce::dsp::IIR::Coefficients<float>::makeHighShelf(sampleRate_, 1500.0f, 0.5f, 4.0f);
    ...
    void LoudnessAnalyzer::prepare(double sampleRate, int /*samplesPerBlock*/)
    {
        ...
        if (samplesPerBlock_ < 64) samplesPerBlock_ = 64;
    ...
    void LoudnessAnalyzer::processBlock(const float* left, const float* right, int numSamples)
    {
        ...
        for (int i = 0; i < numSamples; ++i) {
            ...
            float filtered = shelfFilter_.processSample(hpFilter_.processSample(mono));
        ...
        while (blockSampleCount_ >= samplesPerBlock_) flushBlock();
        ...
        // Momentary: average of last 4 blocks (400ms with 100ms overlaps)
        {
            ...
            if (count > 0) momentaryLUFS_.store(computeLUFS(sum, count), std::memory_order_release);
        }
        ...
        // Short-term: average of last 30 blocks (3s)
        {
            ...
            if (count > 0) shortTermLUFS_.store(computeLUFS(sum, count), std::memory_order_release);
        }
    ...
    void LoudnessAnalyzer::updateTruePeak(const float* left, const float* right, int numSamples)
    {
        ...
        truePeakDBTP_.store((truePeak_ > 1e-12f) ? juce::Decibels::gainToDecibels(truePeak_) : -100.0f,
