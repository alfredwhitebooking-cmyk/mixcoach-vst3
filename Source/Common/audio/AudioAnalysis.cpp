...
void AudioAnalysis::prepare(double sampleRate, int /*samplesPerBlock*/)
{
    ...
    if (!hiResFFT_)
        hiResFFT_ = std::make_unique<juce::dsp::FFT>(kHiResFFTOrder);
    ...
    for (auto& bin : hiResSpectrumAtomic_)
        bin.store(0.0f, std::memory_order_release);
    hiResCount_ = 0;
...
void AudioAnalysis::process(const float* channelData, int numSamples)
{
    ...
    if (hiResFFT_ && numSamples > 0)
        feedHiResFFT(channelData, numSamples);
...
{
    if (!hiResFFT_ || data == nullptr || numSamples <= 0)
        return;
    ...
    for (int s = 0; s < numSamples; ++s)
    {
        ...
        if (hiResCount_ >= kHiResFFTSize)
        {
            ...
            for (int i = 0; i < kHiResFFTSize; ++i)
            {
                ...
                const float bh = kBH_a0
                    - kBH_a1 * std::cos(angle)
                    + kBH_a2 * std::cos(2.0f * angle)
            ...
            std::memmove(hiResFifo_.data(), hiResFifo_.data() + kHiResHopSize,
                         keep * sizeof(float));
            hiResCount_ = keep;
        }
    }
...
{
    if (dest == nullptr || maxBins <= 0)
        return 0;

    const int count = std::min(maxBins, kHiResNumBins);
    for (int i = 0; i < count; ++i)
        dest[i] = hiResSpectrumAtomic_[i].load(std::memory_order_acquire);
