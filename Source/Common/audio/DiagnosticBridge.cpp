...
namespace mixcoach {
    ...
    {
        if (isPraise) return juce::Colour(0xFF10B981); // Green for praise

        if (isCritical) return juce::Colour(0xFFEF4444); // Red for critical
        ...
        if (t < 0.5f) return yellow.interpolatedWith(orange, t * 2.0f);
        else
    ...
    {
        if (isPraise) return juce::Colour(0xFF10B981);  // Green for healthy phase
        if (isWarning) return juce::Colour(0xFFEF4444); // Red for phase warning
