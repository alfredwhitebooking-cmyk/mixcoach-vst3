...
bool SharedAudioMemory::initialize(const juce::String& mapName)
{
    ...
    if (fileMapping_ == nullptr) {
        LogHelper::writeToLog("[SharedAudioMemory] ERROR: No se pudo crear/abrir "
    ...
    if (fileView_ == nullptr) {
        LogHelper::writeToLog("[SharedAudioMemory] ERROR: No se pudo mapear "
    ...
    if (isNew) {
        ...
        LogHelper::writeToLog("[SharedAudioMemory] CREADO nuevo mapping de audio: "
            + mapName_ + " (" + juce::String((int)(kAudioMemSize / 1024)) + " KB)");
    } else {
        LogHelper::writeToLog("[SharedAudioMemory] ABIERTO mapping de audio existente: "
            + mapName_);
    }
...
int SharedAudioMemory::readSamples(int slotIndex, float* outData, int maxSamples) noexcept
{
    ...
    for (int64_t i = 0; i < toRead; ++i)
        outData[i] = slot.buffer[(rp + i) % kAudioBufferSize];
...
{
    if (block_ == nullptr || slotIndex < 0 || slotIndex >= kMaxAudioSlots)
        return false;
