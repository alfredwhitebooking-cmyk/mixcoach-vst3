...
bool SharedAudioMemoryV2::initialize(const juce::String& mapName) {
    ...
    if (fileMapping_ == nullptr) {
        // Create new mapping
        fileMapping_ = CreateFileMappingW(INVALID_HANDLE_VALUE, nullptr, PAGE_READWRITE, 0,
            static_cast<DWORD>(sizeof(SharedAudioBlockV2)), mapName_.toWideCharPointer());
        if (fileMapping_ != nullptr) {
    ...
    if (fileMapping_ == nullptr) {
        LogHelper::writeToLog("[SharedAudioMemoryV2] ERROR: Cannot create/open file mapping (" + juce::String(GetLastError()) + ")");
        return false;
    }
...
void SharedAudioMemoryV2::writeStereoSamples(int slotIndex, const float* left, const float* right, int numSamples) noexcept {
    if (!block_ || slotIndex < 0 || slotIndex >= kMaxAudioSlots || left == nullptr || right == nullptr || numSamples <= 0)
        return;
...
int SharedAudioMemoryV2::readStereoSamples(int slotIndex, float* outLeft, float* outRight, int maxSamples) noexcept {
    if (!block_ || slotIndex < 0 || slotIndex >= kMaxAudioSlots || outLeft == nullptr || outRight == nullptr || maxSamples <= 0)
        return 0;
    auto& slot = block_->slots[slotIndex];
    int64_t wpL = slot.writePosL;
    int64_t wpR = slot.writePosR;
...
void SharedAudioMemoryV2::scanTruePeak(int slotIndex, float& peakL, float& peakR) const noexcept
{
    peakL = 0.0f;
    peakR = 0.0f;
    if (!block_ || slotIndex < 0 || slotIndex >= kMaxAudioSlots)
        return;
...
int SharedAudioMemoryV2::available(int slotIndex) const noexcept {
    ...
    if (diff > kAudioBufferSize)
        diff = kAudioBufferSize;
