...
namespace mixcoach {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    void
    LlmClient::sendPrompt(const juce::String& systemPrompt, const juce::String& userPrompt, ResponseCallback callback)
    {
        {
            const juce::ScopedLock lock(mutex_);
            if (pendingRequests_.size() < 10) // Máximo 10 requests encolados
            {
                ...
                if (callback) callback(false, {}, "[LlmClient] Cola llena, request ignorado.");
                return;
            }
        }
    ...
    {
        {
            ...
            if (pendingRequests_.size() < 10) {
                pendingRequests_.push_back(
                    {systemPrompt, userPrompt, nullptr, std::move(onToken), std::move(onComplete)});
            }
            else {
                if (onComplete)
                    juce::MessageManager::callAsync(
                        [cb = std::move(onComplete)]() { cb({}, "[LlmClient] Cola llena, request ignorado."); });
                return;
            }
        }
        notify();
    ...
    static bool
    checkEndpoint(LlmClient::Provider provider, const juce::String& endpointUrl, const juce::String& apiKey);
    ...
    void LlmClient::checkAvailability()
    {
        ...
        if (available)
            LogHelper::writeToLog("[LlmClient] API disponible: " + getProviderName() + " @ " + config_.endpointUrl
                                  + " (modelo: " + config_.model + ")");
        else
            LogHelper::writeToLog("[LlmClient] API NO disponible: " + getProviderName() + " @ " + config_.endpointUrl);
    }
    ...
    bool LlmClient::checkAllProviders()
    {
        ...
        if (config_.useFallback) {
            bool fallbackAvailable =
                checkEndpoint(config_.fallbackProvider, config_.fallbackEndpointUrl, config_.fallbackApiKey);
            if (fallbackAvailable) {
                // ═══ Conmutar al fallback permanentemente ═══
                LogHelper::writeToLog(
                    "[LlmClient] Primario NO disponible, fallback disponible."
                    " Conmutando a "
                    + config_.fallbackEndpointUrl + " (modelo: " + config_.fallbackModel + ")");
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    static bool checkEndpoint(LlmClient::Provider provider, const juce::String& endpointUrl, const juce::String& apiKey)
    {
        try {
            ...
            if (provider == LlmClient::Provider::OpenAICompatible) {
                ...
                if (!endpoint.endsWithChar('/')) endpoint += "/";
                if (!endpoint.endsWith("models")) endpoint += "models";

                if (apiKey.isNotEmpty()) requestHeaders.set("Authorization", "Bearer " + apiKey);
            }
            else {
                // Ollama: GET /api/tags
                endpoint = endpointUrl;
    ...
    void LlmClient::run()
    {
        ...
        while (!threadShouldExit()) {
            ...
            if (request.callback || request.isStreaming()) {
                busy_.store(true);
                if (request.isStreaming()) processStreamRequest(request);
                else
    ...
    void LlmClient::switchToFallbackConfig()
    {
        ...
        LogHelper::writeToLog("[LlmClient] Conmutado a fallback: " + config_.endpointUrl + " (modelo: " + config_.model
                              + ")");
    }
    ...
    void LlmClient::processRequest(const PendingRequest& request)
    {
        ...
        if (!success && config_.useFallback
            && (config_.provider != config_.fallbackProvider || config_.endpointUrl != config_.fallbackEndpointUrl)) {
            LogHelper::writeToLog(
                "[LlmClient] Request falló con primario."
            ...
            if (fbSuccess) {
                ...
                switchToFallbackConfig();
                available_.store(true);
                LogHelper::writeToLog("[LlmClient] Fallback exitoso. Conmutado a: " + config_.model + " @ "
                                      + config_.endpointUrl);
            }
            else {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    bool LlmClient::processRequestWithConfig(const PendingRequest& request, const Config& cfg, bool fireCallback)
    {
        try {
            ...
            LogHelper::writeToLog(juce::String("[LlmClient] Enviando prompt a ") + (isOpenAI ? "cloud" : "local") + " ("
                                  + cfg.model + ", " + juce::String(cfg.maxTokens) + " tokens)...");
            ...
            if (response.isEmpty()) {
                if (fireCallback && request.callback) {
                    juce::MessageManager::callAsync([cb = request.callback, isOpenAI]() {
                        juce::String msg = isOpenAI ? "[LlmClient] No hubo respuesta del proveedor cloud. Verifica tu "
                                                      "API key y conexión a internet."
                                                    : "[LlmClient] No hubo respuesta de Ollama. Verifica que Ollama "
                                                      "esté corriendo en http://localhost:11434";
                        cb(false, {}, msg);
                    });
                }
                return false;
            ...
            if (!resultJson.isObject()) {
                if (fireCallback && request.callback) {
                    juce::MessageManager::callAsync([cb = request.callback, response]() {
            ...
            if (rootObj == nullptr) {
                if (fireCallback && request.callback) {
                    juce::MessageManager::callAsync(
                        [cb = request.callback]() { cb(false, {}, "[LlmClient] Error parseando respuesta JSON"); });
                }
                return false;
            }
            ...
            if (isOpenAI) {
                ...
                if (choices.isArray() && choices.getArray()->size() > 0) {
                    ...
                    if (choiceObj != nullptr) {
                        ...
                        if (msgObj != nullptr) content = msgObj->getProperty("content").toString().trim();
                    }
                }
            }
            else {
            ...
            if (content.isEmpty()) {
                if (fireCallback && request.callback) {
                    ...
                    juce::MessageManager::callAsync([cb = request.callback, preview]() {
                        cb(false, {}, "[LlmClient] No se pudo extraer contenido de la respuesta: " + preview);
                    });
                }
                return false;
            }
            ...
            if (fireCallback) available_.store(true);

            // ─── Entregar respuesta al message thread ────────────────────────
            if (fireCallback && request.callback) {
                juce::MessageManager::callAsync([cb = request.callback, content]() { cb(true, content, {}); });
            }

            LogHelper::writeToLog("[LlmClient] Respuesta recibida (" + juce::String(content.length()) + " chars)");
            return true;
        }
        catch (const std::exception& e) {
            LogHelper::writeToLog("[LlmClient] Excepción: " + juce::String(e.what()));
            if (fireCallback && request.callback) {
                juce::MessageManager::callAsync([cb = request.callback, msg = juce::String(e.what())]() {
            ...
            if (fireCallback && request.callback) {
                juce::MessageManager::callAsync(
                    [cb = request.callback]() { cb(false, {}, "[LlmClient] Error desconocido"); });
            }
            return false;
        }
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    juce::String LlmClient::makeRequestWithConfig(const juce::String& jsonBody, const Config& cfg, int& statusCode)
    {
        try {
            // ─── Determinar endpoint según el proveedor ─────────────────────
            juce::String endpoint = cfg.endpointUrl;
            if (endpoint.endsWithChar('/')) endpoint = endpoint.dropLastCharacters(1);

            if (cfg.provider == Provider::OpenAICompatible) endpoint += "/chat/completions";
            else
                endpoint += "/api/chat";
    ...
    void LlmClient::processStreamRequest(const PendingRequest& request)
    {
        ...
        if (!streamOk && config_.useFallback
            && (config_.provider != config_.fallbackProvider || config_.endpointUrl != config_.fallbackEndpointUrl)) {
            LogHelper::writeToLog(
                "[LlmClient] Stream falló con primario."
            ...
            if (fbStreamOk) {
                switchToFallbackConfig();
                available_.store(true);
                LogHelper::writeToLog("[LlmClient] Fallback streaming exitoso. Conmutado a: " + config_.model + " @ "
                                      + config_.endpointUrl);
            }
            else {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    bool LlmClient::processStreamWithConfig(const PendingRequest& request, const Config& cfg)
    {
        try {
            ...
            if (isOpenAI) endpoint += "/chat/completions";
            else
                endpoint += "/api/chat";
            ...
            if (inputStream == nullptr || statusCode != 200) {
                juce::String errMsg = (inputStream == nullptr) ? "[LlmClient] Error de conexión a " + endpoint
                                                               : "[LlmClient] HTTP " + juce::String(statusCode);
                LogHelper::writeToLog(errMsg);
                if (request.onStreamComplete) {
                    juce::MessageManager::callAsync([cb = request.onStreamComplete, errMsg]() { cb({}, errMsg); });
                }
                return false;
            }
            ...
            while (!inputStream->isExhausted() && !threadShouldExit()) {
                    ...
                    if (json.isObject()) {
                        ...
                        if (obj != nullptr) {
                            ...
                            if (msgVal.isObject()) {
                                ...
                                if (msgObj != nullptr) contentToken = msgObj->getProperty("content").toString();
                            }

                            // Check done flag
                            auto doneVal = obj->getProperty("done");
                            if (doneVal.isBool() && static_cast<bool>(doneVal)) isDone = true;
                        }
                    }
                }
                ...
                if (contentToken.isNotEmpty()) {
                    ...
                    if (request.onToken) {
                        ...
                        juce::MessageManager::callAsync([tokenCb, tokenCopy]() { tokenCb(tokenCopy); });
                    }
                }
            ...
            if (hasError) {
                ...
                if (request.onStreamComplete) {
                    juce::MessageManager::callAsync([cb = request.onStreamComplete, errorMsg]() { cb({}, errorMsg); });
                }
            }
            else {
                LogHelper::writeToLog("[LlmClient] Streaming completado (" + juce::String(fullResponse.length())
                                      + " chars)");
                if (request.onStreamComplete) {
                    juce::String responseCopy = fullResponse;
                    auto completeCb           = request.onStreamComplete;
                    juce::MessageManager::callAsync([completeCb, responseCopy]() { completeCb(responseCopy, {}); });
                }
            }
            ...
            if (request.onStreamComplete) {
                juce::MessageManager::callAsync(
                    [cb = request.onStreamComplete]() { cb({}, "[LlmClient] Error desconocido en streaming"); });
            }
            return false;
        }
    }
