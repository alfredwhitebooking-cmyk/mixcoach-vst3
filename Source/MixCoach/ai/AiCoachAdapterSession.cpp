    ...
    , phaseManager_(phaseManager)
{
    ...
    if (userProfile_.engineerName.isNotEmpty())
    {
        ...
        LogHelper::writeToLog("[AiCoachAdapter] Nombre propagado al CoachEngine: "
                              + userProfile_.engineerName);
    }
}
...
void AiCoachAdapter::loadSessionMemory(const juce::File& file)
{
    try {
        ...
        if (root->hasProperty("genre"))
            genre_ = root->getProperty("genre").toString();
        ...
        if (root->hasProperty("sessionHistory")) {
            ...
            if (historyArr != nullptr) {
                for (int i = 0; i < historyArr->size(); ++i) {
                    ...
                    change.beforeValue = static_cast<float>(static_cast<double>(changeObj->getProperty("beforeValue")));
                    change.afterValue = static_cast<float>(static_cast<double>(changeObj->getProperty("afterValue")));
                    if ((int)sessionHistory_.size() < kMaxSessionChanges)
        ...
        if (root->hasProperty("sessionMap")) {
            ...
            if (mapObj != nullptr)
                coachEngine_.loadSessionMapFromJson(*mapObj);
        }
        ...
        if (root->hasProperty("differenceProfile")) {
            ...
            if (dpObj != nullptr)
                coachEngine_.loadDifferenceProfileFromJson(*dpObj);
        }
            ...
            if (intentsArr != nullptr) {
                for (int i = 0; i < intentsArr->size(); ++i) {
                    ...
                    if (slotIndex >= 0 && slotIndex < SlotRegistry::kMaxSlots)
                        trackIntents_[slotIndex] = intent;
                }
            }
        }
...
static juce::File getMixCoachBaseDir()
{
    ...
    if (localAppData.isNotEmpty())
        return juce::File(localAppData).getChildFile("MixCoach");
#endif
                            ...
                            std::function<void(bool, const juce::String&)> callback)
{
    if (llmClient_ == nullptr) {
        if (callback)
            callback(false, "LLM not available");
        return;
    }
    ...
    llmClient_->sendPrompt(system, fullUserMsg,
        [this, callback, userMessage](bool success, const juce::String& response, const juce::String& error) {
            ...
            if (callback)
                callback(success, success ? response : error);
        });
}
                                  ...
                                  std::function<void(bool success, const juce::String& response)> onComplete)
{
    if (llmClient_ == nullptr) {
        if (onComplete)
            onComplete(false, "LLM not available");
        return;
    }
    ...
    llmClient_->sendPromptStream(system, fullUserMsg,
        onToken,
    ...
    if (s_knowledgeBasePath.isEmpty()) {
        static const juce::String kLocations[] = {
            ...
            juce::File::getSpecialLocation(juce::File::currentExecutableFile)
                .getParentDirectory().getChildFile("knowledge").getFullPathName(),
                                                    ...
                                                    const juce::String& sectionLabel)
{
    ...
    if (nextSection > lineStart)
        return fullContent.substring(lineStart, nextSection);
...
void AiCoachAdapter::updateUserProfile()
{
    ...
    LogHelper::writeToLog("[AiCoachAdapter] User profile updated: " + juce::String(userProfile_.sessionCount) + " sessions");
}
...
void AiCoachAdapter::loadUserProfile(const juce::File& file)
{
    try {
        ...
        if (root->hasProperty("sessionCount"))
            userProfile_.sessionCount = root->getProperty("sessionCount");
        if (root->hasProperty("mostUsedGenre"))
            userProfile_.mostUsedGenre = root->getProperty("mostUsedGenre").toString();
        if (root->hasProperty("valid"))
            userProfile_.valid = root->getProperty("valid");
        if (root->hasProperty("engineerName"))
            userProfile_.engineerName = root->getProperty("engineerName").toString();
...
juce::String AiCoachAdapter::bandLabel(int bandIndex) noexcept
{
    static const char* labels[] = {
        "Sub (0-86Hz)", "Bass (86-301Hz)", "Low-Mid (301-1076Hz)",
        "High-Mid (1076-3532Hz)", "Presence (3532-8355Hz)", "Air (8355-16458Hz)"
    };
    if (bandIndex >= 0 && bandIndex < 6)
        return juce::String(labels[bandIndex]);
...
juce::String AiCoachAdapter::busLabel(BusType bus) noexcept
{
    switch (bus) {
        ...
        case BusType::Bass:   return "Bass";
        case BusType::Guitars: return "Guitars";
        case BusType::Keys:   return "Keys";
        case BusType::Vocals: return "Vocals";
        case BusType::FX:     return "FX";
        case BusType::Melody: return "Melody";
        default: return "None";
    }
}
...
{
    switch (phase) {
        case MentorPhase::Organizacion:  return "Organización";
        case MentorPhase::GainStaging:   return "Gain Staging";
        case MentorPhase::Balance:       return "Balance";
        case MentorPhase::EQ:            return "EQ";
        case MentorPhase::Compresion:    return "Compresión";
        case MentorPhase::Espacio:       return "Espacio";
        case MentorPhase::MasterCheck:   return "Master Check";
...
{
    switch (status) {
        case SuggestionStatus::Green:  return "\xf0\x9f\x9f\xa2"; // green
        case SuggestionStatus::Yellow: return "\xf0\x9f\x9f\xa1"; // yellow
        case SuggestionStatus::Red:    return "\xf0\x9f\x94\xb4"; // red
        case SuggestionStatus::White:  return "\xe2\x9a\xaa";     // white
        default:                       return "\xe2\x9a\xab";     // gray
    }
}
...
const AiCoachAdapter::TrackIntent& AiCoachAdapter::getTrackIntent(int slotIndex) const
{
    ...
    if (slotIndex >= 0 && slotIndex < SlotRegistry::kMaxSlots)
        return trackIntents_[slotIndex];
                                   ...
                                   float afterValue)
{
    ...
    while ((int)sessionHistory_.size() > kMaxSessionChanges)
