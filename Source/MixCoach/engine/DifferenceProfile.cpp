...
void DifferenceProfile::computeRegionEnergies(
    const float bandEnergies[30],
    float regionEnergies[6]) noexcept
{
    static const int kRegionRanges[6][2] = {
...
float DifferenceProfile::computeDeltaScore(
...
DifferenceProfile DifferenceProfile::build(
    ...
    const juce::String& sectionLabel)
{
    ...
    {
        float masterPeak = juce::jmax(master.getLeftAnalysis().getPeak(),
                                       master.getRightAnalysis().getPeak());
        float masterRMS  = juce::jmax(master.getLeftAnalysis().getRMS(),
                                       master.getRightAnalysis().getRMS());
        if (masterRMS > -60.0f && masterPeak > -60.0f)
            dp.mixCrestFactor = masterPeak - masterRMS;
    }

    {
        ...
        if (spectrum != nullptr && masterAnalysis.getLastUpdateTime() > 0)
        {
            ...
            if (totalEnergy > 0.0)
                dp.mixSpectralCentroidHz = static_cast<float>(weightedSum / totalEnergy);
        }
    }
    ...
    for (const auto& g : gaps)
    {
        switch (g.severity)
        {
            case GapSeverity::Critical: dp.criticalGaps++; break;
            case GapSeverity::Warning:  dp.warningGaps++;  break;
            case GapSeverity::Info:     dp.infoGaps++;     break;
            case GapSeverity::Praise:   dp.praiseCount++;  break;
        }
    }

    LogHelper::writeToLog("[DifferenceProfile] Built: "
                          + refName + " | "
                          + "LUFS mix=" + juce::String(dp.mixIntegratedLUFS, 1)
                          + " ref=" + juce::String(dp.refIntegratedLUFS, 1)
                          + " delta=" + juce::String(dp.deltaLUFS, 1)
                          + " | Score=" + juce::String(dp.deltaScore, 2)
                          + " | Gaps: " + juce::String(dp.criticalGaps) + "c/"
                          + juce::String(dp.warningGaps) + "w/"
...
DifferenceProfile DifferenceProfile::fromJson(const juce::DynamicObject& obj)
{
    ...
    auto readDbl = [&](const char* key, double def) -> double {
        ...
        if (!v.isVoid() && !v.isObject() && !v.isArray() && !v.isString())
            return (double)v;
        return def;
    };
    ...
    // Arrays: region energies (6 regions)
    {
        ...
        if (mixVar.isArray())
        {
            ...
            for (int r = 0; r < 6 && r < arr->size(); ++r)
                dp.mixRegionEnergy[r] = (float)(double)(*arr)[r];
        }
        if (refVar.isArray())
        {
            auto* arr = refVar.getArray();
            for (int r = 0; r < 6 && r < arr->size(); ++r)
                dp.refRegionEnergy[r] = (float)(double)(*arr)[r];
        }
        if (deltaVar.isArray())
        {
            auto* arr = deltaVar.getArray();
            for (int r = 0; r < 6 && r < arr->size(); ++r)
                dp.deltaRegionEnergy[r] = (float)(double)(*arr)[r];
        }
    }
...
{
    if (!valid)
        return "[DIFFERENCE PROFILE] No reference loaded.\n";

    juce::String s;

    s += "[DIFFERENCE PROFILE: " + referenceName;
    if (activeSectionLabel.isNotEmpty() && activeSectionLabel != "Full")
        s += " - " + activeSectionLabel;
    s += "]\n";

    s += "LUFS: ref " + fmtLUFS(refIntegratedLUFS)
         + " | mix " + fmtLUFS(mixIntegratedLUFS)
         + " | delta " + fmtDelta(deltaLUFS, "LUFS");
    if (deltaLUFS > 1.5f)
        s += " (mix quieter)";
    ...
    s += "Crest: ref " + juce::String(refCrestFactor, 1) + " dB"
         + " | mix " + juce::String(mixCrestFactor, 1) + " dB"
         + " | delta " + fmtDelta(deltaCrestFactor, "dB") + "\n";

    s += "Corr:  ref " + juce::String(refCorrelation, 2)
         + " | mix " + juce::String(mixCorrelation, 2)
         + " | delta " + fmtDelta(deltaCorrelation, "") + "\n";

    s += "Similarity: " + juce::String(spectralSimilarity, 2)
         + " | DeltaScore: " + juce::String(deltaScore, 2);
    ...
    s += "TruePeak: ref " + juce::String(refTruePeakDBTP, 1) + " dBTP"
         + " | mix " + juce::String(mixTruePeakDBTP, 1) + " dBTP\n";
    s += "LRA:      ref " + juce::String(refLoudnessRange, 1) + " LU"
         + " | mix " + juce::String(mixLoudnessRange, 1) + " LU"
         + " | delta " + fmtDelta(deltaLoudnessRange, "LU") + "\n";
...
{
    if (!valid)
        return "[DIFFERENCE PROFILE] INVALID\n";
    ...
    for (int r = 0; r < 6; ++r)
    {
        s += juce::String("  ") + juce::String(regionLabel(r)) + ": "
             + "ref " + fmtDb(refRegionEnergy[r]) + " | "
             + "mix " + fmtDb(mixRegionEnergy[r]) + " | "
             + "delta " + fmtDelta(deltaRegionEnergy[r], "dB") + "\n";
    }
    s += "\n";
    ...
    s += "── Domain Gaps ──────────────────────────────────────\n";
    s += "  Total: " + juce::String(totalGaps)
         + " | Critical: " + juce::String(criticalGaps)
         + " | Warning: " + juce::String(warningGaps)
         + " | Info: " + juce::String(infoGaps)
