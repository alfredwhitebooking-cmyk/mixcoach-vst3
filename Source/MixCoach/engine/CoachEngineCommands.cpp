...
namespace mixcoach {
    ...
    void CoachEngine::executeCommand(const juce::String& command)
    {
        ...
        if (lower == "/next" || lower == "/avanzar") {
            ...
            respondWith(
                "\xF0\x9F\x94\x8D Ejecutando an\xC3\xA1"
            ...
            if (!semanticDiffs.empty()) {
                ...
                juce::String semMsg =
                    "\n\xF0\x9F\xA7\xA0 **An\xC3\xA1"
                    "lisis por rol** ("
                    + juce::String((int)semanticDiffs.size()) + " pistas analizadas):\n";
                for (const auto& diff : semanticDiffs) {
                    juce::String summary = diff.toTextSummary();
                    if (summary.isNotEmpty()) semMsg += "  \xF0\x9F\x94\x8D " + summary + "\n";
                }
                semMsg += "\n\xF0\x9F\x93\x8A **Total:** " + juce::String(totalProblems) + " problemas, "
            ...
            if (active == 0) {
                respondWith("\xF0\x9F\x93\x84 No hay pistas activas para exportar.", MentorMessage::Type::Info);
            }
            else {
                juce::String report;
            ...
            if (active == 0) {
                respondWith(
                    "\xF0\x9F\x93\x8A No hay pistas activas para generar estad\xC3\xAD"
                    "sticas.",
                    MentorMessage::Type::Info);
            }
            else {
            ...
            respondWith("\xE2\x9D\x93 Comando no reconocido. Escribe **/help** para ver los disponibles.",
