...
namespace mixcoach {
    ...
    SemanticIssue SemanticComparator::makeIssue(SemanticIssue::Severity sev,
    ...
    SemanticIssue SemanticComparator::makePraise(SemanticIssue::Severity sev,
                                                 SemanticIssue::Domain dom,
                                                 const juce::String& trackName,
                                                 TrackRole role,
                                                 const juce::String& message)
    ...
    {
        if (!hasProblems() && praises.empty()) return {};
                                                  ...
                                                  const TrackSpectralProfile& profile)
    {
        ...
        if (!profile.hasData()) return diff;
        ...
        std::sort(diff.issues.begin(), diff.issues.end(), [](const SemanticIssue& a, const SemanticIssue& b) {
            ...
            auto prio = [](SemanticIssue::Severity s) -> int {
                switch (s) {
                    ...
                    case SemanticIssue::Severity::Info:
                        return 2;
                    case SemanticIssue::Severity::Praise:
                        return 3;
                    default:
                        return 4;
                }
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    std::vector<SemanticDiff>
    SemanticComparator::compareAllTracks(const SlotRegistry& registry,
                                         ...
                                         const std::array<TrackRole, SlotRegistry::kMaxSlots>& roles)
    {
        ...
        registry.forEachActive([&](const SlotInfo& info) {
            ...
            if (role == TrackRole::Unknown) {
                ...
                switch (info.bus) {
                    case BusType::Drums:
                        role = TrackRole::DrumBus;
                        break;
                    case BusType::Bass:
                        role = TrackRole::BassBus;
                        break;
                    case BusType::Vocals:
                        role = TrackRole::VozBus;
                        break;
                    case BusType::Guitars:
                        role = TrackRole::GuitarBus;
                        break;
                    case BusType::Keys:
                        role = TrackRole::KeysBus;
                        break;
                    case BusType::Melody:
                        role = TrackRole::Strings;
                        break;
                    default:
                        break;
                }
            ...
            if (diff.hasProblems() || !diff.praises.empty()) results.push_back(std::move(diff));
        });
    ...
    //  CHECK: Gain
    // ═══════════════════════════════════════════════════════════════════════════
    void SemanticComparator::checkGain(SemanticDiff& diff, const TrackSpectralProfile& p, const ExpectedProfile& e)
    {
        float dev = e.getPeakDeviation(p.peakDb);

        if (p.peakDb > -0.5f) {
            // Clipping
            diff.issues.push_back(makeIssue(SemanticIssue::Severity::Critical,
                                            SemanticIssue::Domain::Gain,
                                            diff.trackName,
                                            diff.role,
            ...
            diff.issues.push_back(makeIssue(
                SemanticIssue::Severity::Warning,
                SemanticIssue::Domain::Gain,
                diff.trackName,
                diff.role,
                juce::String(dev, 1) + " dB above target peak of " + formatDb(e.peakTargetDb),
                p.peakDb,
                e.peakTargetDb,
                "Reduce gain " + juce::String(dev * 0.7f, 1) + " dB (target peak: " + formatDb(e.peakTargetDb) + ")"));
        }
        else if (dev < -e.peakTolerance && p.peakDb > -50.0f) {
            // Demasiado bajo
            diff.issues.push_back(
                makeIssue(SemanticIssue::Severity::Warning,
                          SemanticIssue::Domain::Gain,
                          diff.trackName,
                          diff.role,
                          juce::String(-dev, 1) + " dB below target peak of " + formatDb(e.peakTargetDb),
                          p.peakDb,
                          e.peakTargetDb,
            ...
            // Bien!
            diff.praises.push_back(
                makePraise(SemanticIssue::Severity::Praise,
                           SemanticIssue::Domain::Gain,
                           diff.trackName,
                           diff.role,
                           "Nivel optimo: " + formatDb(p.peakDb) + " (target: " + formatDb(e.peakTargetDb) + ")"));
        }
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    void SemanticComparator::checkDynamics(SemanticDiff& diff, const TrackSpectralProfile& p, const ExpectedProfile& e)
    {
        if (p.crestDb <= 0.0f) return; // Sin datos

        if (p.crestDb > (e.crestTargetDb + e.crestTolerance)) {
            // Demasiado dinámico
            diff.issues.push_back(makeIssue(SemanticIssue::Severity::Warning,
                                            SemanticIssue::Domain::Dynamics,
                                            diff.trackName,
                                            diff.role,
                                            "Crest " + juce::String(p.crestDb, 1)
                                                + " dB — demasiado din\xC3\xA1"
                                                  "mico "
                                                  "(target: "
                                                + juce::String(e.crestTargetDb, 1)
                                                + " dB). "
                                                  "Considera compresi\xC3\xB3"
                                                  "n suave",
                                            p.crestDb,
                                            e.crestTargetDb,
                                            "Prueba compresor 2:1, threshold -"
                                                + juce::String(std::abs(p.peakDb + 6.0f), 1)
                                                + " dB, attack 10ms, release 100ms"));
        }
        else if (p.crestDb < (e.crestTargetDb - e.crestTolerance) && p.crestDb > 1.0f) {
            // Demasiado comprimido
            diff.issues.push_back(makeIssue(SemanticIssue::Severity::Info,
                                            SemanticIssue::Domain::Dynamics,
                                            diff.trackName,
                                            diff.role,
                                            "Crest " + juce::String(p.crestDb, 1) + " dB — bajo para un "
                                                + roleDisplayName(diff.role)
                                                + " (target: " + juce::String(e.crestTargetDb, 1)
                                                + " dB). "
            ...
            diff.praises.push_back(makePraise(SemanticIssue::Severity::Praise,
                                              SemanticIssue::Domain::Dynamics,
                                              diff.trackName,
                                              diff.role,
        ...
        if (hasBandLevel && actualPeak > -90.0f) {
            for (int b = 0; b < 6; ++b) {
                ...
                if (absDev > 12.0f) sev = SemanticIssue::Severity::Critical;
                    ...
                    sev = SemanticIssue::Severity::Warning;
                else
                    sev = SemanticIssue::Severity::Info;
                ...
                juce::String message =
                    bandName + juce::String(": ") + direction + " " + juce::String(amount, 1) + "dB vs target.";
                ...
                if (b == 0 && isCut && deviation > 10.0f) {
                    ...
                    recommendation = juce::String(actionWord) + " " + juce::String(amount, 1) + "dB con Bell en "
                                     + freqStr + " Q=" + juce::String(q, 1);
                }
                else if (b <= 1 && isCut) {
                    // Graves demasiado fuertes → shelf o HPF
                    recommendation = juce::String(actionWord) + " " + juce::String(amount, 1) + "dB con "
                                     + juce::String(filterType) + " en " + freqStr + " Q=" + juce::String(q, 1);
                }
                else if (b >= 4 && !isCut) {
                ...
                auto issue           = makeIssue(sev,
                                                 SemanticIssue::Domain::Spectral,
                                                 diff.trackName,
                                                 diff.role,
                                                 message,
                                                 actualRelative,
        ...
        if (hasCrestData) {
            ...
            if (dominantBand >= 0 && maxCrest >= 1.0f) {
                ...
                if (dominantBand != expectedDominant) {
                    ...
                    if (actualRel < -8.0f) {
                        diff.issues.push_back(makeIssue(
                            SemanticIssue::Severity::Info,
                            SemanticIssue::Domain::Spectral,
                            diff.trackName,
                            diff.role,
                            "Banda dominante es " + juce::String(SpectralProfiler::kBandNames[dominantBand])
                                + " pero se esperaba " + juce::String(SpectralProfiler::kBandNames[expectedDominant])
                                + " para un " + roleDisplayName(diff.role),
                            p.crestPerBand[dominantBand],
                            p.crestPerBand[expectedDominant],
                ...
                if (lowFreqEnergy > highFreqEnergy + 6.0f && diff.role != TrackRole::BassSub
                    ...
                    diff.issues.push_back(
                        makeIssue(SemanticIssue::Severity::Info,
                                  SemanticIssue::Domain::Spectral,
                                  diff.trackName,
                                  diff.role,
                                  "Energia grave inusualmente alta para un " + roleDisplayName(diff.role),
                                  lowFreqEnergy,
                                  highFreqEnergy,
                ...
                if ((diff.role == TrackRole::BassSub || diff.role == TrackRole::Bass808) && lowFreqEnergy < 6.0f) {
                    diff.issues.push_back(makeIssue(SemanticIssue::Severity::Warning,
                                                    SemanticIssue::Domain::Spectral,
                                                    diff.trackName,
                                                    diff.role,
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    void
    SemanticComparator::checkStereoWidth(SemanticDiff& diff, const TrackSpectralProfile& p, const ExpectedProfile& e)
    {
        for (int b = 0; b < 6; ++b) {
            if (!e.isWidthInBandOk(b, p.stereoWidthPerBand[b])) {
                ...
                if (b <= 1) {
                    ...
                    diff.issues.push_back(makeIssue(SemanticIssue::Severity::Warning,
                                                    SemanticIssue::Domain::StereoWidth,
                                                    diff.trackName,
                                                    diff.role,
        ...
        if (!p.isMonoCompatible) {
            diff.issues.push_back(makeIssue(SemanticIssue::Severity::Info,
                                            SemanticIssue::Domain::MonoCompat,
                                            diff.trackName,
                                            diff.role,
    ...
    //  CHECK: Transient
    // ═══════════════════════════════════════════════════════════════════════════
    void SemanticComparator::checkTransient(SemanticDiff& diff, const TrackSpectralProfile& p, const ExpectedProfile& e)
    {
        if (p.transientRatio <= 0.0f) return;

        if (p.transientRatio > e.transientRatioMax) {
            diff.issues.push_back(makeIssue(SemanticIssue::Severity::Info,
                                            SemanticIssue::Domain::Transient,
                                            diff.trackName,
                                            diff.role,
            ...
            diff.issues.push_back(makeIssue(SemanticIssue::Severity::Info,
                                            ...
                                            diff.role,
                                            "Transiente ratio " + juce::String(p.transientRatio, 2)
                                                + " — muy bajo, el ataque se pierde " + "(target: > "
                                                + juce::String(e.transientRatioMin, 1) + ")",
                                            p.transientRatio,
                                            e.transientRatioMin,
                                            "Usa un transient shaper con attack mas rapido o "
                                            "reduce el release del compresor"));
        }
        else if (p.transientRatio >= e.transientRatioMin && p.transientRatio <= e.transientRatioMax) {
            diff.praises.push_back(makePraise(SemanticIssue::Severity::Praise,
                                              SemanticIssue::Domain::Transient,
                                              diff.trackName,
                                              diff.role,
                                              "Ataque natural: transient ratio " + juce::String(p.transientRatio, 2)));
        }
    }
