...
SessionMapEntry SessionMapEntry::fromJson(const juce::DynamicObject& obj)
{
    ...
    if (obj.hasProperty("confidence")) entry.confidence = static_cast<float>(static_cast<double>(obj.getProperty("confidence")));
    if (obj.hasProperty("peakDb"))    entry.peakDb    = static_cast<float>(static_cast<double>(obj.getProperty("peakDb")));
    ...
    if (obj.hasProperty("tracks")) {
        ...
        if (trackArr != nullptr) {
            for (int i = 0; i < trackArr->size(); ++i) {
                ...
                if (trackObj != nullptr)
                    cat.tracks.push_back(SessionMapEntry::fromJson(*trackObj));
            }
        }
    }
    return cat;
    ...
    if (obj.hasProperty("categories")) {
        ...
        if (catArr != nullptr) {
            for (int i = 0; i < catArr->size(); ++i) {
                ...
                if (catObj != nullptr)
                    map.categories.push_back(SessionMapCategory::fromJson(*catObj));
            }
        }
    }
    return map;
...
juce::String SessionMap::toText() const
{
    ...
    juce::String map;
    map += "\xF0\x9F\x97\xBA **SESSION MAP** \xE2\x80\x94 "
           + juce::String(totalTracks) + " track" + (totalTracks != 1 ? "s" : "")
           + "\n\n";
    ...
    for (const auto& cat : categories) {
        ...
        for (int e = 0; e < (int)cat.tracks.size(); ++e) {
            ...
            // Conector: ├── o └──
            map += isLast ? "  \xE2\x94\x94\xE2\x94\x80\xE2\x94\x80 "
                          : "  \xE2\x94\x9C\xE2\x94\x80\xE2\x94\x80 ";

            // Rol
            if (entry.roleName.isNotEmpty())
                map += "\xE2\x97\x8F " + entry.roleName;
            else
                map += "\xE2\x97\x8B Unknown";

            // Nombre de pista (si no está duplicado en el rol)
            if (!entry.trackName.containsIgnoreCase(entry.roleName))
                map += "  \xE2\x86\x92 " + entry.trackName;

            // Indicador de nivel
            if (entry.hasSignal) {
                if (entry.peakDb > -0.5f)
                    map += "  \xF0\x9F\x94\xB4";
