...
TrackSpectralProfile SpectralProfiler::computeProfile(const TrackAudioResult& result) noexcept
{
    ...
    if (p.rmsDb > -90.0f && p.peakDb > -90.0f)
        p.crestDb = p.peakDb - p.rmsDb;
    ...
    for (int b = 0; b < 6; ++b)
        totalWidth += result.stereoWidthPerBand[b];
    ...
    p.isBright = (p.bandLevelDb[4] > p.bandLevelDb[2]
                  || p.bandLevelDb[5] > p.bandLevelDb[2]);
...
TrackSpectralProfile SpectralProfiler::computeProfile(const TrackTelemetry& telem) noexcept
{
    ...
    float totalWidth = 0.0f;
    for (int b = 0; b < 6; ++b)
        totalWidth += telem.stereoWidthPerBand[b];
    p.avgStereoWidth = totalWidth / 6.0f;
...
float SpectralProfiler::estimateFundamental(const float bandLevelDb[6]) noexcept
{
    ...
    for (int b = 0; b < 6; ++b) {
        if (bandLevelDb[b] > -40.0f)
            return kBandCenterFreqs[b];
    }
    return 0.0f;
}
...
{
    return (crest > 12.0f && transientRatio > 1.5f)
        || crest > 16.0f
        || transientRatio > 2.5f;
}
...
float SpectralProfiler::subBassEnergy(const float bandLevelDb[6]) noexcept
{
    ...
    int count = 0;
    for (int b = 0; b < 2; ++b) {
        if (bandLevelDb[b] > -90.0f) { sum += bandLevelDb[b]; ++count; }
    }
...
TrackRole SpectralProfiler::inferTrackRole(const TrackSpectralProfile& p) noexcept
{
    ...
    if (p.hasTransientCharacter)
    {
        ...
        {
            if (p.transientRatio > 2.5f && p.attackTimeMs < 8.0f)
        ...
        if (hasStrongHigh && hToM > 6.0f && p.transientRatio > 3.0f)
        {
            ...
            if (p.avgStereoWidth > 0.3f || corr < 0.6f)
                return TrackRole::HiHatOpen;
        ...
        if (hasStrongHigh && hToM > 8.0f && p.transientRatio > 2.0f
            && corr > 0.7f && p.releaseTimeMs > 80.0f)
        {
        ...
        {
            if (hToM > 3.0f)
                return TrackRole::SnareTrap;
            if (p.releaseTimeMs < 80.0f && p.releaseTimeMs > 0.0f)
                return TrackRole::Snare;
            return TrackRole::Snare;
        }

        // ─── Clap: medios, transientRatio muy alto, corr media ─────────
        if (midE > -50.0f && p.transientRatio > 3.5f && !hasStrongSub && corr < 0.8f)
            return TrackRole::Clap;
        ...
        if (midE > -40.0f && (midE - highE) > 3.0f && p.isMonoCompatible && corr > 0.8f)
        {
            ...
            if (bToM > 2.0f)
                return TrackRole::TomFloor;
            return TrackRole::Tom;
        }

        // ─── Crash: ancho espectro, baja correlacion (stereo amplio) ──
        if (!hasStrongSub && (p.avgStereoWidth > 0.3f || corr < 0.5f))
            return TrackRole::Crash;
    ...
    if (p.hasSustainedCharacter || p.transientRatio < 1.5f)
    {
        ...
        if (p.avgStereoWidth > 0.35f && p.crestDb < 8.0f && midE > -50.0f)
        {
            ...
            if (corr < 0.6f || hasStrongSub || bToM > 0.0f)
                return TrackRole::SynthPad;
            return TrackRole::Strings;
        }

        // ─── Ambiente / Ruido: stereo amplio, baja corr, silencioso ───
        if ((p.avgStereoWidth > 0.4f || corr < 0.3f) && p.peakDb < -14.0f)
            return TrackRole::FxAmbience;

        // ─── Organo: sostenido, mono, medios, poca dispersion stereo ──
        if (midE > -45.0f && p.isMonoCompatible && p.crestDb < 7.0f && corr > 0.8f)
            return TrackRole::KeysOrgan;

        // ─── Bajo synth: variado, crest bajo-medio ────────────────────
        if (hasStrongSub && bToM > 3.0f && p.crestDb < 12.0f)
            return TrackRole::BassSynth;
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    if (p.crestDb >= 6.0f && p.crestDb <= 14.0f && !hasStrongSub
        && midE > -45.0f && highE > midE - 8.0f)
    {
        // VozPrincipal: centrada, correlacion alta, mono
        if (corr > 0.8f && p.avgStereoWidth < 0.15f)
            return TrackRole::VozPrincipal;
        // VozFondo: moderadamente estereo
        if (corr > 0.4f || p.avgStereoWidth < 0.4f)
    ...
    if (p.crestDb >= 8.0f && p.crestDb <= 16.0f && midE > -45.0f)
    {
        ...
        if (p.transientRatio > 1.5f && hasStrongHigh)
            return TrackRole::GuitarAcoustic;
        // Guitarra electrica: sostenida, crest medio, corr media
        if (p.transientRatio < 1.5f && bToM < 3.0f && corr < 0.85f)
            return TrackRole::GuitarElectric;
        // Guitarra lider: presencia, crest alto, mas brillante
        if (p.transientRatio > 1.8f && highE > midE - 3.0f)
            return TrackRole::GuitarLead;
        // Guitarra ritmica
    ...
    if (midE > -45.0f)
    {
        ...
        if (p.crestDb >= 8.0f && p.crestDb <= 16.0f && hasStrongHigh)
            return TrackRole::KeysPiano;
        // Synth lead: presencia, crest medio-alto, corr variable
        if (p.transientRatio > 1.5f && highE > midE - 3.0f)
            return TrackRole::SynthLead;
        // Synth pluck: ataque rapido, decay, ancho
        if (p.transientRatio > 2.0f && p.avgStereoWidth > 0.2f)
            return TrackRole::SynthPluck;
        // Rhodes/Wurly: suave, crest bajo-medio, corr media
        if (p.crestDb < 10.0f && corr > 0.5f && corr < 0.85f)
            return TrackRole::KeysElectric;
    }
    ...
    if (midE > -45.0f && hasStrongHigh && hToM > 3.0f && p.crestDb >= 8.0f && corr > 0.7f)
        return TrackRole::Brass;

    // Strings: sostenido, stereo, crest bajo
    if (midE > -45.0f && p.avgStereoWidth > 0.3f && p.crestDb < 10.0f)
        return TrackRole::Strings;

    // Vientos (Winds): crest medio, presencia sin ser brillante
    if (midE > -45.0f && hasStrongHigh && p.crestDb < 12.0f && corr > 0.7f)
        return TrackRole::Winds;

    // Riser: energia creciendo de graves a agudos (sweep up)
    // Detectado por: subE < midE < highE (energia aumenta con la frecuencia)
    if (subE < -50.0f && midE > subE + 6.0f && highE > midE + 3.0f && p.crestDb < 14.0f)
...
TrackRole SpectralProfiler::inferTrackRole(const TrackSpectralProfile& p, const juce::String& genre) noexcept
{
    ...
    if (g == "reggaeton" || g == "latin" || g == "dancehall" || g == "reggae")
    {
        ...
        if (role == TrackRole::BassSub || role == TrackRole::BassFinger)
            return TrackRole::Bass808;
        if (role == TrackRole::BassPick)
            return TrackRole::BassFinger;  // Reggaeton usa finger style mas que pick
        return role;
    }
    ...
    if (g == "trap" || g == "drill" || g == "hip hop" || g == "hiphop")
    {
        ...
        if (role == TrackRole::Snare)
            return TrackRole::SnareTrap;
        if (role == TrackRole::BassFinger || role == TrackRole::BassPick)
            return TrackRole::Bass808;
        if (role == TrackRole::BassSub)
            return TrackRole::Bass808;
        if (role == TrackRole::HiHatOpen)
            return TrackRole::HiHat;   // Trap usa hi-hats cerrados rapidos
        return role;
    }
    ...
    if (g == "rock" || g == "metal" || g == "punk" || g == "alternative" || g == "indie")
    {
        ...
        if (role == TrackRole::GuitarAcoustic)
            return TrackRole::GuitarElectric;  // Rock prioriza electrica
        if (role == TrackRole::SynthPad || role == TrackRole::SynthLead)
            return TrackRole::KeysPiano;  // Mas probable piano que synth
        if (role == TrackRole::Kick808)
            return TrackRole::Kick;
        return role;
    }
    ...
    {
        if (role == TrackRole::BassPick || role == TrackRole::BassFinger)
            return TrackRole::BassSynth;
        if (role == TrackRole::Kick)
            return TrackRole::Kick808;  // Kicks electronicos son mas "808 style"
        if (role == TrackRole::GuitarAcoustic || role == TrackRole::GuitarElectric)
            return TrackRole::SynthLead;  // Synth reemplaza guitarra
        if (role == TrackRole::Brass)
            return TrackRole::SynthLead;  // Brass sintetizado
        return role;
    }
    ...
    {
        if (role == TrackRole::GuitarElectric)
            return TrackRole::GuitarAcoustic;
        if (role == TrackRole::BassPick || role == TrackRole::BassSynth)
            return TrackRole::BassFinger;
        ...
        if (role == TrackRole::SynthLead || role == TrackRole::SynthPad)
            return TrackRole::KeysPiano;
        if (role == TrackRole::SynthPluck)
            return TrackRole::KeysPiano;
        return role;
    }
    ...
    if (g == "r&b" || g == "rnb" || g == "soul" || g == "funk")
    {
        if (role == TrackRole::BassPick)
            return TrackRole::BassFinger;
        if (role == TrackRole::Bass808)
            return role;  // R&B moderno usa 808s
        if (role == TrackRole::KeysOrgan)
            return role;  // Hammond is common in soul/funk
        if (role == TrackRole::KeysPiano)
            return TrackRole::KeysElectric;  // Rhodes/Wurly is more R&B
        return role;
    }
    ...
    if (g == "pop" || g == "lo-fi" || g == "lofi")
    {
        if (role == TrackRole::BassPick)
            return TrackRole::BassFinger;
        if (role == TrackRole::GuitarElectric)
            return TrackRole::GuitarAcoustic;  // Lofi usa acustica
        return role;
    }
    ...
    {
        if (role == TrackRole::GuitarElectric || role == TrackRole::GuitarLead)
            return TrackRole::GuitarAcoustic;
        if (role == TrackRole::BassPick || role == TrackRole::BassSynth)
            return TrackRole::BassFinger;
        return role;
