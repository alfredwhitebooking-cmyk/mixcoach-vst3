                                       ...
                                       int spectralBand)
{
    ...
    if (existing.slotIndex == slotIndex
        && existing.status == TrackRecommendation::Status::Pending
    ...
    LogHelper::writeToLog("[CoachEngine] Recomendacion creada: slot="
                          + juce::String(slotIndex) + " \"" + trackName
                          + "\" " + action + " [" + verifyMetric + "] "
                          + "(" + juce::String(beforeValue, 1) + " dB \xE2\x86\x92 "
                          + juce::String(expectedAfter, 1) + " dB)");
}
...
void CoachEngine::storeRecommendationFromSemanticAnalysis()
{
    ...
    for (const auto& diff : diffs)
    {
        for (const auto& issue : diff.issues)
        {
            ...
            if (msg.contains("eq") || msg.contains("freq")
                || msg.contains("hz") || msg.contains("espectral")
                || msg.contains("spectral") || msg.contains("tonal")
                || msg.contains("balance"))
            {
                domain = TrackRecommendation::Domain::Tonal;
                verifyMetric = "band_" + juce::String(diff.spectralFocusBand);
            }
            else if (msg.contains("compres") || msg.contains("crest")
                     || msg.contains("dinam") || msg.contains("dynamics")
                     || msg.contains("peak") || msg.contains("rms"))
            {
                domain = TrackRecommendation::Domain::Dynamics;
                verifyMetric = "crest";
            }
            else if (msg.contains("pan") || msg.contains("estereo")
                     || msg.contains("stereo") || msg.contains("phase")
                     || msg.contains("correlacion") || msg.contains("correlation"))
            {
            ...
            registry.forEachActive([&](const SlotInfo& info) {
                ...
                if (name == diff.trackName || info.slotIndex == diff.slotIndex)
                    slotFound = info.slotIndex;
            });
            ...
            else if (domain == TrackRecommendation::Domain::Dynamics)
            {
                ...
                if (beforeValue <= 0.0f)
                    beforeValue = 10.0f; // Fallback genérico
            }
            ...
            float delta = 3.0f;
            auto lowerMsg = msg;
            bool isReduction = lowerMsg.contains("reduce") || lowerMsg.contains("baja")
                            || lowerMsg.contains("cut") || lowerMsg.contains("menos")
                            || lowerMsg.contains("too much") || lowerMsg.contains("demasiado")
                            || lowerMsg.contains("exceso") || lowerMsg.contains("excess")
                            || lowerMsg.contains("clipping") || lowerMsg.contains("sobre-comp")
                            || lowerMsg.contains("over-comp") || lowerMsg.contains("overcomp")
                            || lowerMsg.contains("reduce") || lowerMsg.contains("bajar");
            ...
            storeRecommendation(
                slotFound, diff.trackName, domain,
                issue.recommendation.isEmpty() ? issue.message : issue.recommendation,
                beforeValue, expectedAfter, delta,
                verifyMetric, 0.0f, diff.spectralFocusBand);
        }
    }
}
...
void CoachEngine::verifyTrackCorrections()
{
    ...
    for (int i = 0; i < SlotRegistry::kMaxSlots; ++i)
    {
        ...
        if (rec.verifyMetric == "peak")
            currentValue = juce::jmax(telem.peakLeft, telem.peakRight);
        else if (rec.verifyMetric.startsWith("band_"))
        {
            int bandIdx = rec.verifyMetric.substring(5).getIntValue();
            if (bandIdx >= 0 && bandIdx < 30)
                currentValue = telem.spectrum[bandIdx];
            else
                valid = false;
        }
            ...
            && std::abs(initialToTarget) > 0.5f)
        {
            ...
            if (rec.verifyRetries >= at.maxRetriesBeforeIgnore)
            {
                ...
                LogHelper::writeToLog("[CoachEngine] Recomendacion IGNORADA: "
                    + rec.trackName + " \"" + rec.action.substring(0, 80) + "\"");
            }
        ...
        else if (appliedRatio >= at.underApplyTarget && appliedRatio <= at.overApplyTarget)
        {
            ...
            if (trackChangeCallback_)
                trackChangeCallback_(i, rec.trackName, rec.action,
                                     rec.verifyInitial, currentValue);

            LogHelper::writeToLog("[CoachEngine] Recomendacion APLICADA: "
                + rec.trackName + " \"" + rec.action.substring(0, 60)
        ...
        else if (appliedRatio > at.overApplyTarget)
        {
            ...
            feedbackMsg = "\xF0\x9F\x91\x80 Veo que en **" + rec.trackName + "** te pasaste "
                        + juce::String(overBy, 1) + " dB del target. "
                        + "Suger\xC3\xAD " + juce::String(std::abs(initialToTarget), 1)
                        + " dB, ajustaste " + juce::String(std::abs(actualDelta), 1)
                        + " dB. Prueba compensar subiendo ~" + juce::String(overBy * 0.5f, 1) + " dB.";
            ...
            LogHelper::writeToLog("[CoachEngine] Recomendacion SOBRE-APLICADA: "
                + rec.trackName + " (by " + juce::String(overBy, 1) + " dB)");
        }
        else if (appliedRatio >= at.goodStartRatio && appliedRatio < at.underApplyTarget)
        {
            ...
            LogHelper::writeToLog("[CoachEngine] Recomendacion SUB-APLICADA: "
                + rec.trackName + " (ratio=" + juce::String(appliedRatio, 2)
                + ", falta " + juce::String(remainingDb, 1) + " dB)");
        }
        ...
        if (statusFinalized)
        {
            ...
            if (correctionHistory_.size() % 10 == 0)
                recalcAdaptiveThresholds();
        }
        ...
        else if (newStatus != rec.status)
        {
            ...
            if (newStatus == TrackRecommendation::Status::Superseded
                && rec.feedbackMessage.isNotEmpty())
            {
...
void CoachEngine::generateFollowUp(int slotIndex, const TrackRecommendation& oldRec)
{
    ...
    juce::String direction = (residualDelta > 0)
        ? "Sube " + juce::String(residualDb, 1) + " dB"
        : "Baja " + juce::String(residualDb, 1) + " dB";

    juce::String followUpAction;
    switch (oldRec.domain) {
        case TrackRecommendation::Domain::Gain:
            followUpAction = direction + " en el fader de " + oldRec.trackName
                + " (te pasaste por " + juce::String(residualDb, 1) + " dB)";
            break;
        case TrackRecommendation::Domain::Tonal:
            followUpAction = direction + " en EQ de " + oldRec.trackName
                + " (ajuste fino, Q=1.8)";
            break;
        case TrackRecommendation::Domain::Dynamics:
            followUpAction = direction + " el threshold del compresor en "
                + oldRec.trackName + " (ajuste fino)";
            break;
        case TrackRecommendation::Domain::Spatial:
            followUpAction = direction + " el pan de " + oldRec.trackName
                + " (centrando un poco)";
            break;
        default:
    ...
    if (!correctionHistory_.empty())
        correctionHistory_.back().hadFollowUp = true;

    // Almacenar como nueva recomendación (con delta corregido)
    storeRecommendation(slotIndex, oldRec.trackName,
                        oldRec.domain,
                        followUpAction,
                        newBeforeValue,
                        newExpectedAfter,
    ...
    LogHelper::writeToLog("[CoachEngine] Follow-up generado: slot="
        + juce::String(slotIndex) + " residual="
        + juce::String(residualDb, 1) + " dB ("
        + (residualDelta > 0 ? "needs +" : "needs ")
        + juce::String(residualDb, 1) + ")");
}
...
const TrackRecommendation* CoachEngine::getMostUrgentRecommendation() const
{
    ...
    for (int i = 0; i < SlotRegistry::kMaxSlots; ++i)
    {
        ...
        if (rec.slotIndex != i || rec.status != TrackRecommendation::Status::Pending)
            continue;
...
void CoachEngine::dismissRecommendation(int slotIndex)
{
    ...
    {
        respondWith("No hay recomendación activa para este track.",
    ...
    if (rec.status == TrackRecommendation::Status::Pending)
    {
        ...
        juce::String msg = "\xE2\x8F\xAD **Recomendación descartada:** "
            + rec.trackName + " \"" + actionPreview + "\"\n"
...
{
    if (correctionHistory_.size() < 5)
        return; // No hay suficiente historial para ajustar
    ...
    {
        switch (entry.finalStatus) {
            case TrackRecommendation::Status::Applied:      totalApplied++; break;
            case TrackRecommendation::Status::OverApplied:  totalOver++;    break;
            case TrackRecommendation::Status::UnderApplied: totalUnder++;   break;
            case TrackRecommendation::Status::Ignored:      totalIgnored++; break;
            default: break;
        }
    }
    ...
    LogHelper::writeToLog("[CoachEngine] Adaptive thresholds: applied="
        + juce::String(totalApplied) + " over=" + juce::String(totalOver)
        + " under=" + juce::String(totalUnder) + " ignored="
        + juce::String(totalIgnored) + " (total=" + juce::String(total) + ")");
    ...
    if (overRatio > 0.5f)
    {
        ...
        LogHelper::writeToLog("[CoachEngine] → Perfil sobre-aplicador detectado: "
            "thresholds relajados (under=" + juce::String(adaptiveThresholds_.underApplyTarget, 2)
    ...
    else if (exactRatio > 0.5f)
    {
        ...
        LogHelper::writeToLog("[CoachEngine] → Perfil preciso detectado: "
            "thresholds estrechos (under=" + juce::String(adaptiveThresholds_.underApplyTarget, 2)
            + ", over=" + juce::String(adaptiveThresholds_.overApplyTarget, 2) + ")");
    }
    ...
    if (ignoreRatio > 0.6f)
    {
        ...
        LogHelper::writeToLog("[CoachEngine] → Perfil que ignora detectado: "
    ...
    else if (ignoreRatio < 0.2f && exactRatio > 0.4f)
    {
        ...
        LogHelper::writeToLog("[CoachEngine] → Usuario receptivo: "
    ...
    if (underRatio > 0.4f && overRatio < 0.3f)
    {
        ...
        LogHelper::writeToLog("[CoachEngine] → Perfil sub-aplicador detectado: "
...
void CoachEngine::detectUnpromptedChanges(int focusSlotIndex)
{
    ...
    auto checkSlot = [&](int idx) {
        ...
        if (rec.slotIndex == idx && rec.status == TrackRecommendation::Status::Pending)
            return;
        ...
        if (trackName.isEmpty())
            trackName = "Track " + juce::String(idx + 1);
        ...
        // Log
        LogHelper::writeToLog("[CoachEngine] Cambio detectado (no solicitado): "
            + trackName + " " + direction + " " + juce::String(absChange, 1) + " dB");
    ...
    registry.forEachActive([&](const SlotInfo& info) {
        ...
        auto& state = trackStates_[idx];
        float currentPeak = juce::jmax(telem.peakLeft, telem.peakRight);
        if (currentPeak > -90.0f && currentPeak < 0.0f)
            state.lastPeakDb = currentPeak;
    });
}
