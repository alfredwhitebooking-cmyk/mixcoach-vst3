...
{
    juce::String s;
    s += "  " + statusEmoji() + " Paso " + juce::String(stepNumber)
         + "/" + juce::String(totalSteps) + " — ";

    switch (domain) {
        case Domain::Gain:     s += "GANANCIA"; break;
        case Domain::Tonal:    s += "TONAL"; break;
        case Domain::Dynamics: s += "DINAMICA"; break;
        case Domain::Spatial:  s += "ESPACIAL"; break;
        case Domain::Loudness: s += "LOUDNESS"; break;
    }

    s += ": " + description + "\n";
    s += "       Target: " + juce::String(targetValue, 1) + " | Current: "
         + juce::String(currentValue, 1) + " | Progreso: "
         + juce::String(static_cast<int>(getProgress() * 100.0f)) + "%\n";
    s += "       " + suggestion + "\n";
    return s;
}
...
{
    switch (status) {
        case PlanStatus::Pending:    return "\xe2\x97\x8b";     // ○
        case PlanStatus::InProgress: return "\xf0\x9f\x94\x84"; // 🔄
        case PlanStatus::Completed:  return "\xe2\x9c\x85";     // ✅
        case PlanStatus::Skipped:    return "\xe2\x8f\x8f";     // ⏏
    }
    return "?";
}
...
void PlanManager::updateFromGaps(const std::vector<DomainGap>& gaps)
{
    ...
    if (steps_.empty())
    {
        ...
        for (const auto& gap : gaps)
        {
            ...
            if (gap.severity == GapSeverity::Praise)
                continue;
        ...
        LogHelper::writeToLog("[PlanManager] Nuevo plan creado: "
                              + juce::String(total) + " pasos");
    }
    else
    {
...
void PlanManager::reevaluate(const std::vector<DomainGap>& gaps)
{
        ...
        for (const auto& gap : gaps)
        {
            if (gap.domain == step.domain && gap.metric == step.metric)
            {
                    ...
                    || (gapSeverity == GapSeverity::Info && step.originalSeverity >= GapSeverity::Warning))
                {
                    ...
                    completedCount++;
                    updatedCount++;
                    LogHelper::writeToLog("[PlanManager] Paso completado: "
                                          + step.metric + " ("
                                          + juce::String(step.initialValue, 1) + " → "
                                          + juce::String(step.currentValue, 1) + ")");
                }
                else if (gapSeverity == GapSeverity::Info
                         || std::abs(gap.normalisedGap) < step.getProgress())
                {
    ...
    if (updatedCount > 0)
        LogHelper::writeToLog("[PlanManager] Reevaluacion: " + juce::String(updatedCount)
                              + " pasos actualizados, " + juce::String(completedCount)
                              + " completados");
}
...
void PlanManager::markCompleted(int stepIndex)
{
    if (stepIndex >= 0 && stepIndex < static_cast<int>(steps_.size()))
    {
        ...
        LogHelper::writeToLog("[PlanManager] Paso marcado manual como completado: "
                              + steps_[stepIndex].metric);
    }
}
...
void PlanManager::markSkipped(int stepIndex)
{
    if (stepIndex >= 0 && stepIndex < static_cast<int>(steps_.size()))
    {
        ...
        steps_[stepIndex].completedAtUs = juce::Time::getMillisecondCounter() * 1000;
        LogHelper::writeToLog("[PlanManager] Paso saltado: "
                              + steps_[stepIndex].metric);
    }
}
...
int PlanManager::findStep(const DomainGap& gap) const noexcept
{
    ...
    {
        if (steps_[i].domain == gap.domain && steps_[i].metric == gap.metric)
            return i;
    }
    return -1;
}
...
void PlanManager::loadFromJson(const juce::DynamicObject& obj)
{
    ...
    for (int i = 0; i < stepsArr->size(); ++i)
    {
        ...
        step.originalSeverity = static_cast<GapSeverity>(static_cast<int>(stepObj->getProperty("originalSeverity")));
    ...
    if (!steps_.empty())
        LogHelper::writeToLog("[PlanManager] Plan restaurado: "
                              + juce::String((int)steps_.size()) + " pasos");
}
