...
juce::String ImpactData::toImpactMessage() const
{
    ...
    {
        if (!hasAny) { msg += "\xF0\x9F\x93\x8A "; hasAny = true; }
        msg += "Correlaci\xC3\xB3n: " + juce::String(correlationBefore, 2)
             + " \xE2\x86\x92 " + juce::String(correlationAfter, 2) + " (";
        if (corrDiff > 0) msg += "mejor\xC3\xB3";
        else              msg += "empeor\xC3\xB3";
    ...
    if (lufsBefore > -80.0f && lufsAfter > -80.0f)
    {
        ...
        if (std::abs(lufsDiff) >= 1.0f)
        {
            ...
            msg += "LUFS: " + juce::String(lufsBefore, 1)
                 + " \xE2\x86\x92 " + juce::String(lufsAfter, 1)
                 + " LU (" + (lufsDiff > 0 ? "m\xC3\xA1s fuerte" : "m\xC3\xA1s suave") + ")";
        }
    }
    ...
    if (std::abs(widthDiff) >= 0.08f && stereoWidthBefore > 0.0f && stereoWidthAfter > 0.0f)
    {
        ...
        msg += "Ancho est\xC3\xA9reo: " + juce::String(stereoWidthBefore, 2)
             + " \xE2\x86\x92 " + juce::String(stereoWidthAfter, 2)
             + " (" + (widthDiff > 0 ? "\xC3\xA1" : "se") + "ampli\xC3\xB3" // split for readability
             + ")";
    }
    ...
    if (crestBefore > 0.0f && crestAfter > 0.0f)
    {
        ...
        if (std::abs(crestDiff) >= 2.0f)
        {
            ...
            msg += "Crest: " + juce::String(crestBefore, 1)
                 + " \xE2\x86\x92 " + juce::String(crestAfter, 1)
    ...
    if (centroidBefore > 0.0f && centroidAfter > 0.0f)
    {
        ...
        if (std::abs(centroidDiff) >= 0.10f)
        {
            ...
            msg += "Centroide: " + juce::String(centroidBefore, 2)
                 + " \xE2\x86\x92 " + juce::String(centroidAfter, 2)
    ...
    if (truePeakBefore > -60.0f && truePeakAfter > -60.0f)
    {
        ...
        if (std::abs(tpDiff) >= 1.0f)
        {
            ...
            msg += "True Peak: " + juce::String(truePeakBefore, 1)
                 + " \xE2\x86\x92 " + juce::String(truePeakAfter, 1)
                 + " dBTP";
            if (truePeakAfter > -1.0f && truePeakBefore > -1.0f)
                msg += " \xE2\x9A\xA0 CLIPPING!";
    ...
    if (lraBefore > 0.0f && lraAfter > 0.0f)
    {
        ...
        if (std::abs(lraDiff) >= 2.0f)
        {
            ...
            msg += "Rango din\xC3\xA1mico: " + juce::String(lraBefore, 1)
                 + " \xE2\x86\x92 " + juce::String(lraAfter, 1)
                 + " LU (" + (lraDiff > 0 ? "mayor variedad" : "m\xC3\xA1s uniforme") + ")";
        }
    }
...
juce::String WorkflowEvent::toChatMessage(const juce::String& name) const
{
    ...
    switch (action) {
        case WorkflowAction::FaderUp:
            msg = "🎚️ **" + name + "** subiste el nivel **" + juce::String(magnitude, 1)
                  + " dB** (de " + juce::String(beforeValue, 1) + " a "
                  + juce::String(afterValue, 1) + " dB). "
                  + "Buena jugada — revisa que no enmascare otras pistas en el bus.";
            break;

        case WorkflowAction::FaderDown:
            msg = "🎚️ **" + name + "** bajaste el nivel **" + juce::String(magnitude, 1)
                  + " dB** (de " + juce::String(beforeValue, 1) + " a "
                  + juce::String(afterValue, 1) + " dB). "
        ...
        case WorkflowAction::EQBoost:
            msg = "🎛️ Detecto un realce en **" + regionLabel + "** ("
                  + juce::String(frequencyHint, 0) + " Hz) en **" + name
                  + "** de **" + juce::String(magnitude, 1) + " dB**. ";
        ...
        case WorkflowAction::EQCut:
            msg = "🎛️ Detecto un corte en **" + regionLabel + "** ("
                  + juce::String(frequencyHint, 0) + " Hz) en **" + name
                  + "** de **" + juce::String(magnitude, 1) + " dB**. "
        ...
        case WorkflowAction::CompressionOn:
            msg = "⚡ **" + name + "** — el crest factor bajó **"
                  + juce::String(magnitude, 1) + " dB** (dinámica más controlada). "
                  + "Prueba attack rápido (~10ms) y release medio (~80ms) "
                  + "si buscas un sonido más ajustado.";
            break;

        case WorkflowAction::CompressionOff:
            msg = "⚡ **" + name + "** — la dinámica aumentó **"
                  + juce::String(magnitude, 1) + " dB** (crest factor más alto). "
                  + "La pista respira más natural ahora.";
            break;
        ...
        case WorkflowAction::StereoWiden:
            msg = "🌐 **" + name + "** — la imagen estéreo se ensanchó "
                  + juce::String(magnitude, 2) + ". "
                  + "Verifica compatibilidad mono si usas ensanchadores.";
            break;

        case WorkflowAction::Muted:
            msg = "🔇 **" + name + "** silenciada. "
                  + "A veces menos es más — buen criterio.";
            break;
...
juce::String WorkflowEvent::toShortSummary() const
{
    juce::String s;
    switch (action) {
        case WorkflowAction::FaderUp:        s = "nivel +" + juce::String(magnitude, 1) + "dB"; break;
        case WorkflowAction::FaderDown:      s = "nivel " + juce::String(magnitude, 1) + "dB"; break;
        case WorkflowAction::EQBoost:        s = "EQ+" + regionLabel + " " + juce::String(magnitude, 1) + "dB"; break;
        case WorkflowAction::EQCut:          s = "EQ-" + regionLabel + " " + juce::String(magnitude, 1) + "dB"; break;
        case WorkflowAction::CompressionOn:  s = "compresi\xC3\xB3" "n " + juce::String(magnitude, 1) + "dB crest"; break;
        case WorkflowAction::CompressionOff: s = "expansion " + juce::String(magnitude, 1) + "dB crest"; break;
        case WorkflowAction::PanLeft:        s = "pan L " + juce::String(magnitude, 1) + "dB"; break;
        case WorkflowAction::PanRight:       s = "pan R " + juce::String(magnitude, 1) + "dB"; break;
        case WorkflowAction::StereoWiden:    s = "stereo +" + juce::String(magnitude, 2); break;
        case WorkflowAction::Muted:          s = "muted"; break;
        case WorkflowAction::Unmuted:        s = "unmuted"; break;
        case WorkflowAction::TrackAdded:     s = "added"; break;
        case WorkflowAction::TrackRemoved:   s = "removed"; break;
        default:                             s = ""; break;
    }
    ...
    if (impact.valid)
    {
        ...
        if (std::abs(corrDiff) >= 0.10f)
        {
            s += " | corr " + juce::String(impact.correlationBefore, 2)
                 + "\xE2\x86\x92" + juce::String(impact.correlationAfter, 2);
        }
        if (impact.lufsBefore > -80.0f && impact.lufsAfter > -80.0f
            && std::abs(impact.lufsAfter - impact.lufsBefore) >= 1.0f)
        {
            s += " | LUFS " + juce::String(impact.lufsBefore, 1)
                 + "\xE2\x86\x92" + juce::String(impact.lufsAfter, 1);
        }
    }
...
juce::String WorkflowEvent::toString() const
{
    ...
    s += toShortSummary();
    s += " (mag=" + juce::String(magnitude, 1)
         + " before=" + juce::String(beforeValue, 1)
...
// ═══════════════════════════════════════════════════════════════════════════
void WorkflowDetector::updateTracking(int slotIndex,
                                       ...
                                       const juce::String& trackName)
{
    ...
    bl.crestFactor = (bl.rmsCombined > -80.0f)
                     ? bl.peakCombined - bl.rmsCombined : 0.0f;

    // L/R balance
    bl.lrBalance = (result.peakLeft > -80.0f && result.peakRight > -80.0f)
                   ? result.peakLeft - result.peakRight : 0.0f;

    // Stereo width promedio (6 bandas)
    {
        float sum = 0.0f;
        for (int b = 0; b < 6; ++b)
            sum += result.stereoWidthPerBand[b];
        bl.avgStereoWidth = sum / 6.0f;
    }

    // 30-band spectral energy
    for (int b = 0; b < 30; ++b)
        bl.bandEnergies[b] = result.bandEnergies[b];
...
std::vector<WorkflowEvent> WorkflowDetector::detectChanges(int64_t nowUs)
{
    ...
    for (int i = 0; i < SlotRegistry::kMaxSlots; ++i) {
        ...
        if (!track.baseline.valid || !track.previous.valid)
            continue;
        ...
        if (wasPrevSilent && isCurrentlySilent)
            continue;
    ...
    for (const auto& ev : newEvents) {
        if ((int)recentEvents_.size() >= kMaxRecentEvents)
...
// ═══════════════════════════════════════════════════════════════════════════
ImpactData WorkflowDetector::computeImpact(const MasterSnapshot& before,
                                            const MasterSnapshot& after) const
{
    ImpactData impact;

    // Sin datos previos válidos — no podemos calcular impacto
    if (!before.valid || !after.valid || before.timestampUs == 0)
        return impact;
...
// ═══════════════════════════════════════════════════════════════════════════
void WorkflowDetector::detectFaderChange(TrackState& track, int slotIndex,
                                          int64_t nowUs,
                                          std::vector<WorkflowEvent>& events)
{
    ...
    // Cooldown
    if (nowUs - track.lastFaderEventUs < kFaderCooldownUs)
        return;
    ...
    if (cur.peakCombined < -50.0f || prev.peakCombined < -50.0f)
        return;

    float peakDelta = cur.peakCombined - prev.peakCombined; // + = subió

    // Umbral mínimo
    if (std::abs(peakDelta) < kFaderThresholdDb)
        return;
...
// ═══════════════════════════════════════════════════════════════════════════
void WorkflowDetector::detectEQChange(TrackState& track, int slotIndex,
                                       int64_t nowUs,
                                       std::vector<WorkflowEvent>& events)
{
    juce::ignoreUnused(slotIndex);
    ...
    // Cooldown
    if (nowUs - track.lastEQEventUs < kEQCooldownUs)
        return;
    ...
    if (peakDelta > kEQLevelStabilityDb || rmsDelta > kEQLevelStabilityDb)
        return;

    float sim = spectralSimilarity(cur.bandEnergies, prev.bandEnergies);

    // EQ = cambio espectral significativo + nivel estable
    if (sim >= kSpectralDissimilarity)
        return; // Todavía es similar — no es EQ
    ...
    if (magnitudeDb < kEQSpectralThreshold)
        return;
...
// ═══════════════════════════════════════════════════════════════════════════
void WorkflowDetector::detectDynamicsChange(TrackState& track, int slotIndex,
                                             int64_t nowUs,
                                             std::vector<WorkflowEvent>& events)
{
    juce::ignoreUnused(slotIndex);
    ...
    // Cooldown
    if (nowUs - track.lastDynamicsEventUs < kDynamicsCooldownUs)
        return;
    ...
    if (cur.crestFactor <= 0.0f || prev.crestFactor <= 0.0f)
        return;
    if (cur.rmsCombined < -50.0f || prev.rmsCombined < -50.0f)
        return;

    float crestDelta = std::abs(cur.crestFactor - prev.crestFactor);

    if (crestDelta < kCrestThresholdDb)
        return;
    ...
    ev.action = (cur.crestFactor < prev.crestFactor)
                ? WorkflowAction::CompressionOn
...
// ═══════════════════════════════════════════════════════════════════════════
void WorkflowDetector::detectPanChange(TrackState& track, int slotIndex,
                                        int64_t nowUs,
                                        std::vector<WorkflowEvent>& events)
{
    juce::ignoreUnused(slotIndex);
    ...
    // Cooldown
    if (nowUs - track.lastPanEventUs < kPanCooldownUs)
        return;
...
// ═══════════════════════════════════════════════════════════════════════════
void WorkflowDetector::detectMuteChange(TrackState& track, int slotIndex,
                                         int64_t nowUs,
                                         std::vector<WorkflowEvent>& events)
{
    juce::ignoreUnused(slotIndex);
    ...
    // Cooldown
    if (nowUs - track.lastMuteEventUs < kMuteCooldownUs)
        return;
...
float WorkflowDetector::spectralSimilarity(const float a[30], const float b[30])
{
    ...
    if (activeBands < 3 || normA < 1e-10 || normB < 1e-10)
        return 1.0f;
...
void WorkflowDetector::computeRegionProfile(const float bandEnergies[30],
...
int WorkflowDetector::findWorstRegion(const float baseline[6], const float current[6],
                                       float& magnitudeDb)
{
    int worstIdx = 0;
    float worstMag = 0.0f;
    ...
    for (int r = 0; r < 6; ++r) {
        if (baseline[r] < -80.0f || current[r] < -80.0f)
            continue;
