...
namespace mixcoach {
    ...
    struct ReferenceProgress
    {
        ...
        {
            if (!hasAudio || !hasReference) return {};
            juce::String s;
            ...
            if (delta != 0.0f) s += " (" + juce::String(delta * 100.0f, 1) + "%)";
            s += "\n";
            s += "  Gaps: " + juce::String(totalGaps) + " total, " + juce::String(criticalGaps) + " critical, "
                 + juce::String(warningGaps) + " warning, " + juce::String(resolvedGaps) + " resolved\n";
            return s;
        }
    };
    ...
    public:
        ...
        using LlmResponseCallback =
            std::function<bool(const juce::String& userMessage, std::function<void(const juce::String&)>)>;
        ...
        using LlmStreamingCallback =
            std::function<bool(const juce::String& userMessage,
        ...
        void
        setStreamingCallbacks(StreamStartedCallback onStarted, StreamTokenCallback onToken, StreamEndedCallback onEnded)
        {
        ...
        using SetupLlmCallback =
            std::function<void(const juce::String& prompt, std::function<void(const juce::String&)>)>;
        ...
        void setUserInteracted() noexcept { lastUserInteractionTimeUs_ = juce::Time::getMillisecondCounter() * 1000; }
        ...
        [[nodiscard]] ReferenceAnalyzer& getReferenceAnalyzer() noexcept { return refAnalyzer_; }
        ...
        [[nodiscard]] bool hasPendingReference() const noexcept { return pendingReferencePath_.isNotEmpty(); }
        ...
        void applyReferenceAnalysis(const float* bufferL,
                                    const float* bufferR,
                                    int64_t numSamples,
                                    int numChannels,
                                    double sampleRate,
        ...
        [[nodiscard]] std::array<BusGroupSummary, kNumBuses + 1> getBusSummaries() const
        {
            return computeBusSummaries();
        }
    ...
    private:
        ...
        static constexpr int64_t kReferenceDrivenProgressCooldownUs =
            60 * 1000 * 1000; // 60s entre mensajes de progreso
        ...
        void analyzeReferenceBuffer(const float* bufferL,
                                    const float* bufferR,
                                    int64_t numSamples,
                                    int numChannels,
                                    double sampleRate,
        ...
        void computeReferenceSections(const juce::AudioBuffer<float>& fileBuffer, double sampleRate, int numChannels);
    ...
    public:
        ...
        // ─── Loop de Corrección ──────────────────────────────────────────────
        void storeRecommendation(int slotIndex,
                                 const juce::String& trackName,
        ...
        static constexpr int64_t kReferenceReanalysisIntervalUs =
            30 * 1000 * 1000; // 30s entre re-análisis de referencia
        ...
        {
            return slotIndex >= 0 && slotIndex < SlotRegistry::kMaxSlots && signalOrderApplied_[slotIndex]
                   && !signalOrderConfirmed_[slotIndex];
        }

        /** Marca una inferencia por signal-order como confirmada por el usuario. */
        void confirmSignalOrderRole(int slotIndex) noexcept
        {
            if (slotIndex >= 0 && slotIndex < SlotRegistry::kMaxSlots) signalOrderConfirmed_[slotIndex] = true;
        }
        ...
        {
            return slotIndex >= 0 && slotIndex < SlotRegistry::kMaxSlots && trackRoleWasInferred_[slotIndex]
        ...
        {
            if (slotIndex >= 0 && slotIndex < SlotRegistry::kMaxSlots) trackRoles_[slotIndex] = role;
        }
        ...
        {
            if (slotIndex >= 0 && slotIndex < SlotRegistry::kMaxSlots) return trackRoles_[slotIndex];
            return TrackRole::Unknown;
        }

        /** Obtiene el array completo de roles (para SemanticComparator). */
        [[nodiscard]] const std::array<TrackRole, SlotRegistry::kMaxSlots>& getTrackRoles() const noexcept
        {
            return trackRoles_;
        }
        ...
        void setReferenceDrivenMode(bool enabled) noexcept
        {
            ...
            if (!enabled) referenceDrivenWelcomeSent_ = false; // reset para posible re-activación
        }
        ...
        [[nodiscard]] const ReferenceProgress& getReferenceProgress() const noexcept { return referenceProgress_; }
        ...
        [[nodiscard]] bool shouldNormalizeReference() const noexcept { return coachMode_ == CoachMode::Mix; }
    ...
    public:
        ...
        struct TrackTonalAdvice
        {
            ...
            static constexpr const char* kExcessSuggestion(int r) noexcept
            {
                switch (r) {
                    case 0:
                        return "reduce 50-100 Hz";
                    case 1:
                        return "reduce 100-300 Hz";
                    case 2:
                        return "reduce 300-1000 Hz";
                    case 3:
                        return "reduce 1-3 kHz";
                    case 4:
                        return "reduce 3-8 kHz";
                    case 5:
                        return "reduce 8-16 kHz";
                    default:
                        return "";
                }
            }
            ...
            static constexpr const char* kDeficitSuggestion(int r) noexcept
            {
                switch (r) {
                    case 0:
                        return "refuerza 50-100 Hz";
                    case 1:
                        return "refuerza 100-300 Hz";
                    case 2:
                        return "refuerza 300-1000 Hz";
                    case 3:
                        return "refuerza 1-3 kHz";
                    case 4:
                        return "refuerza 3-8 kHz";
                    case 5:
                        return "refuerza 8-16 kHz";
                    default:
                        return "";
                }
            }
        ...
        // ═══════════════════════════════════════════════════════════════════════════
        //  TrackAdvice — Advice consolidado por pista (Sprint 7)
        //  Unifica Gain + Dynamics + Tonal + Phase en un solo struct
        //  con un status único para el UI (health dot 🟢🟡🔴).
        // ═══════════════════════════════════════════════════════════════════════════
        struct TrackAdvice
        {
            int slotIndex = -1;
            ...
            // Los 4 advices individuales
            TrackGainAdvice gain;
            TrackDynamicsAdvice dynamics;
            TrackTonalAdvice tonal;
            TrackPhaseAdvice phase;

            // Status consolidado — el PEOR status entre los 4 dominios
            enum class Status : uint8_t
            {
                OnTarget,   // Todos los dominios en rango
                NearTarget, // Algún dominio cerca del límite
                OffTarget,  // Uno o más dominios fuera de rango
                NoSignal,   // Sin señal detectable
                UnknownRole // Rol no especificado
            };
            Status status = Status::UnknownRole;

            // Severidad consolidada (0.0 = perfecto, 1.0 = crítico)
            float consolidatedSeverity = 0.0f;

            // Dominio con peor status
            enum class Domain : uint8_t
            {
                None,
                Gain,
            ...
            Domain worstDomain = Domain::None;
            juce::String worstMessage; // Mensaje del dominio peor
            ...
            [[nodiscard]] bool isOnTarget() const noexcept { return status == Status::OnTarget; }

            /** Computa el status consolidado y worstDomain a partir de los 4 advices.
                Se llama después de poblar gain, dynamics, tonal y phase. */
            void computeConsolidated() noexcept;
        };
    ...
    public:
        ...
        // ═════════════════════
        /** Analiza TODAS las pistas y consolida Gain + Dynamics + Tonal + Phase
            en TrackAdvice por pista. Almacena los resultados internamente.
            Llama a analyzeTrackGain/Dynamics/Tonal/Phase por cada pista activa
            y computa el status consolidado + worstDomain. */
        void analyzeAllTracks();

        /** Retorna el TrackAdvice consolidado para una pista (último analyzeAllTracks).
            Retorna TrackAdvice vacío si el slot no tiene datos. */
        [[nodiscard]] TrackAdvice getTrackAdvice(int slotIndex) const noexcept;

    private:
        // ═══ Sprint 7: TrackAdvice cache (último analyzeAllTracks) ═══
        std::array<TrackAdvice, SlotRegistry::kMaxSlots> trackAdvices_;
