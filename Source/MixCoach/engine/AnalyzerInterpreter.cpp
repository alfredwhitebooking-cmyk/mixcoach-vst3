...
namespace mixcoach {
    ...
    juce::String AnalyzerInterpretation::toFullMessage() const
    {
        ...
        msg += "**" + interpretation + "**";
        if (reading.isNotEmpty()) msg += " (" + reading + ")";
        msg += "\n";

        if (consequence.isNotEmpty()) msg += "    → " + consequence + "\n";

        if (action.isNotEmpty()) msg += "    \xF0\x9F\x92\xA1 " + action;
    ...
    BandDiagnostic AnalyzerInterpretation::toBandDiagnostic() const
    {
        ...
        if (reading.isNotEmpty()) diag.description += " (" + reading + ")";
        if (consequence.isNotEmpty()) diag.description += ": " + consequence;
        if (action.isNotEmpty()) diag.description += " | \xF0\x9F\x92\xA1 " + action;
    ...
    AnalyzerInterpretation AnalyzerInterpreter::interpretCorrelation(float correlation, const juce::String& genre)
    {
        ...
        if (g.isNotEmpty()) {
            ...
            if (correlation < minCorr && correlation >= 0.0f) {
                interp.action +=
                    " (para " + genre + " se recomienda correlaci\xC3\xB3n > " + juce::String(minCorr, 1) + ")";
            }
        }
    ...
    AnalyzerInterpretation AnalyzerInterpreter::interpretCrest(float crestDb, const juce::String& genre)
    {
        AnalyzerInterpretation interp;
        ...
        if (crestDb < 6.0f && genre.isNotEmpty()) {
            interp.consequence += ". Rango esperado para " + genre + ": " + getCrestRangeString(genre);
        }
    ...
    AnalyzerInterpretation AnalyzerInterpreter::interpretCentroid(float ratio, const juce::String& genre)
    {
        AnalyzerInterpretation interp;
        interp.domain = AnalyzerInterpretation::Domain::Centroid;
        ratio         = juce::jlimit(0.4f, 2.5f, ratio);

        interp.reading        = "Centroid: " + juce::String((ratio - 1.0f) * 100.0f, 0) + "% vs referencia";
    ...
    AnalyzerInterpretation
    AnalyzerInterpreter::interpretLUFS(float integratedLUFS, float truePeakDBTP, float lra, const juce::String& genre)
    {
        AnalyzerInterpretation interp;
        ...
        interp.reading = "LUFS: " + juce::String(integratedLUFS, 1) + " (target " + juce::String(target, 1)
                         + ") | TP: " + juce::String(truePeakDBTP, 1) + " dBTP | LRA: " + juce::String(lra, 1) + " LU";
        ...
        if (diff > 2.0f) {
            ...
            interp.consequence =
                "Diferencia de " + juce::String(std::abs(diff), 1) + " LU respecto al target de " + genre;
    ...
    AnalyzerInterpretation AnalyzerInterpreter::interpretPhaseScope(float correlation, float stereoWidth)
    {
        AnalyzerInterpretation interp;
        interp.domain = AnalyzerInterpretation::Domain::PhaseScope;

        interp.reading = "Corr: " + juce::String(correlation, 2) + " | Width: " + juce::String(stereoWidth, 2);
    ...
    AnalyzerInterpretation AnalyzerInterpreter::interpretSpectralBand(float bandEnergyDb,
                                                                      int bandIndex,
                                                                      ...
                                                                      float referenceDb)
    {
        ...
        interp.reading = bandName + " (" + juce::String((int)kBandFreqs[idx][0]) + "-"
                         + juce::String((int)kBandFreqs[idx][1]) + " Hz): " + juce::String(bandEnergyDb, 1) + " dBFS";
    ...
    AnalyzerInterpretation
    AnalyzerInterpreter::interpretGain(float peakLeft, float peakRight, float rmsLeft, float rmsRight)
    {
        AnalyzerInterpretation interp;
        ...
        float crestDb      = (rmsCombined > -90.0f && peakCombined > -90.0f) ? peakCombined - rmsCombined : 0.0f;
        float lrDiff       = std::abs(peakLeft - peakRight);

        interp.reading = "Peak L: " + juce::String(peakLeft, 1) + " dB | R: " + juce::String(peakRight, 1)
                         + " dB | RMS L: " + juce::String(rmsLeft, 1) + " dB | R: " + juce::String(rmsRight, 1) + " dB";
        ...
        if (lrDiff > 10.0f && peakLeft > -60.0f && peakRight > -60.0f) {
            ...
            interp.consequence    = "La imagen estéreo está inclinada " + juce::String(lrDiff, 1) + " dB hacia "
                                    + (peakLeft > peakRight ? "el canal izquierdo" : "el canal derecho");
        ...
        if (peakCombined > -3.0f) {
            ...
            interp.severity       = 0.6f;
            interp.consequence = "Picos a " + juce::String(peakCombined, 1) + " dB — muy cerca de 0 dBFS, sin headroom";
        ...
        if (crestDb > 0.0f && rmsCombined > -50.0f) {
            if (crestDb < 4.0f) {
                ...
                interp.severity       = 0.6f;
                interp.consequence =
                    "Crest factor de " + juce::String(crestDb, 1) + " dB — la pista suena aplastada, sin transientes";
        ...
        if (lrDiff > 6.0f && peakLeft > -60.0f && peakRight > -60.0f) {
            ...
            interp.severity       = 0.3f;
            interp.consequence =
                "Diferencia de " + juce::String(lrDiff, 1) + " dB entre canales — perceptible en auriculares";
        ...
        if (peakCombined >= -12.0f && peakCombined <= -3.0f && lrDiff < 3.0f) {
            ...
            interp.isPraise       = true;
            interp.consequence =
                "Peak de " + juce::String(peakCombined, 1) + " dB con buen balance L/R — nivel ideal para mezcla";
            interp.action     = "—";
            interp.lowFreqHz  = 20.0f;
            interp.highFreqHz = 20000.0f;
            return interp;
    ...
    AnalyzerInterpretation AnalyzerInterpreter::interpretStereoWidth(const float stereoWidthPerBand[6],
                                                                     float avgStereoWidth)
    {
        ...
        if (subTooWide || bassTooWide) {
            ...
            interp.severity       = 0.7f;
            interp.isCritical     = true;
            interp.consequence = "La banda " + juce::String(bandName) + " tiene ancho estéreo ("
                                 + juce::String(subTooWide ? stereoWidthPerBand[0] : stereoWidthPerBand[1], 2)
                                 + "). Los graves deben ser mono para evitar cancelación de fase y pérdida de pegada";
            interp.action      = "Aplica un bass mono (procesado M/S o utility) hasta " + juce::String((int)freq)
                                 + " Hz. El bajo y el kick deben estar centrados";
        ...
        if (allNarrow && avgStereoWidth < 0.15f) {
            ...
            interp.action =
                "Agrega ancho estéreo en HiMid/Pres/Air usando Stereo Shaper, "
        ...
        if (allWide && avgStereoWidth > 0.6f) {
            ...
            interp.action =
                "Reduce el ancho de elementos secundarios (reverbs, pads) y "
        ...
        if ((hiMidNarrow || presNarrow || airNarrow) && avgStereoWidth < 0.3f) {
            ...
            interp.consequence =
                "Las bandas " + narrowBands + " tienen ancho estéreo muy bajo — la mezcla suena cerrada en agudos";
            interp.action =
                "Agrega ancho estéreo en高频 usando delays cortos (<15ms) o "
    ...
    AnalyzerInterpretation AnalyzerInterpreter::interpretMidSide(const float midEnergyPerBand[6],
                                                                 const float sideEnergyPerBand[6],
                                                                 float avgStereoWidth)
    {
        AnalyzerInterpretation interp;
        ...
        for (int b = 0; b < 6; ++b) {
            if (bands[b].valid) reading += " " + juce::String(kBand6Names[b]) + "=" + juce::String(bands[b].ratio, 2);
            else
        ...
        if (maxRatioBand >= 0 && maxRatioBand <= 1 && maxRatio > 0.5f) {
            ...
            interp.consequence    = "La banda " + juce::String(kBand6Names[maxRatioBand])
                                    + " tiene más energía Side que Mid (ratio "
                                    + juce::String(bands[maxRatioBand].ratio, 2)
                                    + "). Esto causa pérdida de pegada y problemas de fase en mono";
            interp.action =
                "Aplica procesado M/S para centrar los graves: pasa el contenido "
                "de "
                + juce::String((int)kBand6Freqs[maxRatioBand][0]) + "-"
        ...
        if (wideBands >= 4 && maxRatio > 0.8f) {
            ...
            interp.severity       = 0.5f;
            interp.consequence = juce::String(wideBands) + " de 6 bandas tienen side significativo (>0.5 ratio). "
                                 + "La banda " + juce::String(kBand6Names[maxRatioBand]) + " es la más extrema (ratio "
                                 + juce::String(bands[maxRatioBand].ratio, 2) + ")";
        ...
        if (narrowBands >= 4 && avgStereoWidth < 0.2f) {
            ...
            interp.severity       = 0.3f;
            interp.consequence    = juce::String(narrowBands) + " de 6 bandas tienen poco contenido Side (<0.1 ratio). "
        ...
        // ═══ Caso 5: Balance M/S saludable ═════════════════════════════════════
        {
            int healthyBands = 0;
            for (int b = 0; b < 6; ++b) {
                if (bands[b].valid && bands[b].ratio >= 0.1f && bands[b].ratio <= 0.5f) healthyBands++;
            }
            if (healthyBands >= 4) {
                interp.interpretation = "BALANCE MID/SIDE SALUDABLE";
                interp.severity       = 0.1f;
                interp.isPraise       = true;
                interp.consequence    = juce::String(healthyBands) + " de 6 bandas tienen balance M/S saludable. "
    ...
    std::vector<AnalyzerInterpretation> AnalyzerInterpreter::interpretAll(float correlation,
                                                                          float crestDb,
                                                                          float centroidRatio,
    ...
    std::vector<BandDiagnostic>
    AnalyzerInterpreter::toBandDiagnostics(const std::vector<AnalyzerInterpretation>& interpretations)
    {
        ...
        for (const auto& interp : interpretations) {
            ...
            if (interp.interpretation.isNotEmpty()) diagnostics.push_back(interp.toBandDiagnostic());
        }

        return diagnostics;
    }

    juce::String AnalyzerInterpreter::formatCoachMessage(const std::vector<AnalyzerInterpretation>& interpretations,
                                                         int maxEntries)
    {
        ...
        for (const auto& interp : interpretations) sorted.push_back(std::cref(interp));

        std::sort(sorted.begin(), sorted.end(), [](const AnalyzerInterpretation& a, const AnalyzerInterpretation& b) {
        ...
        for (const auto& ref : sorted) {
            ...
            if (count > 0 && interp.isPraise) continue;
        ...
        if (sorted.size() > maxEntries) {
            msg += "... y " + juce::String((int)sorted.size() - maxEntries) + " interpretaciones adicionales.\n";
        }
