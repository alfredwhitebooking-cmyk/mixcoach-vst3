...
namespace mixcoach {
    ...
    void CoachEngine::startSetupDialogue()
    {
        ...
        if (engineerName_.isNotEmpty()) {
            LogHelper::writeToLog("[CoachEngineSetup] Usuario recurrente: " + engineerName_ + " — saltando onboarding");
    ...
    void CoachEngine::sendFallbackWelcome()
    {
        juce::String msg;
        msg +=
            "\xF0\x9F\x8E\xA7 **\xC2\xA1"
            "Hola! Bienvenido a MixCoach.**\n\n";
        msg +=
            "Antes de empezar, \xC2\xBF"
            "C\xC3\xB3mo te llamas o tienes nombre art\xC3\xADstico?\n";
    ...
    void CoachEngine::detectAndSetEngineerName(const juce::String& message)
    {
        ...
        if (lower.isEmpty()) {
            respondWithPremium(
                "\xC2\xBFPodr\xC3\xAD"
                "as decirme tu nombre o nombre art\xC3\xADstico? "
        ...
        if (cleanName.startsWithIgnoreCase("soy ")) cleanName = cleanName.substring(4).trim();
        ...
        if (cleanName.isEmpty() || cleanName.length() > 40) {
            respondWithPremium(
                "\xC2\xBFPodr\xC3\xAD"
                "as repetirlo? Un nombre corto est\xC3\xA1 bien.",
                MentorMessage::Type::Question);
            return;
        }
        ...
        if (engineerNameCallback_) engineerNameCallback_(cleanName);
    ...
    void CoachEngine::detectAndSetMode(const juce::String& message)
    {
        ...
        bool isMix = lower.contains("mix") || lower.contains("mezcla") || lower.contains("mezclar")
                     || lower.contains("produc") || lower.contains("track") || lower.contains("pista")
                     || lower.contains("instrumento") || lower.contains("beats") || lower.contains("cancion")
                     || lower.contains("canci");

        bool isMaster = lower.contains("master") || lower.contains("masterizar") || lower.contains("mastering")
                        || lower.contains("finalizar") || lower.contains("final") || lower.contains("loudness");

        // Default: if has active Messengers, assume Mix Mode
        if (!isMix && !isMaster) {
            int active = sharedData_.getSlotRegistry().activeCount();
            if (active > 0) isMix = true;
            else
    ...
    void CoachEngine::detectAndSetDestination(const juce::String& message)
    {
        ...
        if (lower.contains("spotify")) dest = MasterDestination::Spotify;
    ...
    void CoachEngine::detectAndSetGenre(const juce::String& message)
    {
        ...
        const juce::StringArray knownGenres = {
            "reggaeton", "pop",     "rock",   "hip-hop", "hip hop", "rap",      "trap",      "edm",      "electronic",
            "house",     "techno",  "trance", "dubstep", "jazz",    "blues",    "classical", "clasica",  "orchestra",
            "orquestal", "latin",   "latina", "bachata", "salsa",   "merengue", "rnb",       "r&b",      "soul",
            "funk",      "country", "folk",   "indie",   "metal",   "punk",     "hardcore",  "lo-fi",    "lofi",
            "ambient",   "drone",   "k-pop",  "kpop",    "j-pop",   "jpop",     "reggae",    "dancehall"};
        ...
        for (auto& g : knownGenres) {
            if (lower.contains(g)) {
                ...
                if (detectedGenre.isNotEmpty())
                    detectedGenre = detectedGenre.substring(0, 1).toUpperCase() + detectedGenre.substring(1);
                break;
            }
        }
        ...
        if (detectedGenre.isEmpty()) {
            ...
            if (detectedGenre.length() > 30) detectedGenre = detectedGenre.substring(0, 30);
        }
    ...
    juce::String CoachEngine::scanAndShowResults() const
    {
        ...
        if (active == 0) {
            result +=
                "  (Aun no hay pistas activas. Los Messengers apareceran "
        ...
        registry.forEachActive([&](const SlotInfo& info) {
            ...
            if (busName.isNotEmpty()) result += "  [" + getBusIcon(info.bus) + " " + busName + "]";
            result += "\n";
        });
    ...
    void CoachEngine::advanceFromSetup()
    {
        ...
        LogHelper::writeToLog("[CoachEngine] Setup completado. Modo: " + juce::String(isMixMode() ? "Mix" : "Master")
                              + ". Genero: " + setupGenre_ + ". Avanzando a fase "
    ...
    {
        if (setupGenre_.isEmpty()) setupGenre_ = "No especificado";
        if (coachMode_ != CoachMode::Mix && coachMode_ != CoachMode::Master) coachMode_ = CoachMode::Mix;
        advanceFromSetup();
    }
    ...
    void CoachEngine::sendPhaseGuidance(MentorPhase phase)
    {
        ...
        const char* phaseLabel = isMasterMode() ? masterPhaseNames[static_cast<int>(phase)]
        ...
        if (isMasterMode()) {
            switch (phase) {
                ...
                case MentorPhase::Espacio:
                    msg +=
                        "\xF0\x9F\x94\xAE **Imagen Est\xC3\xA9reo**\n\n"
                        "Voy a sugerir:\n"
                ...
                case MentorPhase::MasterCheck: {
                    ...
                    if (!hasRef) {
                        msg +=
                            "Carga una referencia a volumen real para comparar:\n"
                ...
                default:
                    msg +=
                        "\xF0\x9F\x92\xA1 Trabajando en el master. "
            ...
            switch (phase) {
                ...
                case MentorPhase::GainStaging:
                    msg +=
                        ...
                        "\xF0\x9F\x92\xA1 Solo niveles — nada de EQ ni compresi\xC3\xB3n a\xC3\xBA"
                        "n.";
                    break;
                ...
                case MentorPhase::Balance:
                    msg +=
                        ...
                        "\xF0\x9F\x92\xA1 Solo faders y pan — sin EQ ni compresi\xC3\xB3n todav\xC3\xAD"
                        "a.";
                    break;
                ...
                case MentorPhase::Espacio:
                    msg +=
                        "\xF0\x9F\x94\xAE **Espacio y Profundidad**\n\n"
                ...
                default:
                    msg +=
                        "\xF0\x9F\x92\xA1 Puedes preguntarme cualquier cosa sobre la mezcla "
    ...
    {
        if (referenceMetadata_.valid()) return referenceMetadata_.name;
        return {};
    }
    ...
    std::array<BusGroupSummary, kNumBuses + 1> CoachEngine::computeBusSummaries() const
    {
        ...
        registry.forEachActive([&](const SlotInfo& info) {
            ...
            if (busIdx < 0 || busIdx > kNumBuses) busIdx = 0;
            ...
            if (rms > summary.rmsSum || summary.rmsSum < -90.0f) summary.rmsSum = rms;
    ...
    std::vector<DomainGap> CoachEngine::getReferenceGaps() const
    {
        ...
        return ReferenceDrivenEngine::computeGaps(referenceFingerprint_, audioAnalyzer_, setupGenre_);
    }
    ...
    float CoachEngine::spectrumBandEnergy(const float* spectrum, int startBin, int endBin) const noexcept
    {
        ...
        for (int i = startBin; i < n; ++i) sum += spectrum[i];
