...
{
    if (spectrum == nullptr || startBin < 0 || endBin <= startBin || startBin >= kNumSpectrumBins)
        return -100.0f;
...
static float spectralCosineSimilarity(const float a[30], const float b[30])
{
    ...
    if (active < 3 || normA < 1e-10 || normB < 1e-10)
        return 0.0f;
...
// ═══════════════════════════════════════════════════════════════════════════
void CoachEngine::analyzeReferenceBuffer(const float* bufferL, const float* bufferR,
                                          ...
                                          const juce::String& filePath)
{
    ...
    LogHelper::writeToLog("[CoachEngine] Analizando referencia: \""
                          + juce::File(filePath).getFileName()
                          + "\" (" + juce::String(static_cast<int>(numChannels))
                          + "ch, " + juce::String(static_cast<int>(sampleRate))
                          + "Hz, " + juce::String(static_cast<double>(numSamples) / sampleRate, 1) + "s)");
    ...
    if (shouldNormalizeReference())
    {
        ...
        if (peak > 1e-10f)
        {
            ...
            LogHelper::writeToLog("[CoachEngine] Referencia normalizada a -6 dBFS "
                                  "(peak=" + juce::String(juce::Decibels::gainToDecibels(peak), 1)
                                  + " dBFS, scale=" + juce::String(scale, 3) + ")");
        }
        else
        {
    ...
    for (int pos = 0; pos < totalSamples; pos += kBlockSize)
    {
        ...
        if (numChannels >= 2 && procR != nullptr)
        {
            for (int i = 0; i < thisBlock; ++i)
                monoData[i] = (procL[pos + i] + procR[pos + i]) * 0.5f;
        }
        else
        {
        ...
        if (numChannels >= 2 && procR != nullptr)
            loudness.processBlock(procL + pos, procR + pos, thisBlock);
        else
        ...
        double blockRms = 0.0;
        for (int i = 0; i < thisBlock; ++i)
            blockRms += static_cast<double>(monoData[i] * monoData[i]);
    ...
    if (rmsCount > 0)
    {
        ...
        if (avgRms > -80.0f && peakDb > -80.0f)
            fp.crestFactor = peakDb - avgRms;
    }
    ...
    if (numChannels >= 2 && procR != nullptr)
    {
        ...
        if (corrCount > 0)
            fp.correlation = static_cast<float>(corrSum / corrCount);
    }
    ...
    // (se usa como expected centroid cuando hay referencia cargada)
    {
        ...
        for (int b = 0; b < 30; ++b)
        {
            ...
            if (energyDb > -80.0f)
            {
                ...
                if (lowFreq < 1.0f)
                    centerFreq = 30.0f; // Banda 0: floor en ~30 Hz
        ...
        if (totalEnergy > 0.0)
            fp.spectralCentroidHz = static_cast<float>(weightedSum / totalEnergy);

        LogHelper::writeToLog("[CoachEngine] Referencia centroide espectral: "
                              + juce::String(fp.spectralCentroidHz, 0) + " Hz");
    }
    ...
    if (numChannels >= 2 && procR != nullptr)
        fullBuffer.copyFrom(1, 0, procR, totalSamples);
    ...
    LogHelper::writeToLog("[CoachEngine] Referencia analizada: "
                          + juce::String(static_cast<int>(sampleRate)) + "Hz, "
                          + juce::String(numSamples) + "samples, "
                          + "LUFS_I=" + juce::String(fp.lufsIntegrated, 1) + ", "
                          + "Crest=" + juce::String(fp.crestFactor, 1) + "dB, "
                          + "Corr=" + juce::String(fp.correlation, 2));
}
...
// ═══════════════════════════════════════════════════════════════════════════
void CoachEngine::applyReferenceAnalysis(const float* bufferL, const float* bufferR,
                                          int64_t numSamples, int numChannels,
                                          double sampleRate,
                                          const juce::String& filePath)
{
        ...
        if (spectrum != nullptr && master.getLastUpdateTime() > 0)
        {
            for (int r = 0; r < 6; ++r)
            {
                static const int kRegionBands[6][2] = {
                    {0, 2}, {2, 5}, {5, 10}, {10, 18}, {18, 25}, {25, 30}
                };
                float sum = 0.0f;
                int count = 0;
            ...
            dp.mixCrestFactor = audioAnalyzer_.getMasterAnalysis().getPeak()
                               - audioAnalyzer_.getMasterAnalysis().getRMS();
                                            ...
                                            int numChannels)
{
    ...
    for (int w = 0; w < numWindows; ++w)
    {
        ...
        for (int i = 0; i < thisSize; ++i)
        {
            ...
            if (numChannels >= 2)
                s = (s + fileBuffer.getSample(1, startSample + i)) * 0.5f;
        ...
        double rms = 0.0;
        for (int i = 0; i < thisSize; ++i)
            rms += static_cast<double>(monoBuf[static_cast<size_t>(i)]
                                       * monoBuf[static_cast<size_t>(i)]);
    ...
    {
        float sim = spectralCosineSimilarity(windows[i - 1].bandEnergies,
                                              windows[i].bandEnergies);
    ...
    // Filtrar boundaries muy cercanos (< 4 ventanas = ~4s)
    {
        ...
        {
            if (boundaryIndices[i] - filtered.back() >= 4)
                filtered.push_back(boundaryIndices[i]);
        }
    ...
    for (size_t b = 0; b < boundaryIndices.size() - 1; ++b)
    {
        ...
        for (int band = 0; band < 30; ++band)
        {
            if (bandCounts[band] > 0)
                sec.avgBandEnergies[band] /= bandCounts[band];
            else
    ...
    for (size_t i = 0; i < sections.size(); ++i)
    {
        ...
        for (size_t j = i + 1; j < sections.size(); ++j)
        {
            ...
            float sim = spectralCosineSimilarity(sections[i].avgBandEnergies,
                                                  sections[j].avgBandEnergies);
            ...
            if (energyDiff < 3.0f)
                adjustedSim += 0.05f; // Pequeño bonus por energía similar
        ...
        for (int b = 0; b < 30; ++b)
            if (bandCounts[b] > 0)
                cluster.avgBandEnergies[b] /= bandCounts[b];
    ...
    std::sort(clusters.begin(), clusters.end(),
              [](const SectionCluster& a, const SectionCluster& b) {
    ...
    for (auto& cluster : clusters)
    {
        ...
        std::sort(cluster.memberSectionIndices.begin(),
                  cluster.memberSectionIndices.end());
        ...
        if (numMembers >= 2 && !cluster.labeled)
        {
            ...
            if (cluster.avgEnergy > -20.0f)
                cluster.label = "Chorus";
            else
                cluster.label = "Verse";
    ...
    for (auto& cluster : clusters)
    {
        if (!cluster.labeled && cluster.memberSectionIndices.size() >= 1)
        {
            int firstMember = cluster.memberSectionIndices.front();
            bool isFirstSection = (firstMember == 0);
            bool isLastSection  = (cluster.memberSectionIndices.back()
                                   == static_cast<int>(sections.size()) - 1);

            if (isFirstSection && cluster.avgEnergy < -25.0f)
                cluster.label = "Intro";
    ...
    for (const auto& cluster : clusters)
    {
        for (int secIdx : cluster.memberSectionIndices)
        {
            ...
            for (int b = 0; b < 30; ++b)
                fp.bandEnergies[b] = sec.avgBandEnergies[b];
    ...
    std::sort(referenceSections_.begin(), referenceSections_.end(),
    ...
    // ═══ LOG ─────────────────────────────────────────────────────────────
    {
        ...
        for (size_t i = 0; i < referenceSections_.size(); ++i)
        {
            ...
            logMsg += "  [" + juce::String(static_cast<int>(i)) + "] "
                      + s.label + " "
                      + juce::String(s.startSeconds, 1) + "s-"
                      + juce::String(s.endSeconds, 1) + "s\n";
        }
        ...
        for (const auto& s : referenceSections_)
        {
            ...
            chatMsg += "  \xE2\x97\x8F **" + s.label + "** "
                       + juce::String(s.startSeconds, 1) + "s - "
                       + juce::String(s.endSeconds, 1) + "s ("
                       + juce::String(s.endSeconds - s.startSeconds, 1) + "s)\n";
        }
