...
{
    if (currentPhase_ >= MentorPhase::MasterCheck)
...
bool PhaseManager::isPhaseComplete(MentorPhase phase) const
{
    switch (phase) {
        case MentorPhase::Organizacion:
        {
            ...
            if (!orgMetrics_.hasData)
                return registry_.activeCount() > 0; // Fallback si no hay métricas

            return orgMetrics_.identifiedTracks > 0
                && orgMetrics_.bussedTracks >= orgMetrics_.totalTracks
        ...
        case MentorPhase::GainStaging:
        {
            ...
            if (!gainMetrics_.hasData)
                return false;

            if (gainMetrics_.clippingCount > 0)
                return false;

            if (gainMetrics_.maxGlobalPeak < -18.0f)
                return false;

            if (gainMetrics_.maxGlobalPeak > -3.0f)
                return false;
        ...
        case MentorPhase::Balance:
        {
            ...
            if (!balanceMetrics_.hasData)
                return false;

            if (balanceMetrics_.totalPairs < 1)
                return false;

            float ratio = (float)balanceMetrics_.unbalancedPairs
                          / (float)std::max(balanceMetrics_.totalPairs, 1);
        ...
        case MentorPhase::Compresion:
        {
            ...
            if (!dynamicsMetrics_.hasData)
                return false;

            return dynamicsMetrics_.avgCrestFactor >= 6.0f
                   && dynamicsMetrics_.avgCrestFactor <= 14.0f;
        }

        case MentorPhase::Espacio:
        {
            // Complete when: spatial FX applied, correlation healthy
            if (!espacioMetrics_.hasData)
                return false;

            return espacioMetrics_.avgCorrelation > 0.3f
                   && espacioMetrics_.avgCorrelation < 0.8f;
        }
...
float PhaseManager::getPhaseProgress(MentorPhase phase) const
{
    switch (phase) {
        ...
        case MentorPhase::GainStaging:
        {
            if (!gainMetrics_.hasData || gainMetrics_.activeCount < 1)
                return 0.0f;

            float clipScore     = (gainMetrics_.clippingCount == 0) ? 0.4f : 0.0f;
            float headroomScore = (gainMetrics_.maxGlobalPeak > -18.0f
                                   && gainMetrics_.maxGlobalPeak < -3.0f) ? 0.3f : 0.0f;
        ...
        case MentorPhase::Balance:
        {
            if (!balanceMetrics_.hasData || balanceMetrics_.totalPairs < 1)
                return 0.0f;
        ...
        case MentorPhase::Compresion:
        {
            ...
            if (dynamicsMetrics_.healthyDynamics == 0)
                return 0.0f;

            float healthyRatio = (float)dynamicsMetrics_.healthyDynamics
                                 / (float)std::max(dynamicsMetrics_.healthyDynamics
                                                   + dynamicsMetrics_.overcompressed
                                                   + dynamicsMetrics_.undercompressed, 1);
        ...
        case MentorPhase::Espacio:
        {
            ...
            if (espacioMetrics_.avgCorrelation > 0.3f
                && espacioMetrics_.avgCorrelation < 0.8f) score += 0.2f;
        ...
        case MentorPhase::MasterCheck:
        {
            if (!masterCheckMetrics_.hasData)
                return 0.0f;
            ...
            if (masterCheckMetrics_.referenceLoaded)
            {
                ...
                if (masterCheckMetrics_.gapsResolved > 0 && masterCheckMetrics_.totalGaps > 0)
                    score += (float)masterCheckMetrics_.gapsResolved
                             / (float)masterCheckMetrics_.totalGaps * 0.2f;
            }
...
{
    switch (phase) {
        case MentorPhase::Organizacion:  return 0;
        case MentorPhase::GainStaging:   return 1;
        case MentorPhase::Balance:       return 3;
        case MentorPhase::EQ:            return 3;
        case MentorPhase::Compresion:    return 5;
        case MentorPhase::Espacio:       return 3;
        case MentorPhase::MasterCheck:   return 3;
        default:                         return 0;
    }
}
...
const char* PhaseManager::phaseDescription(MentorPhase phase)
{
    ...
    switch (phase) {
        ...
        case MentorPhase::Balance:
            return "Balancea faders y paneo: niveles relativos entre instrumentos, sin procesar a\xC3\xBA" "n";
        case MentorPhase::EQ:
            return "Corrige balance tonal: EQ por pista y bus, carving espectral, elimina enmascaramiento";
        case MentorPhase::Compresion:
            return "Controla din\xC3\xA1" "mica: compresores, saturadores, crest factor saludable";
        case MentorPhase::Espacio:
            return "Crea profundidad: reverb, delay, ancho est\xC3\xA9" "reo, automatizaci\xC3\xB3" "n, panoramas";
        case MentorPhase::MasterCheck:
            return "Verificaci\xC3\xB3" "n final: referencia, LUFS target, compatibilidad mono, veredicto";
        default:
            return "";
    }
}
...
const char* PhaseManager::getPhaseDescription(MentorPhase phase) const noexcept
{
    ...
    if (isMasterMode())
    {
        switch (phase) {
            case MentorPhase::Organizacion:
                return "Configura el destino de masterizaci\xC3\xB3" "n y analiza el master bus";
            case MentorPhase::GainStaging:
                return "Mide niveles: LUFS moment\xC3\xA1" "neo, True Peak, crest factor del master";
            case MentorPhase::Balance:
                return "Eval\xC3\xBA" "a balance espectral global: sub, graves, medios, agudos";
            case MentorPhase::EQ:
                return "Aplica EQ sutiles al master para corregir balance tonal";
            case MentorPhase::Compresion:
                return "Compresi\xC3\xB3" "n suave, saturaci\xC3\xB3" "n y limitaci\xC3\xB3" "n para alcanzar target LUFS";
            case MentorPhase::Espacio:
                return "Ensancha est\xC3\xA9" "reo, correlaci\xC3\xB3" "n, profundidad y automatizaci\xC3\xB3" "n final";
            case MentorPhase::MasterCheck:
...
{
    if (isMasterMode())
        return masterPhaseNames[static_cast<int>(phase)];
