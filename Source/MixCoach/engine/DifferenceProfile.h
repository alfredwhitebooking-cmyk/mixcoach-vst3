...
namespace mixcoach {
    ...
    struct DifferenceProfile
    {
        ...
        [[nodiscard]] bool hasIssues() const noexcept { return criticalGaps > 0 || warningGaps > 0; }
        ...
        [[nodiscard]] const DomainGap* getMostSevereGap() const noexcept
        {
            ...
            for (const auto& g : domainGaps) {
                if (static_cast<int>(g.severity) < static_cast<int>(worst->severity)) worst = &g;
            }
            return worst;
        }
        ...
        static DifferenceProfile build(const ReferenceFingerprint& fp,
    ...
    private:
        ...
        static void computeRegionEnergies(const float bandEnergies[30], float regionEnergies[6]) noexcept;

        /** Computa el delta score agregado (0.0-1.0) desde los deltas individuales. */
        static float computeDeltaScore(const float regionDeltas[6], float deltaLUFS, float deltaCrest) noexcept;
    };
