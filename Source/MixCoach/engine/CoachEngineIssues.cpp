...
namespace mixcoach {
    ...
    std::vector<CoachEngine::TrackIssue> CoachEngine::collectAllIssues()
    {
        ...
        if (registry.activeCount() == 0) return issues;
        ...
        registry.forEachActive([&](const SlotInfo& info) {
            ...
            if (issue.trackName.isEmpty()) issue.trackName = "Pista " + juce::String(info.slotIndex + 1);
            ...
            if (peak <= -0.5f && peak > nearClipThreshold) {
                ...
                issue.description =
                    issue.trackName + " esta cerca del clipping (" + juce::String(peak, 1) + " dB" + roleHint + ")";
                issue.actionVerb     = "reducir";
                issue.suggestedDelta = peak + 6.0f;
                generateOptionsForIssue(issue, setupGenre_);
                issues.push_back(issue);
            ...
            if (peak < lowSignalThreshold && registry.activeCount() > 1) {
                ...
                juce::String roleHint = hasRole ? (" para " + juce::String(expProfile.name)
                                                   + " (target: " + juce::String(expProfile.peakTargetDb, 1) + " dB)")
                                                : "";
                issue.description =
                    issue.trackName + " tiene senial muy baja (" + juce::String(peak, 1) + " dB" + roleHint + ")";
            ...
            if (telem.crestFactor > 0.0f && telem.rmsLeft > -40.0f) {
                ...
                if (telem.crestFactor < (crestTarget - crestTol)) {
                    ...
                    juce::String roleHint     = hasRole ? (" para " + juce::String(expProfile.name)
                                                           + " (target: " + juce::String(crestTarget, 1) + " dB)")
                                                        : "";
                    crestIssue.description    = issue.trackName + " tiene poca dinamica (crest: "
                                                + juce::String(telem.crestFactor, 1) + " dB" + roleHint + ")";
            ...
            // ─── Spectral detectors (band energies) ────────────────────────
            {
                ...
                for (int b = 0; b < 30; ++b) {
                    ...
                    if (b < 2) {
                        subAvg = (subAvg * subN + e) / (subN + 1);
                        ++subN;
                    }
                    else if (b < 5) {
                        bassAvg = (bassAvg * bassN + e) / (bassN + 1);
                        ++bassN;
                    }
                    else if (b < 10) {
                        loMidAvg = (loMidAvg * loMidN + e) / (loMidN + 1);
                        ++loMidN;
                    }
                    else if (b < 18) {
                        hiMidAvg = (hiMidAvg * hiMidN + e) / (hiMidN + 1);
                        ++hiMidN;
                    }
                    else if (b < 25) {
                        presAvg = (presAvg * presN + e) / (presN + 1);
                        ++presN;
                    }
                    else {
                        airAvg = (airAvg * airN + e) / (airN + 1);
                        ++airN;
                    }
                }
                ...
                if (hasSpectralData) {
                    ...
                    if (bassN + loMidN + hiMidN + presN > 0) {
                        float sumBody = bassAvg * bassN + loMidAvg * loMidN + hiMidAvg * hiMidN + presAvg * presN;
                    ...
                    if (subBassAvg > -80.0f && bodyAvg > -80.0f && subBassAvg > bodyAvg + 6.0f) {
                        ...
                        gravesIssue.description    = issue.trackName + " tiene exceso de graves (sub-bass "
                                                     + juce::String(subBassAvg, 1) + " dB vs body "
                                                     + juce::String(bodyAvg, 1) + " dB)";
                    ...
                    if (presEnergy > -80.0f && bodyAvg > -80.0f && presEnergy > bodyAvg + 6.0f) {
                        ...
                        presIssue.description    = issue.trackName + " tiene exceso de presencia ("
                                                   + juce::String(presEnergy, 1) + " dB vs body "
                                                   + juce::String(bodyAvg, 1) + " dB)";
                    ...
                    if (presAirAvg > -80.0f && bodyAvg > -80.0f && presAirAvg < bodyAvg - 6.0f) {
                        ...
                        faltaIssue.description    = issue.trackName + " falta de presencia/aire (body "
                                                    + juce::String(bodyAvg, 1) + " dB vs " + juce::String(presAirAvg, 1)
                                                    + " dB)";
            ...
            {
                float lowSignalThreshold       = hasRole ? (expProfile.peakTargetDb - expProfile.peakTolerance - 6.0f)
                                                         : -30.0f;
                ...
                bool hasLRImbalance   = (std::fabs(telem.peakLeft - telem.peakRight) > 6.0f && telem.rmsLeft > -40.0f);
                float crestTarget     = hasRole ? expProfile.crestTargetDb : 12.0f;
                float crestTol        = hasRole ? expProfile.crestTolerance : 6.0f;
                ...
                if (!isClipping && !isNearClip && !isLowSignal && !hasLRImbalance && !isOvercompressed
                    && !hasPhaseIssue) {
                    bool goodLevel = hasRole ? expProfile.isPeakInRange(peak) : (peak > -24.0f && peak < -6.0f);
    ...
    std::vector<int> CoachEngine::identifyOptimalTracks()
    {
        ...
        auto& registry = sharedData_.getSlotRegistry();
        if (registry.activeCount() == 0) return optimal;
        ...
        registry.forEachActive([&](const SlotInfo& info) {
            ...
            if (goodLevel && goodCrest && goodPhase) optimal.push_back(info.slotIndex);
        });
    ...
    void CoachEngine::generateOptionsForIssue(TrackIssue& issue, const juce::String& genre)
    {
        ...
        if (issue.domain == "gain") {
            ...
            issue.optionB =
                "Reducir ancho est\xC3\xA9"
                "reo o centrar panorama";
        }
