...
static int clampScore(int s) noexcept { return juce::jlimit(0, 100, s); }
static float absF(float v) noexcept { return v < 0.0f ? -v : v; }
    ...
    if (engine.hasReferenceAudio())
    {
        ...
        if (!gaps.empty())
        {
            ...
            for (const auto& g : gaps)
            {
                ...
                {
                    switch (g.severity) {
                        case GapSeverity::Critical: refPenalty += 20; break;
                        case GapSeverity::Warning:  refPenalty += 10; break;
                        case GapSeverity::Info:     refPenalty += 3;  break;
                        case GapSeverity::Praise:   refPraise += 5;   break;
                    }
                }
            }
    ...
    if (sharedData != nullptr)
    {
        ...
        registry.forEachActive([&](const SlotInfo& info) {
            ...
            if (result.peakLeft > -60.0f || result.peakRight > -60.0f)
            {
                ...
                for (int b = 1; b < 5; ++b) {
                    if (profile.bandLevelDb[b] > -60.0f && profile.bandLevelDb[b-1] > -60.0f && profile.bandLevelDb[b+1] > -60.0f) {
...
// ═══════════════════════════════════════════════════════════════════════════
static int computeSpatialScore(const CoachEngine& /*engine*/,
                               const AudioAnalyzer& analyzer,
                               MixScore& score)
{
    int spatialScore = 80;
    ...
    if (sharedData != nullptr)
    {
        ...
        registry.forEachActive([&](const SlotInfo& info) {
            ...
            if (result.peakLeft > -60.0f || result.peakRight > -60.0f)
            {
                ...
                float avgWidth = 0.0f;
                for (int b = 0; b < 6; ++b)
                    avgWidth += result.stereoWidthPerBand[b];
                avgWidth /= 6.0f;

                if (avgWidth > 0.7f)
                    wideTracks++;   // Extremely wide — potential phase issues
...
static int computeReferenceScore(const CoachEngine& engine)
{
    if (!engine.hasReference())
        return 0;  // Sin referencia → score 0 (no ponderado)
    ...
    if (engine.hasReferenceAudio())
    {
        ...
        {
            switch (g.severity) {
                case GapSeverity::Critical: score -= 15; break;
                case GapSeverity::Warning:  score -= 8;  break;
                case GapSeverity::Info:     score -= 3;  break;
                case GapSeverity::Praise:   score += 3;  break;
            }
        }
...
// ═══════════════════════════════════════════════════════════════════════════
MixScore MixScore::compute(const CoachEngine& engine,
                           const AudioAnalyzer& analyzer,
                           const juce::String& genre)
{
    MixScore s;
    ...
    auto* sharedData = SharedData::safeGetInstance();
    if (sharedData != nullptr)
        s.activeTrackCount = sharedData->getSlotRegistry().activeCount();
    ...
    if (hasRef)
    {
        s.overall = (s.gain * 25 + s.tonal * 25 + s.dynamics * 20
                     + s.spatial * 15 + s.reference * 15) / 100;
    }
    else
    {
        s.overall = (s.gain * 30 + s.tonal * 30 + s.dynamics * 20
                     + s.spatial * 20) / 100;
    }
    ...
    LogHelper::writeToLog("[MixScore] Score: " + juce::String(s.overall)
                          + " (G:" + juce::String(s.gain)
                          + " T:" + juce::String(s.tonal)
                          + " D:" + juce::String(s.dynamics)
                          + " S:" + juce::String(s.spatial)
                          + " R:" + juce::String(s.reference)
                          + ") tracks=" + juce::String(s.activeTrackCount)
                          + " clip=" + juce::String(s.clippingTrackCount)
                          + " - " + s.statusLabel);
...
juce::String MixScore::toTextSummary() const
{
    ...
    if (clippingTrackCount > 0)
        s += " (clipping: " + juce::String(clippingTrackCount) + " tracks)";
    s += "\n";
    ...
    if (reference > 0)
        s += "  * Reference: " + juce::String(reference) + "/100\n";
    else
...
juce::String MixScore::toDetailedString() const
{
    ...
    s += "Tracks: " + juce::String(activeTrackCount)
         + " | Clipping: " + juce::String(clippingTrackCount)
    ...
    if (clippingTrackCount > 0)
        s += " (" + juce::String(clippingTrackCount) + " tracks clipping!)";
    s += "\n";
    s += "  * Headroom:     " + juce::String(gainHeadroom) + "/100";
    if (masterPeakDb > -50.0f)
        s += " (master peak: " + juce::String(masterPeakDb, 1) + " dBFS)";
    s += "\n";
    s += "  * L/R Balance:  " + juce::String(gainLRBalance) + "/100\n";
    s += "  * Low Signal:   " + juce::String(gainLowSignal) + "/100";
    if (lowSignalTrackCount > 0)
        s += " (" + juce::String(lowSignalTrackCount) + " tracks with low signal)";
    s += "\n\n";
    ...
    if (reference > 0)
        s += "  * Ref Alignment: " + juce::String(tonalRefAlignment) + "/100\n";
    s += "\n";

    // ─── Dynamics ──────────────────────────────────────────────────────
    s += "[DYNAMICS " + juce::String(dynamics) + "/100]\n";
    s += "  * Loudness:    " + juce::String(dynLoudness) + "/100";
    if (masterIntegratedLUFS > -40.0f)
        s += " (LUFS: " + juce::String(masterIntegratedLUFS, 1) + ")";
    s += "\n";
    ...
    if (masterCorrelation > -2.0f)
        s += " (phase: " + juce::String(masterCorrelation, 2) + ")";
    s += "\n";
    ...
    // ─── Reference ─────────────────────────────────────────────────────
    if (reference > 0)
        s += "[REFERENCE " + juce::String(reference) + "/100] (loaded)\n";
    else
