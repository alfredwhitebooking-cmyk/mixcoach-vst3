...
namespace mixcoach {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    void
    TrackFeedCore::updateTrackState(int slotIndex, const TrackAudioResult& result, const SlotInfo& info, TrackRole role)
    {
        ...
        // Espectro
        for (int b = 0; b < 30; ++b) current.bandEnergies[b] = result.bandEnergies[b];
        for (int b = 0; b < 6; ++b) {
        ...
        current.crestFactor  = (current.rmsCombined > -80.0f) ? current.peakCombined - current.rmsCombined : 0.0f;
        {
            float wSum = 0.0f;
            for (int b = 0; b < 6; ++b) wSum += current.stereoWidthPerBand[b];
    ...
    void TrackFeedCore::generateEvents(TrackState& prev, const TrackState& current)
    {
        ...
        if (now <= 0) now = juce::Time::getMillisecondCounter() * 1000;
        ...
        if (current.isClipping() && !prev.wasClipping) {
            if (canFireEvent(current, TrackEventType::ClippingDetected, now)) {
                ...
                ev.timestampUs = now;
                ev.message =
                    "🔴 " + current.trackName + " está recortando a " + juce::String(current.peakCombined, 1) + " dB";
        ...
        // Clipping cleared
            ...
            ev.timestampUs = now;
            ev.message =
                "🟢 " + current.trackName + " ya no recorta (" + juce::String(current.peakCombined, 1) + " dB)";
            pushEvent(current.slotIndex, ev);
        }

        // ═══ 3. Level spike (peak subió > 6 dB respecto al ciclo anterior) ═══
        if (prev.peakCombined > -80.0f && (current.peakCombined - prev.peakCombined) > 6.0f) {
            if (canFireEvent(current, TrackEventType::LevelSpike, now)) {
                ...
                ev.message     = "⚡ " + current.trackName + " subió "
                                 + juce::String(current.peakCombined - prev.peakCombined, 1) + " dB ("
        ...
        if (current.peakCombined < kLowSignalThreshold && !prev.wasLowSignal) {
            if (canFireEvent(current, TrackEventType::LowSignal, now)) {
                ...
                ev.timestampUs = now;
                ev.message =
                    "🔇 " + current.trackName + " señal muy baja (" + juce::String(current.peakCombined, 1) + " dB)";
                ev.value     = current.peakCombined;
        ...
        if (current.crestFactor > 0.0f) {
            ...
            if (current.crestFactor < kCrestOvercompressed && prev.crestFactor >= kCrestOvercompressed) {
                if (canFireEvent(current, TrackEventType::CrestTooLow, now, 120 * 1000 * 1000)) {
                    ...
                    ev.severity    = 0.7f;
                    ev.timestampUs = now;
                    ev.message     = "⚡ " + current.trackName + " crest bajo (" + juce::String(current.crestFactor, 1)
                                     + " dB) — ¿sobre-comprimido?";
            ...
            // Muy dinámico (crest muy alto)
                ...
                if (canFireEvent(current, TrackEventType::CrestTooHigh, now, 120 * 1000 * 1000)) {
                    ...
                    ev.severity    = 0.5f;
                    ev.timestampUs = now;
                    ev.message     = "⚡ " + current.trackName + " crest alto (" + juce::String(current.crestFactor, 1)
        ...
        if (current.correlation < kPhaseIssueThreshold && prev.correlation >= kPhaseIssueThreshold) {
            ...
            if (canFireEvent(current, TrackEventType::StereoCollapse, now)) {
                ...
                ev.timestampUs = now;
                ev.message =
                    "🔮 " + current.trackName + " correlación baja (" + juce::String(current.correlation, 2) + ")";
                ev.value     = current.correlation;
        ...
        // Detecta si alguna banda espectral domina excesivamente
        {
            ...
            if (spectralSpread > kSpectralSpreadThreshold) {
                if (canFireEvent(current, TrackEventType::SpectralImbalance, now, 120 * 1000 * 1000)) {
                    ...
                    ev.timestampUs = now;
                    ev.message = "🎛️ " + current.trackName + " desbalance espectral (" + juce::String(spectralSpread, 1)
                                 + " dB entre bandas)";
        ...
        if (current.transientRatio > kTransientThreshold && prev.transientRatio <= kTransientThreshold) {
            if (canFireEvent(current, TrackEventType::TransientDetected, now, 30 * 1000 * 1000)) {
                ...
                ev.timestampUs = now;
                ev.message =
                    current.trackName + " transiente fuerte (ratio " + juce::String(current.transientRatio, 1) + ")";
                pushEvent(current.slotIndex, ev);
            }
        }
    }
    ...
    void TrackFeedCore::updateAttention(TrackState& state)
    {
        ...
        if (state.isClipping()) loudnessScore = 0.4f; // Clipping = máxima urgencia
        ...
        if (state.crestFactor < kCrestOvercompressed && state.crestFactor > 0.0f)
            ...
            if (spread > kSpectralSpreadThreshold) spectralScore = 0.2f;
        ...
        if (state.correlation < kPhaseIssueThreshold) stereoScore = 0.2f; // Fase invertida
        ...
        if (hasAnyGap && referenceBonus < 0.02f) referenceBonus = 0.02f;
    ...
    void TrackFeedCore::updateTrackRole(int slotIndex, TrackRole role)
    {
        ...
        if (prevRole != role) {
            ...
            ev.message     = "🏷️ " + states_[slotIndex].trackName + " → " + juce::String(getRoleName(role));
    ...
    void TrackFeedCore::markTrackDisappeared(int slotIndex)
    {
        ...
        if (!states_[slotIndex].active) return;
    ...
    {
        const juce::ScopedLock lock(lock_);
        for (auto& state : states_) state = TrackState{};
        for (auto& history : eventHistory_) history.clear();
        globalEvents_.clear();
    ...
    {
        const juce::ScopedLock lock(lock_);
        if (slotIndex >= 0 && slotIndex < SlotRegistry::kMaxSlots) return states_[slotIndex];
        return TrackState{};
    }
    ...
    {
        const juce::ScopedLock lock(lock_);
        if (trackId < 0 || trackId >= SlotRegistry::kMaxSlots) return {};

        const auto& history = eventHistory_[trackId];
        if (history.empty()) return {};

        int64_t cutoff = (juce::Time::getMillisecondCounter() * 1000) - TrackStateDefaults::kEventRetentionUs;
        ...
        for (int i = (int)history.size() - 1; i >= 0 && (int)result.size() < maxEvents; --i) {
            if (history[i].timestampUs >= cutoff) result.push_back(history[i]);
        }
    ...
    std::vector<TrackEvent> TrackFeedCore::getGlobalEvents(int maxEvents) const
    {
        ...
        if ((int)globalEventCache_.size() > maxEvents)
            return std::vector<TrackEvent>(globalEventCache_.begin(), globalEventCache_.begin() + maxEvents);
    ...
    {
        const juce::ScopedLock lock(lock_);
        if (trackId >= 0 && trackId < SlotRegistry::kMaxSlots) return states_[trackId].attentionScore;
        return 0.0f;
    }
    ...
    std::vector<int> TrackFeedCore::getActiveTracksByPriority() const
    {
        ...
        for (int i = 0; i < SlotRegistry::kMaxSlots; ++i) {
            if (states_[i].active && states_[i].hasSignal()) scored.emplace_back(i, states_[i].attentionScore);
        }
        ...
        for (const auto& s : scored) result.push_back(s.first);
    ...
    void TrackFeedCore::pushEvent(int slotIndex, const TrackEvent& event)
    {
        ...
        if ((int)history.size() > TrackStateDefaults::kMaxEventsPerTrack) pruneEventHistory(slotIndex);
    ...
    void TrackFeedCore::pushGlobalEvent(const TrackEvent& event)
    {
        ...
        if (callbackCopy) callbackCopy(event);
    }

    void TrackFeedCore::pruneEventHistory(int slotIndex)
    {
        auto& history = eventHistory_[slotIndex];
        if ((int)history.size() <= TrackStateDefaults::kMaxEventsPerTrack) return;
    ...
    void TrackFeedCore::rebuildGlobalCache()
    {
        ...
        for (const auto& history : eventHistory_) {
            for (const auto& ev : history) {
                if (ev.timestampUs >= cutoff) globalEventCache_.push_back(ev);
            }
        }
        ...
        std::sort(globalEventCache_.begin(), globalEventCache_.end(), [](const TrackEvent& a, const TrackEvent& b) {
    ...
    bool
    TrackFeedCore::canFireEvent(const TrackState& state, TrackEventType type, int64_t nowUs, int64_t cooldownUs) const
    {
        // Buscar el cooldown correspondiente al tipo de evento
        int64_t lastEventUs = 0;

        switch (type) {
            case TrackEventType::ClippingDetected:
                lastEventUs = state.lastClipWarningUs;
                break;
            case TrackEventType::LowSignal:
                lastEventUs = state.lastLowSignalUs;
                break;
            case TrackEventType::CrestTooLow:
            case TrackEventType::CrestTooHigh:
                lastEventUs = state.lastCrestWarningUs;
                break;
            case TrackEventType::PhaseIssue:
            case TrackEventType::StereoCollapse:
                lastEventUs = state.lastPhaseWarningUs;
                break;
            case TrackEventType::SpectralImbalance:
                lastEventUs = state.lastSpectralWarningUs;
                break;
            case TrackEventType::TransientDetected:
                lastEventUs = state.lastTransientWarningUs;
                break;
            default:
                return true; // Sin cooldown para eventos ligeros
        }
    ...
    int TrackFeedCore::getTotalEventCount() const
    {
        const juce::ScopedLock lock(lock_);
        int count = 0;
        for (const auto& history : eventHistory_) count += (int)history.size();
        return count;
    }
