...
{
    if (!hasData())
        return "\\xF0\\x9F\\x97\\xBA No hay pistas en la sesi\\xC3\\xB3n a\\xC3\\xBAn.";

    juce::String msg;
    msg = "\\xF0\\x9F\\x93\\x8B **RESUMEN DE SESI\\xC3\\x93N**\\n"
    ...
    if (achievementCount > 0)
        msg += " | \\xF0\\x9F\\x8F\\x86 " + juce::String(achievementCount) + " logros";
    msg += "\\n\\n";
    ...
    if (clippingTracks > 0 || lowSignalTracks > 0)
    {
        ...
        if (lowSignalTracks > 0) msg += "  \\xE2\\x9A\\xAA Se\\xC3\\xB1al baja: " + juce::String(lowSignalTracks) + "\\n";
    }
    if (healthyTracks > 0)
        msg += "  \\xF0\\x9F\\x9F\\xA2 Saludables: " + juce::String(healthyTracks) + "\\n";
    ...
    if (masterPeak > -90.0f)
    {
        ...
        if (masterLUFS > -80.0f)
            msg += "  LUFS (ST): " + juce::String(masterLUFS, 1) + "\\n";
    }
    ...
    if (referenceLoaded)
    {
        ...
        if (referenceName.isNotEmpty())
            msg += "  " + referenceName + "\\n";
        if (matchScore > 0.0f)
            msg += "  Match: " + juce::String(static_cast<int>(matchScore * 100.0f)) + "%\\n";
        if (referenceGaps > 0)
            msg += "  Gaps pendientes: " + juce::String(referenceGaps) + "\\n";
    }
    ...
    {
        msg += "\\n\\xE2\\x9A\\xA0 **ISSUES:**\\n";
        if (criticalIssues > 0) msg += "  \\xF0\\x9F\\x94\\xB4 Cr\\xC3\\xADticos: " + juce::String(criticalIssues) + "\\n";
...
juce::String SessionContext::toLLMContext() const
{
    ...
    switch (currentPhase) {
        case MentorPhase::Organizacion:
            ...
            ctx += "  PROHIBIDO: EQ, compresi\xC3\xB3n, reverb, saturaci\xC3\xB3n, efectos, procesamiento del master.\\n";
        ...
        case MentorPhase::Balance:
            ...
            break;
        case MentorPhase::EQ:
            ctx += "  ENFOQUE: Balance tonal con EQ. Carving espectral, filtros, correcci\xC3\xB3n de frecuencias.\\n";
        ...
        case MentorPhase::Compresion:
            ...
            ctx += "  PROHIBIDO: Masterizaci\xC3\xB3n final, reverb/delay pesados, expansi\xC3\xB3n est\xC3\xA9reo.\\n";
            break;
    ...
    if (masterPeak > -90.0f)
    {
        ...
        if (masterTruePeak > -90.0f)
            ctx += "  TruePeak: " + juce::String(masterTruePeak, 1) + " dBTP\\n";
    ...
    if (recentUserActions > 0)
        ctx += "RecentUserActions: " + juce::String(recentUserActions) + "\\n";

    // ─── Session duration ─────────────────────────────────────────────────
    if (sessionDurationUs > 0)
        ctx += "SessionDuration: " + juce::String(sessionDurationUs / 1000000) + "s\\n";
...
SessionContext CoachEngine::buildSessionContext(const std::vector<TrackIssue>* issues) const
{
    ...
    registry.forEachActive([&](const SlotInfo& info)
    {
        ...
        {
            case RoleCategory::Drums:  ctx.drumTracks++;   break;
            case RoleCategory::Bass:   ctx.bassTracks++;   break;
            case RoleCategory::Guitars:ctx.guitarTracks++; break;
            case RoleCategory::Keys:   ctx.keysTracks++;   break;
            case RoleCategory::Vocals: ctx.vocalTracks++;  break;
            case RoleCategory::FX:     ctx.fxTracks++;     break;
            case RoleCategory::Melody: ctx.melodyTracks++; break;
            default:                   ctx.unknownTracks++; break;
        }
        ...
        if (name.isNotEmpty() && !name.startsWith("Pista"))
            ctx.namedTracks++;
    ...
    if (lastUserInteractionTimeUs_ > 0)
        ctx.sessionDurationUs = nowUs - lastUserInteractionTimeUs_;
