...
juce::String DomainGap::toTextSummary() const
{
    ...
    switch (domain) {
        case Domain::Gain:
            s += "Nivel: " + metric + " " + fmtLUFS(actualValue)
                 + " vs ref " + fmtLUFS(targetValue)
                 + " (gap " + juce::String(std::abs(gap), 1) + " LUFS)";
            break;
        case Domain::Tonal:
            s += metric + ": " + fmtDb(actualValue)
                 + " vs ref " + fmtDb(targetValue)
                 + " (gap " + juce::String(std::abs(gap), 1) + " dB)";
            break;
        case Domain::Dynamics:
            s += metric + ": " + juce::String(actualValue, 1) + " dB"
                 + " vs ref " + juce::String(targetValue, 1) + " dB"
                 + " (gap " + juce::String(std::abs(gap), 1) + " dB)";
            break;
        case Domain::Spatial:
            s += metric + ": " + juce::String(actualValue, 2)
                 + " vs ref " + juce::String(targetValue, 2);
            break;
        case Domain::Loudness:
            s += metric + ": " + juce::String(actualValue, 1) + " LU"
                 + " vs ref " + juce::String(targetValue, 1) + " LU"
                 + " (gap " + juce::String(std::abs(gap), 1) + " LU)";
            break;
    }
...
juce::String DomainGap::toShortLabel() const
{
    juce::String s;
    switch (severity) {
        case GapSeverity::Critical: s += "\xF0\x9F\x94\xB4 "; break; // 🔴
        case GapSeverity::Warning:  s += "\xF0\x9F\x9F\xA1 "; break; // 🟡
        case GapSeverity::Info:     s += "\xF0\x9F\x94\xB5 "; break; // 🔵
        case GapSeverity::Praise:   s += "\xE2\x9C\x85 ";    break; // ✅
    }
...
juce::String DomainGap::toLLMContextGap() const
{
    ...
    switch (severity) {
        case GapSeverity::Critical: s += "[CRITICAL] "; break;
        case GapSeverity::Warning:  s += "[WARNING]  "; break;
        case GapSeverity::Info:     s += "[INFO]     "; break;
        case GapSeverity::Praise:   s += "[OK]       "; break;
    }
    ...
    switch (domain) {
        case Domain::Gain:
            s += " | actual=" + juce::String(actualValue, 1) + " LUFS"
                 + " target=" + juce::String(targetValue, 1) + " LUFS"
                 + " gap=" + juce::String(gap, 1) + " LUFS";
            break;
        case Domain::Tonal:
            s += " | actual=" + juce::String(actualValue, 1) + " dB"
                 + " target=" + juce::String(targetValue, 1) + " dB"
                 + " gap=" + juce::String(gap, 1) + " dB"
        ...
        case Domain::Dynamics:
            ...
            break;
        case Domain::Spatial:
            s += " | actual=" + juce::String(actualValue, 2)
                 + " target=" + juce::String(targetValue, 2)
                 + " gap=" + juce::String(gap, 2);
            break;
        case Domain::Loudness:
            s += " | actual=" + juce::String(actualValue, 1) + " LU"
                 + " target=" + juce::String(targetValue, 1) + " LU"
                 + " gap=" + juce::String(gap, 1) + " LU";
            break;
    }
...
{
    switch (d) {
        case Domain::Gain:     return "GANANCIA";
        case Domain::Tonal:    return "TONAL";
        case Domain::Dynamics: return "DINAMICA";
        case Domain::Spatial:  return "ESPACIAL";
...
{
    switch (s) {
        case GapSeverity::Critical: return "\xF0\x9F\x94\xB4 CRITICAL "; // 🔴
        case GapSeverity::Warning:  return "\xF0\x9F\x9F\xA1 WARNING  "; // 🟡
        case GapSeverity::Info:     return "\xF0\x9F\x94\xB5 Info     "; // 🔵
        case GapSeverity::Praise:   return "\xE2\x9C\x85 Praise    ";   // ✅
    }
    return "?";
}
...
{
    switch (s) {
        case GapSeverity::Critical: return juce::Colour(0xFFFF5252); // Red
        case GapSeverity::Warning:  return juce::Colour(0xFFFFC107); // Yellow
        case GapSeverity::Info:     return juce::Colour(0xFF00B7FF); // Cyan
        case GapSeverity::Praise:   return juce::Colour(0xFF4CAF50); // Green
    }
...
const char* ReferenceDrivenEngine::regionCenterFreq(int regionIdx) noexcept
{
    static const char* freq[] = {
        "60Hz", "200Hz", "600Hz", "2kHz", "5kHz", "12kHz"
    };
...
const char* ReferenceDrivenEngine::regionFreqRange(int regionIdx) noexcept
{
    static const char* ranges[] = {
        "0-86Hz", "86-301Hz", "301-1076Hz", "1076-3532Hz",
        "3532-8355Hz", "8355-16458Hz"
    };
    return (regionIdx >= 0 && regionIdx < 6) ? ranges[regionIdx] : "?";
}

float ReferenceDrivenEngine::regionEnergy(
    const float bandEnergies[30], int regionStartBand, int regionEndBand)
{
    double sum = 0.0;
    int count = 0;
...
std::vector<DomainGap> ReferenceDrivenEngine::computeGaps(
    ...
    const juce::String& genre)
{
    ...
    if (!fp.valid)
        return gaps; // No reference data — return empty
...
DomainGap ReferenceDrivenEngine::analyzeGainDomain(
    const ReferenceFingerprint& fp,
    const AudioAnalyzer& master)
{
    DomainGap g;
    ...
    if (gapLUFS > 0) {
        g.description = "La mezcla esta " + juce::String(absGap, 1)
                        + " LUFS MAS FUERTE que la referencia";
        if (absGap >= 6.0f) {
            // Gap grande: sugerir reducción escalonada
            g.suggestion = "Baja " + juce::String(absGap * 0.5f, 1)
                           + " dB el fader master primero y vuelve a escuchar. "
            ...
            g.suggestion = "Baja " + juce::String(absGap, 1)
                           + " dB el fader master y verifica con el LUFS meter. "
        ...
        if (absGap >= 6.0f) {
            ...
            g.suggestion = "Sube " + juce::String(absGap, 1)
                           + " dB el fader master o agrega " + juce::String(absGap * 0.3f, 1)
                           + " dB de makeup gain en el compresor del master. "
                           + "Objetivo: " + juce::String(fp.lufsIntegrated, 1) + " LUFS integrated.";
        }
        g.actionVerb = "subir";
    }
    ...
    if (mixTP > -1.0f && refTP < -3.0f) {
        ...
        if (g.severity == GapSeverity::Info)
            g.severity = GapSeverity::Warning;
        ...
        g.suggestion += ". Adem\xC3\xA1" "s, deja al menos 1dB de headroom en el master";
    }
...
std::vector<DomainGap> ReferenceDrivenEngine::analyzeTonalDomain(
    const ReferenceFingerprint& fp,
    const AudioAnalyzer& master)
{
    std::vector<DomainGap> gaps;
    ...
    for (int b = 0; b < 30; ++b) {
        ...
        for (int i = startBin; i < endBin; ++i) {
            ...
            if (mag > 1e-10f)
                sum += 20.0 * std::log10(static_cast<double>(mag));
            else
                sum += -100.0;
            ++count;
        }
    ...
    for (int b = 0; b < 30; ++b) {
        if (fp.bandEnergies[b] > -80.0f) { refSum += fp.bandEnergies[b]; refCount++; }
        if (mixBands[b] > -80.0f) { mixSum += mixBands[b]; mixCount++; }
    }
    ...
    for (int r = 0; r < 6; ++r) {
        ...
        if (refRegion < -80.0f || mixRegion < -80.0f)
            continue; // Sin datos en esta región
    ...
    if (regionDiffs.empty())
        return gaps;

    // Ordenar por magnitud de diferencia descendente
    std::sort(regionDiffs.begin(), regionDiffs.end(),
        [](const RegionDiff& a, const RegionDiff& b) {
    ...
    for (auto& rd : regionDiffs) {
        if (rd.diff > 0 && rd.absDiff >= kSpectralWarningDb) {
            if (worstExcess == nullptr || rd.absDiff > worstExcess->absDiff)
                worstExcess = &rd;
        } else if (rd.diff < 0 && rd.absDiff >= kSpectralWarningDb) {
            if (worstDeficit == nullptr || rd.absDiff > worstDeficit->absDiff)
                worstDeficit = &rd;
        }
    }
    ...
    if (worstExcess != nullptr) {
        ...
        g.metric = juce::String(regionName(worstExcess->regionIdx))
                   + " (" + regionFreqRange(worstExcess->regionIdx) + ")";

        float refRegion = regionEnergy(fp.bandEnergies,
            kRegionBands[worstExcess->regionIdx][0], kRegionBands[worstExcess->regionIdx][1]);
        float mixRegion = regionEnergy(mixBands,
            kRegionBands[worstExcess->regionIdx][0], kRegionBands[worstExcess->regionIdx][1]);
        ...
        if (absGap >= kSpectralCriticalDb)
            g.severity = GapSeverity::Critical;
        else
            g.severity = GapSeverity::Warning;

        float eqGain = suggestedEqGainDb(-absGap, worstExcess->regionIdx); // negativo = reducir
        g.description = juce::String(regionName(worstExcess->regionIdx))
                        + " esta " + juce::String(absGap, 1)
                        + " dB por ENCIMA de la referencia";
        if (std::abs(eqGain) >= 0.5f) {
            g.suggestion = "Reduce " + juce::String(eqGain, 1)
                           + " dB en " + juce::String(regionCenterFreq(worstExcess->regionIdx))
                           + " con un EQ " + juce::String(regionFilterType(worstExcess->regionIdx))
                           + " (Q=" + juce::String(regionQ(worstExcess->regionIdx), 1) + ")"
                           + " en los " + juce::String(regionInstrumentTarget(worstExcess->regionIdx));
        } else {
            g.suggestion = "Atenuacion sutil (<0.5dB) en "
                           + juce::String(regionCenterFreq(worstExcess->regionIdx))
    ...
    if (worstDeficit != nullptr) {
        ...
        float refRegion = regionEnergy(fp.bandEnergies,
            kRegionBands[worstDeficit->regionIdx][0], kRegionBands[worstDeficit->regionIdx][1]);
        float mixRegion = regionEnergy(mixBands,
            kRegionBands[worstDeficit->regionIdx][0], kRegionBands[worstDeficit->regionIdx][1]);
        ...
        g.description = juce::String(regionName(worstDeficit->regionIdx))
                        + " esta " + juce::String(absGap, 1)
                        + " dB por DEBAJO de la referencia";
        if (std::abs(eqGain) >= 0.5f) {
            g.suggestion = "Sube " + juce::String(eqGain, 1)
                           + " dB en " + juce::String(regionCenterFreq(worstDeficit->regionIdx))
                           + " con un EQ " + juce::String(regionFilterType(worstDeficit->regionIdx))
                           + " (Q=" + juce::String(regionQ(worstDeficit->regionIdx), 1) + ")"
                           + " en " + juce::String(regionInstrumentTarget(worstDeficit->regionIdx));
        } else {
            g.suggestion = "Aumento sutil (<0.5dB) en "
                           + juce::String(regionCenterFreq(worstDeficit->regionIdx))
...
DomainGap ReferenceDrivenEngine::analyzeDynamicsDomain(
    const ReferenceFingerprint& fp,
    const AudioAnalyzer& master)
{
    DomainGap g;
    ...
    if (crestGap > 0) {
        ...
        if (absGap >= 6.0f) {
            g.suggestion = "Agrega compresi\xC3\xB3" "n: ratio 3:1, threshold -24dB, "
                           "attack 5ms, release 60ms en los buses de bater\xC3\xAD" "a y bajo. "
                           "Objetivo: crest de " + juce::String(refCrest, 1) + " dB.";
        } else {
            g.suggestion = "Agrega compresi\xC3\xB3" "n suave: ratio 2:1, threshold -18dB, "
                           "attack 10ms, release 80ms en los buses m\xC3\xA1" "s din\xC3\xA1" "micos. "
                           "Objetivo: crest de " + juce::String(refCrest, 1) + " dB.";
        }
        ...
        g.description = "La mezcla es " + juce::String(absGap, 1)
                        ...
                        + juce::String(mixCrest, 1) + "dB vs ref " + juce::String(refCrest, 1) + "dB)";
        if (absGap >= 6.0f) {
            g.suggestion = "Reduce compresi\xC3\xB3" "n agresiva en buses: sube el threshold a -12dB, "
                           "baja el ratio a 1.5:1, attack 20ms, release 120ms. "
                           "Objetivo: crest de " + juce::String(refCrest, 1) + " dB.";
        } else {
            g.suggestion = "Afloja la compresi\xC3\xB3" "n: prueba ratio 1.5:1, "
...
DomainGap ReferenceDrivenEngine::analyzeSpatialDomain(
    const ReferenceFingerprint& fp,
    const AudioAnalyzer& master)
{
    DomainGap g;
    ...
    g.normalisedGap = (kCorrelationThreshold > 0.0f)
        ? std::min(1.0f, absGap / kCorrelationThreshold)
        : 0.0f;
    ...
    if (mixCorr > refCorr + kCorrelationThreshold) {
        ...
        if (mixCorr > 0.85f) {
            g.suggestion = "La mezcla es casi mono! Abre el est\xC3\xA9" "reo: aplica un ensanchador "
                           "est\xC3\xA9" "reo (mix 30-40%) en pads/keys. Agrega delay ping-pong "
                           "1/8 nota, feedback 15%, mix 20% en elementos secundarios. "
                           "Verifica que el bajo y bombo queden en mono (< 80Hz)."
                           "\nObjetivo: correlaci\xC3\xB3" "n de " + juce::String(refCorr, 2);
        } else {
            g.suggestion = "Abre el est\xC3\xA9" "reo suavemente: prueba ensanchador est\xC3\xA9" "reo "
                           "(mix 20-30%) en pads/keys o delay ping-pong "
                           "1/8 nota, feedback 20%, mix 15% en acompa\xC3\xB1" "amiento. "
                           "\nObjetivo: correlaci\xC3\xB3" "n de " + juce::String(refCorr, 2);
        }
        ...
        g.description = "La mezcla tiene correlaci\xC3\xB3" "n " + juce::String(absGap, 2)
                        ...
                        + juce::String(mixCorr, 2) + " vs " + juce::String(refCorr, 2) + ")";
        if (mixCorr < 0.3f) {
            g.suggestion = "PELIGRO: correlaci\xC3\xB3" "n muy baja! Problemas de fase probables. "
                           "Revisa fase en kick y bass con un correlaci\xC3\xB3" "nmetro. "
                           "Prueba M/S EQ: reduce ancho est\xC3\xA9" "reo en 200-800Hz. "
                           "\nObjetivo: correlaci\xC3\xB3" "n de " + juce::String(refCorr, 2);
        } else {
            g.suggestion = "Reduce est\xC3\xA9" "reo widening en medios (400Hz-3kHz). "
                           "Verifica compatibilidad mono. "
                           "Prueba un correlaci\xC3\xB3" "nmetro en el master para identificar "
                           "pistas con fase problem\xC3\xA1" "tica."
                           "\nObjetivo: correlaci\xC3\xB3" "n de " + juce::String(refCorr, 2);
        }
        g.actionVerb = "centrar";
    } else {
        g.severity = GapSeverity::Praise;
        g.description = "Ancho est\xC3\xA9" "reo similar a la referencia ("
                        + juce::String(mixCorr, 2) + " vs " + juce::String(refCorr, 2) + ")";
        g.suggestion = "El balance est\xC3\xA9" "reo ya est\xC3\xA1" " alineado con la referencia. "
                       "Contin\xC3\xBA" "a con el siguiente paso.";
        g.actionVerb = "mantener";
    }
...
DomainGap ReferenceDrivenEngine::analyzeLoudnessRangeDomain(
    const ReferenceFingerprint& fp,
    const AudioAnalyzer& master)
{
    DomainGap g;
    ...
    g.normalisedGap = (kLoudnessRangeCritical > 0.0f)
        ? std::min(1.0f, absGap / kLoudnessRangeCritical)
        : 0.0f;
    ...
    if (rangeGap > 0) {
        ...
        if (absGap >= 6.0f) {
            g.suggestion = "Demasiada variaci\xC3\xB3" "n din\xC3\xA1" "mica. Aplica compresi\xC3\xB3" "n 3:1, "
                           "threshold -28dB, attack 5ms, release 40ms en el bus master. "
                           "Luego compresi\xC3\xB3" "n adicional 2:1, threshold -20dB en los buses "
                           "m\xC3\xA1" "s din\xC3\xA1" "micos. Objetivo: " + juce::String(refRange, 1) + " LU.";
        } else {
            g.suggestion = "Aplica compresi\xC3\xB3" "n suave 2:1, threshold -24dB, "
                           "attack 10ms, release 60ms en el bus master. "
                           "Objetivo: " + juce::String(refRange, 1) + " LU.";
        }
        g.actionVerb = "comprimir";
    } else {
        g.description = "El rango de loudness es " + juce::String(absGap, 1)
                        ...
                        + juce::String(mixRange, 1) + " vs " + juce::String(refRange, 1) + " LU)";
        if (absGap >= 4.0f) {
            g.suggestion = "Mezcla demasiado comprimida. Reduce compresi\xC3\xB3" "n en el master: "
                           "sube threshold a -12dB, baja ratio a 1.5:1. "
                           "Si usas limitador agresivo, lib\xC3\xA9" "ralo 2-3dB. "
                           "Objetivo: " + juce::String(refRange, 1) + " LU.";
        } else {
            g.suggestion = "Afloja la compresi\xC3\xB3" "n del master: prueba threshold -16dB, "
