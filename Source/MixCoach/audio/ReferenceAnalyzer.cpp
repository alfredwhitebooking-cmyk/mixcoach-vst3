...
bool ReferenceAnalyzer::loadFile(const juce::String& filePath)
{
    ...
    if (reader == nullptr)
    {
        LogHelper::writeToLog("[ReferenceAnalyzer] No se pudo leer: " + filePath
    ...
    audioBuffer_.clear();
    audioBuffer_.setSize(static_cast<int>(numChannels),
    ...
    LogHelper::writeToLog("[ReferenceAnalyzer] Referencia cargada: " + fileName_
                          + " (" + juce::String(sampleRate_ / 1000) + " kHz, "
...
// ═══════════════════════════════════════════════════════════════════════════
bool ReferenceAnalyzer::loadFromBuffer(const float* bufferL, const float* bufferR,
...
// ═══════════════════════════════════════════════════════════════════════════
void ReferenceAnalyzer::analyzeAudioBuffer(const juce::AudioBuffer<float>& buffer,
                                            int sampleRate)
{
    ...
    LogHelper::writeToLog("[ReferenceAnalyzer] Análisis completado: "
        "Spectrum bins=" + juce::String(kNumSpectrumBins)
        + " | LUFS M=" + juce::String(getMomentaryLUFS(), 1)
        + " ST=" + juce::String(getShortTermLUFS(), 1)
        + " I=" + juce::String(getIntegratedLUFS(), 1));
}
...
void ReferenceAnalyzer::computeBandEnergies()
{
    ...
    {
        bandEnergies_[b] = spectrumBandEnergy(
            spectrum, kBandBins[b][0], kBandBins[b][1]);
    }
}

float ReferenceAnalyzer::spectrumBandEnergy(const float* spectrum,
...
{
    if (!loaded_ || band < 0 || band >= kNumBands)
        return -100.0f;
...
ReferenceComparison ReferenceAnalyzer::compareWith(const AudioAnalyzer& master) const
{
    ...
    if (masterSpectrum == nullptr || masterAnalysis.getLastUpdateTime() == 0)
        return result;
    ...
    for (int b = 0; b < kNumBands; ++b)
    {
        masterBands[b] = spectrumBandEnergy(
            masterSpectrum, kBandBins[b][0], kBandBins[b][1]);
    }
    ...
    // ─── Generar resumen textual ──────────────────────────────────────────
    {
        ...
        struct BandDiff { float diff; const char* name; };
        BandDiff bandDiffs[kNumBands] = {
            { result.subDiff, "Sub" }, { result.bassDiff, "Bass" },
            { result.lowMidDiff, "Low-Mid" }, { result.highMidDiff, "High-Mid" },
            { result.presenceDiff, "Presence" }, { result.highDiff, "High" },
            { result.airDiff, "Air" }
        };
        ...
        for (int i = 0; i < shown; ++i)
        {
            ...
            {
                summary += "  🔴 " + juce::String(bd.name) + " +"
                           + juce::String(std::abs(bd.diff), 1) + " dB (ref tiene más)\n";
            }
            else
            {
                summary += "  🔵 " + juce::String(bd.name) + " -"
                           + juce::String(std::abs(bd.diff), 1) + " dB (ref tiene menos)\n";
            }
        }
        ...
        if (masterIntegrated > -60.0f)
        {
            ...
            if (lufsGap > 2.0f)
                summary += "  ⬆️ Sube " + juce::String(lufsGap, 1) + " dB para igualar loudness\n";
