...
namespace mixcoach {
                                                                 ...
                                                                 const std::map<juce::String, float>& thresholds)
    {
        ...
        if (auto it = thresholds.find("centroid"); it != thresholds.end()) {
            ...
            {
                juce::String desc = juce::String("Centroid: ") + juce::String(mixMetrics.centroid, 1) + " Hz (ref "
                                    + juce::String(refMetrics.centroid, 1) + " Hz) – "
        ...
        for (int i = 0; i < ReferenceMetrics::kNumBands; ++i) {
            ...
            if (auto it = thresholds.find(key); it != thresholds.end()) {
                ...
                if (std::abs(deltaDb) > it->second) {
                    ...
                    static const char* bandNames[ReferenceMetrics::kNumBands] = {
                        "Sub", "Bass", "Low‑Mid", "High‑Mid", "Presence", "High", "Air"};
                    juce::String desc = juce::String(bandNames[i]) + ": " + dbToString(mixRmsDb) + " (ref "
                                        + dbToString(refRmsDb) + ") – " + (deltaDb > 0 ? "Increase" : "Decrease")
                                        + " by " + dbToString(std::abs(deltaDb)) + ".";
        ...
        if (auto it = thresholds.find("crestFactor"); it != thresholds.end()) {
            ...
            if (std::abs(delta) > it->second) {
                juce::String desc = juce::String("Crest factor: ") + dbToString(mixCrest) + " (ref "
                                    + dbToString(refCrest) + ") – " + (delta > 0 ? "Increase" : "Decrease") + " by "
                                    + dbToString(std::abs(delta)) + ".";
            ...
            if (auto it = thresholds.find(key); it != thresholds.end()) {
                ...
                if (std::abs(delta) > it->second) {
                    static const char* bandNames[ReferenceMetrics::kNumBands] = {
                        "Sub", "Bass", "Low‑Mid", "High‑Mid", "Presence", "High", "Air"};
                    juce::String desc =
                        juce::String(bandNames[i]) + " LUFS: " + juce::String(mixMetrics.lufsByBand[i], 1) + " (ref "
                        + juce::String(refMetrics.lufsByBand[i], 1) + ") – " + (delta > 0 ? "Increase" : "Decrease")
                        + " by " + dbToString(std::abs(delta)) + ".";
