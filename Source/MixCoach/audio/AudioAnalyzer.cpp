...
void AudioAnalyzer::prepare(double sampleRate, int samplesPerBlock)
{
    ...
    if (masterBuffer_.getNumSamples() < samplesPerBlock)
        masterBuffer_.setSize(1, samplesPerBlock);
}
...
void AudioAnalyzer::processBlock(const juce::AudioBuffer<float>& buffer)
{
    ...
    if (buffer.getNumChannels() >= 2) {
        ...
        if (numSamples > masterBuffer_.getNumSamples())
            masterBuffer_.setSize(1, numSamples, false, false, true);
        ...
        if (denom > 1e-12)
            correlation = static_cast<float>(sumProduct / denom);
...
int AudioAnalyzer::flushSamples(float* leftOut, float* rightOut, int maxCount) const
{
    ...
    if (writePos <= readPos || maxCount <= 0)
        return 0;
