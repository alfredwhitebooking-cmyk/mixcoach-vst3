...
constexpr float kLabelFreqsHz[] = {
    20.0f, 30.0f, 50.0f, 70.0f, 100.0f, 200.0f, 300.0f, 500.0f, 700.0f,
    1000.0f, 2000.0f, 3000.0f, 5000.0f, 7000.0f, 10000.0f, 20000.0f
};
...
void SpectrographComponent::drawGrid(juce::Graphics& g, juce::Rectangle<float> plot) const
{
    ...
    {
        auto glowZone = juce::Rectangle<float>(plot.getX(), y0 - 1.5f,
                                                plot.getWidth(), 3.0f);
        juce::ColourGradient zeroGlow(
            juce::Colour(0xFFFFFFFF).withAlpha(0.08f),
    ...
    constexpr float subFreqs[] = { 25.0f, 30.0f, 35.0f, 40.0f, 45.0f, 50.0f,
                                   55.0f, 60.0f, 65.0f, 70.0f, 75.0f, 80.0f,
                                   85.0f, 90.0f, 95.0f };
...
void SpectrographComponent::drawRtaBars(juce::Graphics& g, juce::Rectangle<float> plot) const
{
    ...
    struct BarDim { float x, w, h, topY; float level; juce::Colour col; };
    ...
    for (int i = 0; i < kNumRtaBands; ++i)
    {
        ...
        const float freqLogT = std::log2(band.centerHz / 20.0f)
    ...
    // PASO 1: Spectral Fill — masa continua debajo de las barras
    {
        ...
        juce::Colour fillCol = bars[(size_t)(kNumRtaBands / 4)].col
            .interpolatedWith(bars[(size_t)(kNumRtaBands * 3 / 4)].col, 0.5f);

        juce::ColourGradient fillGrad(
            fillCol.withAlpha(0.12f),
    ...
    for (int i = 0; i < kNumRtaBands; ++i)
    {
        ...
        juce::ColourGradient capGlow(
            bar.col.withAlpha(0.20f),
    ...
    for (int db = 0; db >= (int) kDisplayBottomDb; db -= 5)
    {
        ...
        else
        {
            ...
            g.drawHorizontalLine(juce::roundToInt(y),
                                 labelCol.getRight() - 0.5f,
                                 labelCol.getRight() + 3.0f);
        }
    }
}
...
void SpectrographComponent::drawFreqAxis(juce::Graphics& g, juce::Rectangle<float> plot) const
{
    ...
    static constexpr float kLabelFreqsHz[] = {
        20.0f, 30.0f, 50.0f, 70.0f, 100.0f, 200.0f, 300.0f, 500.0f, 700.0f,
        1000.0f, 2000.0f, 3000.0f, 5000.0f, 7000.0f, 10000.0f, 20000.0f
    };
    ...
    auto labelRow = juce::Rectangle<float>(0.0f, labelY,
                                         (float) getWidth(), labelH);
    ...
    juce::ColourGradient edgeGrad(
        juce::Colour(0xFF8899BB).withAlpha(0.20f),
        edgeGlow.getX(), edgeGlow.getY(),
        juce::Colour(0xFF8899BB).withAlpha(0.0f),
        edgeGlow.getRight(), edgeGlow.getY(), false);
...
void SpectrographComponent::drawSettingsChrome(juce::Graphics& g) const
{
    ...
    static const juce::Colour kModeColours[] = {
        ...
        juce::Colour(0xFF44BBFF),
        juce::Colour(0xFF77DDFF)
    };
    ...
    {
        juce::String tipText = on
            ? juce::CharPointer_UTF8("Hide reference curve  ")
...
{
    if (!waterfallEnabled_ || waterfallCount_ == 0)
        return;
    ...
    struct BandRect { float x; float w; };
    ...
    int startIdx;
    if (waterfallCount_ < kWaterfallRows)
        startIdx = 0;
    else
    ...
    for (int row = 0; row < waterfallCount_; ++row)
    {
        ...
        if (sliceY + sliceH > plot.getBottom())
            continue;
    ...
    juce::ColourGradient fadeGrad(
        MixCoachTheme::bgPanel().withAlpha(0.65f),
...
void SpectrographComponent::drawReferenceOverlay(juce::Graphics& g,
                                                  juce::Rectangle<float> plot) const
{
    if (!referenceEnabled_ || referenceCurve_.empty() || bands_.empty())
        return;
...
void SpectrographComponent::drawDiagnosticOverlay(juce::Graphics& g,
                                                    juce::Rectangle<float> plot) const
{
    if (activeDiagnostics_.empty())
        return;
    ...
    for (const auto& diag : activeDiagnostics_)
    {
        ...
        auto bandRect = juce::Rectangle<float>(x0, plot.getY(),
        ...
        if (diag.isCritical)
        {
            ...
            g.fillRect(bandRect.getX(), bandRect.getY(),
                       bandRect.getWidth(), 2.0f);

            auto glowRect = bandRect.withHeight(8.0f);
            juce::ColourGradient glowGrad(
                diagCol.withAlpha(0.20f),
                juce::Point<float>(glowRect.getCentreX(), glowRect.getY()),
                diagCol.withAlpha(0.0f),
                juce::Point<float>(glowRect.getCentreX(), glowRect.getBottom()),
                false);
        ...
        if (diag.trackName.isNotEmpty())
        {
            ...
            if (diag.trackRole.isNotEmpty())
                label += " (" + diag.trackRole + ")";

            g.setFont(juce::Font(juce::FontOptions(MixCoachTheme::fontSizeMicro)).boldened());
            g.setColour(diagCol.withAlpha(0.85f));

            float labelW = juce::jmin((float)label.length() * 5.0f + 6.0f,
                                      bandRect.getWidth());
            auto labelRect = juce::Rectangle<float>(
                x0 + 2.0f, plot.getY() + 2.0f,
                labelW, 12.0f);
...
void SpectrographComponent::drawFrequencyMarkers(juce::Graphics& g,
                                                  juce::Rectangle<float> plot) const
{
    if (frequencyMarkers_.empty())
        return;
    ...
    for (const auto& marker : frequencyMarkers_)
    {
        ...
        if (marker.isPraise)
            markerCol = juce::Colour(0xFF10B981);
        ...
        g.fillEllipse(x - glowRadius, plot.getY() - glowRadius * 0.3f,
                      glowRadius * 2.0f, glowRadius * 2.0f * 0.6f);

        float midRadius = marker.isCritical ? 12.0f : 9.0f;
        g.setColour(markerCol.withAlpha(0.20f * alpha));
        g.fillEllipse(x - midRadius, plot.getY() - midRadius * 0.3f,
                      midRadius * 2.0f, midRadius * 2.0f * 0.6f);
...

        {
            ...
            if (marker.description.isNotEmpty())
                displayText += " " + marker.description;
...
void SpectrographComponent::drawHoverCursor(juce::Graphics& g,
                                              juce::Rectangle<float> plot) const
{
    if (!mouseOverPlot_)
        return;
    ...
    juce::String freqStr;
    if (hoverFreqHz_ >= 10000.0f)
        freqStr = juce::String(hoverFreqHz_ / 1000.0f, 1) + " kHz";
    ...
    if (tooltipX + tooltipW > plot.getRight())
        tooltipX = x - tooltipW - 10.0f;
    if (tooltipY + tooltipH > plot.getBottom())
        tooltipY = plot.getBottom() - tooltipH - 2.0f;
    if (tooltipY < plot.getY() + 2.0f)
        tooltipY = plot.getY() + 2.0f;
    ...
    g.drawText(freqStr,
               tooltipRect.reduced(5, 0).withHeight(16.0f),
               juce::Justification::centredLeft);

    g.setFont(juce::Font(juce::FontOptions(MixCoachTheme::fontSizeTiny)));
    juce::Colour dbCol = (hoverDbLevel_ > -6.0f)
        ? juce::Colour(0xFFFFCC44)
        : juce::Colour(0xFF8899BB);
    g.setColour(dbCol.withAlpha(0.90f));
    g.drawText(dbStr,
               tooltipRect.reduced(5, 0).withTrimmedTop(16.0f),
               juce::Justification::centredLeft);
}
...
void SpectrographComponent::drawCentroidMarker(juce::Graphics& g,
                                                 juce::Rectangle<float> plot) const
{
    if (!centroidInfo_.valid() || !layout_.valid)
        return;
...

    {
        ...
        juce::Path diamond;
        diamond.addTriangle(actualX, topY - 5.0f,
                            actualX - 5.0f, topY + 2.0f,
                            actualX + 5.0f, topY + 2.0f);
        g.fillPath(diamond);
...

    {
        ...
        juce::Path diamond;
        diamond.addTriangle(expectedX, topY - 5.0f,
                            expectedX - 5.0f, topY + 2.0f,
                            expectedX + 5.0f, topY + 2.0f);
        g.fillPath(diamond);
...

    {
        ...
        juce::String badgeText = "C: " + formatFreqVal(centroidInfo_.actualHz)
                               + " | E: " + formatFreqVal(centroidInfo_.expectedHz);
        if (centroidInfo_.genre.isNotEmpty())
            badgeText += " [" + centroidInfo_.genre + "]";
