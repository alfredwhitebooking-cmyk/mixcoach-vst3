...
namespace mixcoach { class CoachEngine; }
...
public:
    ...
    void setShowSystemMessages(bool show) noexcept { showSystemMessages_ = show; resized(); repaint(); }
...
private:
    ...
    void drawCoachBubbleStreaming(juce::Graphics& g, juce::Rectangle<float> bubbleBounds,
                                  const ChatBubble& msg);
    ...
    void drawSystemMessage(juce::Graphics& g, juce::Rectangle<float> bounds,
                           const ChatBubble& msg);
...
public:
    ...
    {
        masterMeterPanel_.updateMeters(analyzer);
    }
    ...
    void setSuggestions(const std::vector<juce::String>& suggestions) { suggestions_ = suggestions; repaint(); }
    void setAiTyping(bool isTyping) { chatMessages_.setTypingIndicator(isTyping); resized(); }
