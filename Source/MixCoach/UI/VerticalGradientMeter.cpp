...
namespace mixcoach {
    ...
    juce::Colour VerticalGradientMeter::colourForDb(float db) noexcept
    {
        ...
        if (norm < 0.75f)
            return MixCoachTheme::meterLime().interpolatedWith(MixCoachTheme::meterYellow(), (norm - 0.55f) / 0.20f);
        if (norm < 0.88f)
            return MixCoachTheme::meterYellow().interpolatedWith(MixCoachTheme::meterOrange(), (norm - 0.75f) / 0.13f);

        return MixCoachTheme::meterOrange().interpolatedWith(MixCoachTheme::meterRed(), (norm - 0.88f) / 0.12f);
    }

    void VerticalGradientMeter::drawDbScale(juce::Graphics& g, juce::Rectangle<float> bounds, bool topIsZero)
    {
    ...
    void VerticalGradientMeter::drawGradientBar(juce::Graphics& g,
                                                ...
                                                float radius)
    {
        ...
        juce::ColourGradient grad(MixCoachTheme::meterGreen(),
                                  bounds.getCentreX(),
                                  bounds.getBottom(),
    ...
    void VerticalGradientMeter::drawSolidBar(
        juce::Graphics& g, juce::Rectangle<float> bounds, float levelDb, juce::Colour colour, float radius)
    {
        g.setColour(MixCoachTheme::bgDarker());
        g.fillRoundedRectangle(bounds, radius);
    ...
    void VerticalGradientMeter::drawPeakTriangle(
        juce::Graphics& g, juce::Rectangle<float> scaleBounds, float peakHoldDb, juce::Colour colour, bool alignLeft)
    {
        if (peakHoldDb <= kMinDb + 0.5f) return;
        ...
        if (alignLeft) {
            ...
            tri.addTriangle(x, y, x - triSize, y - triSize + 0.5f, x - triSize, y + triSize - 0.5f);
        }
        else {
    ...
    void VerticalGradientMeter::drawPeakReadout(juce::Graphics& g,
                                                juce::Rectangle<float> bounds,
                                                float peakDb,
                                                juce::Colour colour)
    {
    ...
    {
        displayDb = juce::jlimit(VerticalGradientMeter::kMinDb, VerticalGradientMeter::kMaxDb + 3.0f, db);
    ...
    void MeterChannelBallistics::tickHold() noexcept
    {
        if (peakHoldFrames > 0) {
            ...
            peakHoldDb -= 0.55f;
            if (peakHoldDb < VerticalGradientMeter::kMinDb) peakHoldDb = VerticalGradientMeter::kMinDb;
        }
    }
