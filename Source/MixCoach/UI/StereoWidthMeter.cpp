...
void StereoWidthMeter::setPerBandWidth(const float* perBand)
{
    ...
    for (int b = 0; b < kNumBands; ++b)
        perBandValues_[b] = juce::jlimit(0.0f, 1.0f, perBand[b]);
}
...
const StereoWidthMeter::WidthZone& StereoWidthMeter::getZone(float width) const noexcept
{
    for (int i = 0; i < kNumZones; ++i) {
        if (width >= zones_[i].minNorm && width < zones_[i].maxNorm)
            return zones_[i];
    }
...
void StereoWidthMeter::drawHeader(juce::Graphics& g, juce::Rectangle<float> area)
{
    ...
    g.drawText(juce::String::fromUTF8("\xF0\x9F\x8E\xA7 ANCHO ESTEREO"),
    ...
    g.drawText(juce::String(static_cast<int>(curWidth * 100.0f)) + "%",
               badgeArea.removeFromLeft(30), juce::Justification::centredLeft);

    // Zone name + icon
    g.setFont(juce::Font(juce::FontOptions(MixCoachTheme::fontSizeExtraTiny)).boldened());
    g.setColour(badgeCol.withAlpha(0.8f));
    g.drawText(juce::String(zone.icon) + " " + juce::String(zone.label),
               badgeArea, juce::Justification::centredLeft);
}
...
void StereoWidthMeter::drawBar(juce::Graphics& g, juce::Rectangle<float> area)
{
    ...
    g.drawText("0%", juce::Rectangle<float>(area.getX() + 2, area.getBottom() - 10, 16, 8),
               juce::Justification::centredLeft);
    g.drawText("100%", juce::Rectangle<float>(area.getRight() - 22, area.getBottom() - 10, 20, 8),
...
void StereoWidthMeter::drawPerBand(juce::Graphics& g, juce::Rectangle<float> area)
{
    ...
    g.drawText(juce::String::fromUTF8("\xF0\x9F\x93\x8A POR FRECUENCIA"),
    ...
    static const char* kBandHints[kNumBands] = {
        "Ultra", "Graves", "Cuerpo", "Presen.", "Brillo", "Aire"
    };
    ...
    for (int b = 0; b < kNumBands; ++b) {
        ...
        if (val > 0.01f) {
            ...
            else if (val < 0.40f) bandCol = juce::Colour(0xFF22C55E); // verde (natural)
            else if (val < 0.70f) bandCol = juce::Colour(0xFF3B82F6); // azul (amplio)
            else                  bandCol = juce::Colour(0xFFF97316); // naranja (exceso)
