...
namespace mixcoach {
    ...
    void MasterMeterPanel::paint(juce::Graphics& g)
    {
        ...
        // ─── SEPARATOR line ──────────────────────────────────────────────────
        {
            juce::Rectangle<int> sepArea(area.getX(), area.getY(), area.getWidth(), 1);
    ...
    void MasterMeterPanel::drawBackground(juce::Graphics& g, juce::Rectangle<int> bounds)
    {
        ...
        g.drawHorizontalLine((int)b.getY() + 1, (int)b.getX() + 4, (int)b.getRight() - 4);
    }
    ...
    //    [LUFS]        ← 10px, gray
    // ═══════════════════════════════════════════════════════════════════════════
    void MasterMeterPanel::drawBigValue(juce::Graphics& g,
                                        ...
                                        bool dominant)
    {
        ...
        // Value text
        juce::String valStr;
        if (value > -60.0f) valStr = juce::String(value, 1);
        else
            valStr = "--.-";

        // Add + sign for positive values
        if (value > 0.0f && value < 60.0f) valStr = "+" + valStr;
    ...
    void MasterMeterPanel::drawSecondaryRow(juce::Graphics& g, juce::Rectangle<int> bounds)
    {
        ...
        juce::String lraStr = (lra > 0.1f) ? "LRA " + juce::String((int)lra) + " LU" : "LRA -- LU";
        ...
        juce::String tpStr = (tp > -60.0f) ? "TP " + juce::String(tp, 1) + " dBTP" : "TP --.- dBTP";
    ...
    void MasterMeterPanel::drawControlBar(juce::Graphics& g, juce::Rectangle<int> bounds)
    {
        ...
        g.setFont(interFont(MixCoachTheme::fontSizeTiny).boldened());
        g.setColour(MixCoachTheme::textDim());
        g.drawText(presetName + " \u25BC", presetHitArea_, juce::Justification::centred);

        // ─── 2. PEAK target ───────────────────────────────────────────────
        int peakW       = juce::jmin(72, w / 5);
        peakTargetArea_ = juce::Rectangle<int>(presetHitArea_.getRight() + 6, y0 + 3, peakW, h - 6);

        g.setFont(interFont(MixCoachTheme::fontSizeExtraTiny).boldened());
        g.setColour(MixCoachTheme::textMuted());
        g.drawText("Peak:", peakTargetArea_.reduced(4, 0), juce::Justification::centredLeft);

        g.setFont(interFont(MixCoachTheme::fontSizeTiny).boldened());
        g.setColour(MixCoachTheme::textDim());
        g.drawText(juce::String(peakTarget_, 1) + " dB", peakTargetArea_, juce::Justification::centredRight);

        // ─── 3. LOUDNESS target ───────────────────────────────────────────
        int loudW       = juce::jmin(82, w / 5);
        lufsTargetArea_ = juce::Rectangle<int>(peakTargetArea_.getRight() + 6, y0 + 3, loudW, h - 6);
        ...
        // ─── 4. GATE toggle (right) ──────────────────────────────────────
        int gateW    = 48;
        gateHitArea_ = juce::Rectangle<int>(x0 + w - gateW - 4, y0 + 3, gateW, h - 6);
    ...
    void MasterMeterPanel::mouseMove(const juce::MouseEvent& e)
    {
        ...
        setMouseCursor((overPreset || overGate) ? juce::MouseCursor::PointingHandCursor
