...
// ═══════════════════════════════════════════════════════════════════════════
class MixMapComponent : public juce::Component,
                         private juce::Timer {
public:
    MixMapComponent();
    ...
    void updateData(SlotRegistry& registry, SharedData& sharedData,
    ...
    void setSelectedSlot(int slotIndex) noexcept { selectedSlotIndex_ = slotIndex; repaint(); }
    ...
    [[nodiscard]] bool isMapComplete() const noexcept
    {
        for (const auto& group : busGroups_)
            if (group.bus == BusType::None && !group.tracks.empty())
                return false;
...
private:
    ...
    // ─── Render helpers ──────────────────────────────────────────────
    void drawBusHeader(juce::Graphics& g, const BusGroup& group,
                       juce::Rectangle<int> area, int index);
    void drawTrackRow(juce::Graphics& g, const TrackNode& node,
                      juce::Rectangle<int> area, bool even);
    void drawLevelBar(juce::Graphics& g, juce::Rectangle<int> area,
                       float levelDb) const;
    void drawFreqBar(juce::Graphics& g, juce::Rectangle<int> area,
                     const float regionEnergy[6]) const;
    void drawStereoBadge(juce::Graphics& g, juce::Rectangle<int> area,
                         StereoPos pos) const;
    void drawMasterSection(juce::Graphics& g, juce::Rectangle<int> area);
    void drawRoutingArrow(juce::Graphics& g, juce::Point<float> from,
