...
void MeterPanel::drawPeakMeterStereo(juce::Graphics& g, juce::Rectangle<int> bounds)
{
    ...
    auto adjustedScale = scaleArea.withY(lBarBounds.getY())
                                  .withHeight(lBarBounds.getHeight());
...
void MeterPanel::drawStatsPanel(juce::Graphics& g, juce::Rectangle<int> bounds)
{
    ...
    for (int i = 0; i < numModules; ++i) {
        ...
        drawStatModule(g, moduleArea, modules[i].title, modules[i].value,
                       modules[i].unit, modules[i].accent, modules[i].highlight);
    }
}
...
void MeterPanel::drawLufsMeterStereo(juce::Graphics& g, juce::Rectangle<int> bounds)
{
...

    {
        ...
        auto arrowBoundsL = juce::Rectangle<float>(
            lBarBounds.getRight() - 6.0f, targetY - 2.0f, 8.0f, 4.0f);
        auto arrowBoundsR = juce::Rectangle<float>(
            rBarBounds.getRight() - 6.0f, targetY - 2.0f, 8.0f, 4.0f);
        juce::Colour arrowCol = MixCoachTheme::accentCyan().withAlpha(0.60f);

        juce::Path arrowL;
        arrowL.addTriangle(arrowBoundsL.getTopLeft().x, arrowBoundsL.getCentreY(),
                          arrowBoundsL.getTopRight().x, arrowBoundsL.getY(),
                          arrowBoundsL.getTopRight().x, arrowBoundsL.getBottom());
        g.setColour(arrowCol);
        g.fillPath(arrowL);
        g.drawHorizontalLine((int)targetY, arrowBoundsL.getX(), arrowBoundsL.getRight());

        juce::Path arrowR;
        arrowR.addTriangle(arrowBoundsR.getTopLeft().x, arrowBoundsR.getCentreY(),
                          arrowBoundsR.getTopRight().x, arrowBoundsR.getY(),
                          arrowBoundsR.getTopRight().x, arrowBoundsR.getBottom());
        g.setColour(arrowCol);
        g.fillPath(arrowR);
...
void MeterPanel::drawPeakBar(juce::Graphics& g, juce::Rectangle<float> bounds,
                              float level, float peakHold)
{
    ...
    if (norm > 0.01f) {
        ...
        if (level > -6.0f) {
            ...
            if (yelTop < yelBot) {
                ...
                g.fillRect(juce::Rectangle<float>(fillBounds.getX(), yelTop,
                                                   fillBounds.getWidth(), yelBot - yelTop));
            }
        }
        ...
        if (level > 0.0f) {
            ...
            if (redTop < redBot) {
                g.setColour(colRed);
                g.fillRect(juce::Rectangle<float>(fillBounds.getX(), redTop,
                                                   fillBounds.getWidth(), redBot - redTop));
            }
        }
...
void MeterPanel::drawLufsBar(juce::Graphics& g, juce::Rectangle<float> bounds,
                              float level, float targetLUFS)
{
...
void MeterPanel::drawStatModule(juce::Graphics& g, juce::Rectangle<int> bounds,
                                 const juce::String& title, float value,
                                 ...
                                 bool highlight)
{
    ...
    juce::ColourGradient topGlow(
        juce::Colours::white.withAlpha(highlight ? 0.06f : 0.04f),
...
void MeterPanel::drawDbScale(juce::Graphics& g, juce::Rectangle<float> bounds,
                              float minDb, float maxDb, float stepDb,
                              float highlightVal, bool isLufsScale)
{
