...
void SpectrographComponent::rebuildStaticCache()
{
    ...
    cg.drawText(juce::CharPointer_UTF8("RTA"),
                juce::Rectangle<float>(layout_.plot.getX() + 4.0f,
                                       layout_.plot.getY() + 2.0f,
                                       28.0f,
                                       10.0f),
...
void SpectrographComponent::updateSpectrum(const float* data, int numBins)
{
    ...
    if (numBins > 0)
    {
        ...
        for (int i = 0; i < numBins; ++i)
        {
            ...
            const double meanSq = (prefixSq[static_cast<size_t>(highBin + 1)]
                                - prefixSq[static_cast<size_t>(lowBin)]) / count;
    ...
    for (int i = 0; i < kNumRtaBands; ++i)
    {
        ...
        if (pinkNoiseEnabled_)
            tiltCompensation = 3.0f * std::log2(band.centerHz / 1000.0f);
...
bool SpectrographComponent::smoothSpectrum(double sampleRateHz, bool allowRepaint)
{
    ...
    for (int i = 0; i < kNumRtaBands; ++i)
    {
        ...
        else
        {
            ...
            if (bandPeaks_[(size_t) i] != prevPeak)
                needsRepaint = true;
        }
    }
...
void SpectrographComponent::pushWaterfallSlice()
{
    ...
    for (int i = 0; i < kNumRtaBands; ++i)
        slice.levels[(size_t) i] = bandLevels_[(size_t) i].getCurrent();
    slice.valid = true;
...
void SpectrographComponent::addFrequencyMarker(float freqHz, const juce::String& label,
                                                float severity, bool isCritical,
                                                bool isPraise, float durationSecs)
{
    if (freqHz <= 0.0f || freqHz > kMaxFreq)
        return;
    ...
    for (auto& m : frequencyMarkers_) {
        float logRatio = (freqHz > 0.0f && m.frequencyHz > 0.0f)
            ? std::abs(std::log2(freqHz / m.frequencyHz)) : 999.0f;
        if (logRatio < 0.05f) {
            m = marker;
            return;
        }
...
void SpectrographComponent::pruneFrequencyMarkers()
{
    ...
    if (nowMs - lastMarkerPruneMs_ < 500)
        return;
...
void SpectrographComponent::computeDefaultReferenceCurve()
{
    ...
    for (int i = 0; i < kNumRtaBands; ++i)
    {
        ...
        db += p.bassBoost * std::exp(-0.5f * std::pow(
            std::log2(freq / p.bassFreq) / p.bassWidth, 2.0f));

        db += p.scoopDepth * std::exp(-0.5f * std::pow(
            std::log2(freq / p.scoopFreq) / p.scoopWidth, 2.0f));

        db += p.presenceBoost * std::exp(-0.5f * std::pow(
            std::log2(freq / p.presenceFreq) / p.presenceWidth, 2.0f));

        if (freq > p.highRolloff)
            db += p.highRolloffDb * (freq - p.highRolloff) / (20000.0f - p.highRolloff);

        if (freq < p.subRolloff)
            db += p.subRolloffDb * (p.subRolloff - freq) / p.subRolloff;
