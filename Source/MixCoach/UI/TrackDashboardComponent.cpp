...
namespace mixcoach {
    ...
    void TrackDashboardComponent::updateDashboard(SlotRegistry& registry, SharedData& sharedData)
    {
        ...
        registry.forEachActive([&](const SlotInfo& info) {
            ...
            if (peak > -0.5f) master_.clippingTracks++;
            else if (peak > -6.0f)
                master_.warningTracks++;
            if (peak > -60.0f) master_.signalTracks++;

            // Agrupar por bus
            int busIdx = static_cast<int>(info.bus);
            if (busIdx < 0 || busIdx >= kNumBuses) busIdx = kNumBuses; // UNASSIGNED
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    void TrackDashboardComponent::drawBusCard(juce::Graphics& g, juce::Rectangle<int> bounds, const BusSummary& bus)
    {
        ...
        juce::Colour busColour = (bus.busIndex >= 0 && bus.busIndex < kNumBuses) ? getBusColour(bus.busIndex)
        ...
        if (peakNorm > 0.01f) {
            ...
            juce::Colour fillCol;
            if (bus.avgPeakDb > -6.0f) fillCol = MixCoachTheme::meterRed();
