...
namespace mixcoach {
    ...
    {
        headerLabel_.setText(juce::CharPointer_UTF8("\xF0\x9F\x93\x8A VU Meters"), juce::dontSendNotification);
        ...
        for (auto& m : meters_) m.level.reset(-60.0f);
    }
    ...
    {
        if (headerLabel_.isVisible()) headerLabel_.setBounds(getLocalBounds().reduced(4, 2).removeFromTop(18));
    }
    ...
    void VUMetersPanel::paint(juce::Graphics& g)
    {
        ...
        for (int i = 0; i < 4; ++i) {
            int col   = i % 2;
            int row   = i / 2;
            auto cell = juce::Rectangle<int>(x0 + col * (w + gap), y0 + row * (h + gap), w, h);
            drawMeter(g, cell, i, meters_[i].level.getCurrent(), meters_[i].peakHold);
        }
    }

    void VUMetersPanel::drawMeter(juce::Graphics& g, juce::Rectangle<int> bounds, int idx, float levelDb, float peakDb)
    {
        ...
        // ─── Peak triangle ─────────────────────────────────────────────────
        if (peakDb > -59.0f)
            VerticalGradientMeter::drawPeakTriangle(g, scaleArea, peakDb, MixCoachTheme::meterOrange());

        // ─── Peak readout (debajo de la barra, sobre fondo oscuro) ─────────
        auto readoutBg = juce::Rectangle<float>(b.getX(), b.getBottom() + 1.0f, b.getWidth(), 11.0f);
        g.setColour(MixCoachTheme::bgDarker().withAlpha(0.6f));
        g.fillRoundedRectangle(readoutBg, 2.0f);
        juce::String val = (peakDb > -59.0f) ? juce::String(peakDb, 1) + " dB" : "--.- dB";
