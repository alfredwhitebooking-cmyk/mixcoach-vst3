...
namespace mixcoach {
    ...
    AudioFileInfo ReferencePanelComponent::readAudioFileInfo(const juce::File& file)
    {
        ...
        try {
            ...
            if (reader->sampleRate > 0.0) info.durationSeconds = reader->lengthInSamples / reader->sampleRate;
            else
    ...
    {
        // ─── Drop zone unificada ───────────────────────────────────────────────
        dropZone_.onFileDropped = [this](const juce::String& path) { addFileReference(path); };
        dropZone_.onURLAdded    = [this](const juce::String& url) { addURLReference(url); };
        ...
        // ─── Empty state ───────────────────────────────────────────────────────
        emptyLabel_.setText("Carga archivos de audio o pega enlaces de canciones", juce::dontSendNotification);
    ...
    void ReferencePanelComponent::switchTab(ActiveTab tab)
    {
        ...
        if (tab == ActiveTab::Notes) {
            ...
            emptyLabel_.setText(
                references_.empty() ? "Carga archivos de audio o pega enlaces de canciones — la IA usa ambos" : "",
    ...
    juce::String ReferencePanelComponent::parseHTMLTitle(const juce::String& html, const juce::String& url)
    {
        ...
        if (titleStart >= 0 && titleEnd > titleStart) {
            ...
            title = title.replace(" - YouTube", "").replace(" | Spotify", "").replace(" - Spotify", "");
        }
        ...
        if (title.isEmpty()) {
            ...
            if (ogIdx >= 0) {
                ...
                if (localPos >= 0) {
                    ...
                    while (contentStart < html.length() && html[contentStart] != '\"' && html[contentStart] != '\''
                           && html[contentStart] != '>')
                        ++contentStart;
    ...
    juce::String ReferencePanelComponent::extractSoundCloudName(const juce::String& url)
    {
        ...
        if (afterDomain.endsWithChar('/')) afterDomain = afterDomain.dropLastCharacters(1);

        auto parts = juce::StringArray::fromTokens(afterDomain, "/", "");
        if (parts.size() >= 2) {
            juce::String trackName;
            if (parts[0].equalsIgnoreCase("sets") && parts.size() >= 2) trackName = parts[parts.size() - 1];
            else
            ...
            if (result.isNotEmpty() && result.length() > 3) return result;
        }
    ...
    juce::String ReferencePanelComponent::fetchURLTitle(const juce::String& url, int timeoutMs)
    {
        try {
            ...
            auto stream =
                pageUrl.createInputStream(juce::URL::InputStreamOptions(juce::URL::ParameterHandling::inAddress)
    ...
    void ReferencePanelComponent::addFileReference(const juce::String& filePath)
    {
        ...
        if (onFileReferenceAdded) onFileReferenceAdded(filePath);
    }
    ...
    void ReferencePanelComponent::updateReferenceName(int refIndex, const juce::String& newName)
    {
        if (refIndex >= 0 && refIndex < (int)references_.size()) {
            ...
            refreshDisplay();
            if (onReferencesChanged) onReferencesChanged();
            if (onURLReferenceAdded) onURLReferenceAdded(newName, references_[refIndex].path);
        }
    }
    ...
    void ReferencePanelComponent::resized()
    {
        ...
        if (topPad > 0 && totalToDraw == numRefs) area.removeFromTop(topPad);
    ...
    void ReferencePanelComponent::paint(juce::Graphics& g)
    {
        ...
        // ─── Tab buttons ──────────────────────────────────────────────────────
        auto drawTab = [&](juce::Rectangle<int> tabBounds, bool isActive, const juce::String& label, int count) {
            ...
            if (isActive) {
                ...
                auto glowArea =
                    juce::Rectangle<float>((float)(cx - glowW * 0.5f), (float)(tabBounds.getBottom() - 2), glowW, 3.0f);
                juce::ColourGradient glow(MixCoachTheme::accent().withAlpha(0.35f),
                                          (float)cx,
                                          glowArea.getY(),
                ...
                g.drawHorizontalLine(tabBounds.getBottom() - 1, (float)(cx - bw * 0.25f), (float)(cx + bw * 0.25f));
            }

            if (!isActive && count > 0) {
                auto badgeBounds = tabBounds.translated(tabBounds.getWidth() + 2, 0).withWidth(14).reduced(0, 5);
        ...
        // ─── Reference-Driven Mode toggle ─────────────────────────────────────
        {
            ...
            // Background pill
            g.setColour(referenceDrivenMode_ ? MixCoachTheme::accentCyan().withAlpha(0.15f)
                                             : MixCoachTheme::bgDarker().withAlpha(0.30f));
            g.fillRoundedRectangle(toggle, 8.0f);

            // Border
            g.setColour(referenceDrivenMode_ ? MixCoachTheme::accentCyan().withAlpha(0.50f)
            ...
            if (referenceDrivenMode_) {
                ...
                juce::ColourGradient glow(MixCoachTheme::accentCyan().withAlpha(0.0f),
                                          glowArea.getCentreX(),
                                          glowArea.getY(),
                                          MixCoachTheme::accentCyan().withAlpha(0.0f),
                                          glowArea.getCentreX(),
                                          glowArea.getBottom(),
                                          false);
            ...
            // Label
            g.setFont(juce::Font(juce::FontOptions(MixCoachTheme::fontSizeNano)).boldened());
            g.setColour(referenceDrivenMode_ ? MixCoachTheme::accentCyan()
            ...
            float knobX      = referenceDrivenMode_ ? toggle.getRight() - knobRadius - 4.0f
                                                    : toggle.getX() + knobRadius + 4.0f;
            float knobY      = toggle.getCentreY();

            juce::Colour knobCol = referenceDrivenMode_ ? MixCoachTheme::accentCyan()
        ...
        if (referenceDrivenMode_ && refProgressBounds_.getWidth() > 0) {
            ...
            if (fill > 0.01f) {
                ...
                if (fill >= 0.80f) progressCol = MixCoachTheme::success();
                else if (fill >= 0.50f)
                    progressCol = MixCoachTheme::warning();
                else
                    progressCol = MixCoachTheme::error();

                juce::ColourGradient fillGrad(progressCol.brighter(0.2f).withAlpha(0.7f),
                                              fillRect.getX(),
                                              fillRect.getY(),
        ...
        if (isDraggingRow_ && dropTargetVisualIdx_ >= 0) {
            ...
            if (dropTargetVisualIdx_ <= totalV) {
                if (dropTargetVisualIdx_ == 0) drawDropIndicator(g, visibleRowBounds_[0], true);
        ...
        if (!sections_.empty() && !isDraggingRow_) {
            ...
            if (selArea.getWidth() > 40) drawSectionSelector(g, selArea);
        }
    }
    ...
    void ReferencePanelComponent::drawReferenceRow(
        juce::Graphics& g, juce::Rectangle<int> bounds, const MixReference& ref, int index, int visualIndex)
    {
        ...
        auto dotArea = row.removeFromRight(14).withSizeKeepingCentre(6.0f, 6.0f);
        {
            ...
            switch (ref.analysisStatus) {
                case AnalysisStatus::Pending:
                    dotCol = MixCoachTheme::textMuted().withAlpha(0.4f);
                    break;
        ...
        if (isAudio) {
            ...
            if (isPlaying) btnColour = MixCoachTheme::accentCyan();
                ...
                btnColour = MixCoachTheme::accentGlow();
            else
                btnColour = MixCoachTheme::textDim().withAlpha(0.45f);
            ...
            drawLinkIcon(g,
                         ...
                         iconArea.getCentreY(),
                         iconArea.getWidth(),
                         ref.colour.withAlpha(isPlaying ? 0.9f : 0.5f));
        }
        ...
        if (isAudio) {
            if (ref.audioInfo.valid) {
                ...
                metaText = srStr + " kHz \xE2\x80\xA2 " + juce::String(ref.audioInfo.bitDepth) + "-bit \xE2\x80\xA2 "
                           + durationStr;
            }
            else {
            ...
            if (ref.path.contains("youtube.com") || ref.path.contains("youtu.be")) metaText = "Enlace de YouTube";
        ...
        auto nameBounds =
            juce::Rectangle<float>(textBounds.getX(), textTopY, textBounds.getWidth(), nameH).toNearestInt();
        auto metaBounds =
            juce::Rectangle<float>(textBounds.getX(), textTopY + nameH + textGap, textBounds.getWidth(), metaH)
                .toNearestInt();
    ...
    void ReferencePanelComponent::mouseDown(const juce::MouseEvent& e)
    {
        ...
        if (refsTabBounds_.contains(pos)) {
            switchTab(ActiveTab::References);
            return;
        }
        if (notesTabBounds_.contains(pos)) {
            switchTab(ActiveTab::Notes);
            return;
        }
        ...
        if (refModeToggleBounds_.contains(pos)) {
            ...
            if (e.mods.isRightButtonDown()) {
                ...
                menu.showMenuAsync(juce::PopupMenu::Options().withTargetScreenArea(refModeToggleBounds_),
                                   [this](int result) {
                                       if (onRefModeMenuAction) {
                                           if (result == 1) onRefModeMenuAction(RefModeMenuAction::ConfigureGaps);
                                           else if (result == 2)
                                               onRefModeMenuAction(RefModeMenuAction::ViewHistory);
                                           else if (result == 3)
                                               onRefModeMenuAction(RefModeMenuAction::ResetProgress);
                                       }
                                   });
                return;
            }
            ...
            if (onReferenceDrivenModeToggled) onReferenceDrivenModeToggled(referenceDrivenMode_);
            return;
        }
        ...
        if (!sections_.empty()) {
            for (size_t si = 0; si < sectionBtnBounds_.size(); ++si) {
                if (sectionBtnBounds_[si].contains(pos)) {
                    if (onSectionSelected) onSectionSelected(static_cast<int>(si) - 1);
                    if (onSectionSeekTo && si < sections_.size()) onSectionSeekTo(sections_[si].startSeconds);
                    return;
                }
            }
        }
        ...
        for (int i = (int)references_.size() - 1; i >= 0 && drawn < (int)visibleRowBounds_.size(); --i, ++drawn) {
            ...
            if (!rowBounds.contains(pos)) continue;

            auto& ref = references_[i];

            if (ref.type == MixReference::Type::File && drawn < (int)playBtnBounds_.size()
            ...
            if (drawn < (int)deleteBtnBounds_.size() && deleteBtnBounds_[drawn].contains(pos)) {
                removeReference(i);
                return;
            }
    ...
    void ReferencePanelComponent::mouseDrag(const juce::MouseEvent& e)
    {
        ...
        if (dragSourceVisualIdx_ < 0 || dragSourceArrayIdx_ < 0) return;
    ...
    void ReferencePanelComponent::mouseUp(const juce::MouseEvent& e)
    {
        ...
        if (isDraggingRow_) {
            if (dropTargetVisualIdx_ >= 0 && dragSourceVisualIdx_ >= 0 && dragSourceArrayIdx_ >= 0
        ...
        if (dragSourceVisualIdx_ < 0) return;
        ...
        if (ref.type == MixReference::Type::File && drawn < (int)playBtnBounds_.size()
            ...
            if (matchRefIndex_ == i) {
                matchRefIndex_ = -1;
                if (onReferenceSelected) onReferenceSelected(-1);
            }
            else {
                matchRefIndex_ = i;
                if (onReferenceSelected) onReferenceSelected(i);
            }
            repaint();
        }
    ...
    void ReferencePanelComponent::mouseMove(const juce::MouseEvent& e)
    {
        ...
        bool overInteractive = refsTabBounds_.contains(pos) || notesTabBounds_.contains(pos);

        if (!overInteractive) {
            for (size_t i = 0; i < playBtnBounds_.size(); ++i) {
                if (playBtnBounds_[i].contains(pos)) {
        ...
        if (!overInteractive) {
            for (size_t i = 0; i < deleteBtnBounds_.size(); ++i) {
                if (deleteBtnBounds_[i].contains(pos)) {
                    overInteractive = true;
                    break;
                }
            }
        ...
        if (!overInteractive && activeTab_ == ActiveTab::References) {
            for (size_t i = 0; i < visibleRowBounds_.size(); ++i) {
                if (visibleRowBounds_[i].contains(pos)) {
                    overInteractive = true;
                    break;
                }
            }
        ...
        if (!overInteractive && activeTab_ == ActiveTab::References) {
            for (size_t i = 0; i < sectionBtnBounds_.size(); ++i) {
                if (sectionBtnBounds_[i].contains(pos)) {
                    overInteractive = true;
                    break;
                }
            }
        ...
        if (nowHovering != wasHovering_ || (!sections_.empty() && activeTab_ == ActiveTab::References)) {
            ...
            if (!hoverChanged && !sectionBtnBounds_.empty()) {
                ...
                for (size_t i = 0; i < sectionBtnBounds_.size(); ++i) {
                    if (sectionBtnBounds_[i].contains(mouseDownPos_)) {
                        wasInSection = true;
                    }
                    if (sectionBtnBounds_[i].contains(pos)) {
                        nowInSection = true;
            ...
            if (hoverChanged) repaint();
        }

        wasHovering_ = nowHovering;

        setMouseCursor(overInteractive ? juce::MouseCursor::PointingHandCursor : juce::MouseCursor::NormalCursor);
    }
    ...
    void ReferencePanelComponent::drawDropIndicator(juce::Graphics& g, juce::Rectangle<int> rowBounds, bool above)
    {
        float y     = above ? (float)(rowBounds.getY() - 1) : (float)(rowBounds.getBottom() + 1);
        ...
        juce::ColourGradient glow(
            MixCoachTheme::accent().withAlpha(0.0f), left, y, MixCoachTheme::accent().withAlpha(0.0f), right, y, false);
        ...
        arrow.addTriangle(cx - 4.0f, aY - dir * 3.0f, cx, aY + dir * 4.0f, cx + 4.0f, aY - dir * 3.0f);
        g.fillPath(arrow);
    }
        ...
        for (int i = 0; i < numSections; ++i) {
            ...
            if (isActive) {
                ...
                auto underline = juce::Rectangle<float>(
                    btnFloat.getX() + 4.0f, btnFloat.getBottom() - 2.0f, btnFloat.getWidth() - 8.0f, 2.0f);
    ...
    std::vector<juce::String> ReferencePanelComponent::getFilePaths() const
    {
        ...
        for (const auto& ref : references_)
            if (ref.type == MixReference::Type::File) paths.push_back(ref.path);
        return paths;
    }
    ...
    std::vector<juce::String> ReferencePanelComponent::getURLs() const
    {
        ...
        for (const auto& ref : references_)
            if (ref.type == MixReference::Type::URL) urls.push_back(ref.path);
        return urls;
    }
