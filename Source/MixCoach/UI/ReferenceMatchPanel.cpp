...
void ReferenceMatchPanel::timerCallback()
{
    ...
    {
        if (mixSmooth_[r].advance())
            needsRepaint = true;
        if (refSmooth_[r].advance())
            needsRepaint = true;
    }
    if (lufsSmooth_.advance())
        needsRepaint = true;
    if (scoreSmooth_.advance())
        needsRepaint = true;
...
void ReferenceMatchPanel::paint(juce::Graphics& g)
{
    ...
    // ─── Header: nombre de referencia + match score ───────────────────
    {
        ...
        g.drawText("Matching: " + data_.referenceName, header.withTrimmedRight(54),
...
void ReferenceMatchPanel::drawRegionBars(juce::Graphics& g, juce::Rectangle<int> bounds)
{
    ...
    for (int b = 0; b < numBars; ++b)
    {
        ...
        juce::ColourGradient mixGrad(
            kRegionColours[b].brighter(0.3f).withAlpha(0.9f),
            mixRect.getCentreX(), mixRect.getY(),
            kRegionColours[b].withAlpha(0.75f),
            mixRect.getCentreX(), mixRect.getBottom(),
            false);
        ...
        if (refH > 14)
        {
            ...
            g.drawText("R", refRect.reduced(1, 1).removeFromTop(10).removeFromRight(10),
        ...
        if (mixH > 14)
        {
            ...
            g.drawText("M", mixRect.reduced(1, 1).removeFromTop(10).removeFromLeft(10),
        ...
        // ─── Label below column ─────────────────────────────────────────
        auto labelBounds = juce::Rectangle<int>(colX, bounds.getBottom() - labelH,
                                                colW, labelH);
        ...
        if (data_.valid)
        {
            ...
            if (std::abs(delta) < 1.0f)
                diffCol = MixCoachTheme::success().withAlpha(0.5f);
...
void ReferenceMatchPanel::drawDeltaRow(juce::Graphics& g, juce::Rectangle<int> bounds)
{
    ...
    for (int r = 0; r < kNumRegions; ++r)
    {
        ...
        if (fillW > 0)
        {
            ...
            if (absDelta < 1.5f)
                deltaCol = MixCoachTheme::success().withAlpha(0.6f);
            ...
            int fillX;
            if (delta > 0)
                fillX = x + colW / 2; // Ref louder → grow right
            else
...
void ReferenceMatchPanel::drawLUFSMeter(juce::Graphics& g, juce::Rectangle<int> bounds)
{
    ...
    if (fillW > 0) {
        auto fillCol = (lufsDiff > 0.0f)
            ? MixCoachTheme::warning()   // mix louder → yellow
...
void ReferenceMatchPanel::drawCrestMeter(juce::Graphics& g, juce::Rectangle<int> bounds)
{
    ...
    juce::ColourGradient mixGrad(
        mixCol.brighter(0.2f).withAlpha(0.85f), mixRect.getCentreX(), mixRect.getY(),
        mixCol.withAlpha(0.65f), mixRect.getCentreX(), mixRect.getBottom(), false);
    g.setGradientFill(mixGrad);
    g.fillRoundedRectangle(mixRect, 2.0f);
...
void ReferenceMatchPanel::drawCorrelationMeter(juce::Graphics& g, juce::Rectangle<int> bounds)
{
    ...
    if (!isMatch)
    {
        ...
        if (fillW > 0)
        {
            auto fillCol = (corrDiff > 0.0f)
                ? MixCoachTheme::warning()   // mix mas mono → yellow
    ...
    g.drawText(corrText, bounds.removeFromTop(infoH).reduced(2, 0),
...
void ReferenceMatchPanel::drawMetricsRow(juce::Graphics& g, juce::Rectangle<int> bounds)
{
    // ─── LUFS Section ──────────────────────────────────────────────────
    {
        g.setFont(juce::Font(juce::FontOptions(MixCoachTheme::fontSizeExtraTiny)).boldened());
        g.setColour(MixCoachTheme::textMuted());
        g.drawText("LUFS", bounds.removeFromTop(12).reduced(2, 0),
                   juce::Justification::centredLeft);
        ...
        g.setColour(deltaCol);
        juce::String lufsText = juce::String(lufsMix, 1) + " vs "
                              + juce::String(lufsRef, 1) + " LUFS  "
                              + arrow + juce::String(std::abs(lufsDelta), 1);
        g.drawText(lufsText, lufsArea.removeFromTop(10).reduced(2, 0),
                   juce::Justification::centredRight);
    }
    ...
    // ─── Crest Section ─────────────────────────────────────────────────
    {
        g.setFont(juce::Font(juce::FontOptions(MixCoachTheme::fontSizeExtraTiny)).boldened());
        g.setColour(MixCoachTheme::textMuted());
        g.drawText("Crest", bounds.removeFromTop(12).reduced(2, 0),
                   juce::Justification::centredLeft);
    ...
    // ─── Correlation Section ───────────────────────────────────────────
    {
        g.setFont(juce::Font(juce::FontOptions(MixCoachTheme::fontSizeExtraTiny)).boldened());
        g.setColour(MixCoachTheme::textMuted());
        g.drawText("Corr", bounds.removeFromTop(12).reduced(2, 0),
                   juce::Justification::centredLeft);
