...
{
    switch (bus) {
        case BusType::Drums:    return "\xF0\x9F\xA5\x81";
        case BusType::Bass:     return "\xF0\x9F\x8E\xB8";
        case BusType::Guitars:  return "\xF0\x9F\x8E\xB8";
        case BusType::Keys:     return "\xF0\x9F\x8E\xB9";
        case BusType::Vocals:   return "\xF0\x9F\x8E\xA4";
        case BusType::FX:       return "\xF0\x9F\x8E\x9B";
        case BusType::Melody:   return "\xF0\x9F\x8E\xB5";
        default:                return "\xF0\x9F\x93\x8D";
    }
}
                                         ...
                                         const SlotInfo& info)
{
    ...
    juce::String busEmoji = (info.bus != BusType::None)
        ? getBusEmoji(info.bus) + " " : juce::String();
    ...
    if (peakDb > -0.5f) {
        ...
        result.text = busEmoji + "\xF0\x9F\x94\xB4 Recortando a " + juce::String(peakDb, 1)
                      + "dB! Baja " + juce::String(reduceBy, 1) + " dB YA";
        result.status = SuggestionStatus::Red;
        return result;
    }
    if (peakDb > -3.0f) {
        float reduceBy = peakDb + 6.0f;
        result.text = busEmoji + "\xF0\x9F\x9F\xA1 Pico alto " + juce::String(peakDb, 1)
                      + "dB \xE2\x80\x94 baja " + juce::String(reduceBy, 1) + " dB (target: -6dB)";
    ...
    if (rmsDb > -18.0f) {
        ...
        result.text = busEmoji + "\xF0\x9F\x9F\xA2 RMS " + juce::String(rmsDb, 1)
                      + "dB \xE2\x80\x94 sube " + juce::String(boostBy, 1) + " dB (target: -8dB)";
        result.status = SuggestionStatus::Green;
        return result;
    }
...
void MessengerListComponent::updateCoachAdvice(CoachEngine& coach)
{
    for (int idx = 0; idx < SlotRegistry::kMaxSlots; ++idx)
    {
        ...
        switch (bus) {
            case BusType::Drums:    entry.busTargetPeak = -8.0f;  break;
            case BusType::Bass:     entry.busTargetPeak = -10.0f; break;
            case BusType::Guitars:  entry.busTargetPeak = -8.0f;  break;
            case BusType::Keys:     entry.busTargetPeak = -10.0f; break;
            case BusType::Vocals:   entry.busTargetPeak = -6.0f;  break;
            case BusType::FX:       entry.busTargetPeak = -14.0f; break;
            case BusType::Melody:   entry.busTargetPeak = -8.0f;  break;
            default:                entry.busTargetPeak = -8.0f;  break;
        }
        ...
        if (rec != nullptr && rec->slotIndex == idx)
        {
            ...
            juce::String emoji = (entry.info.bus != BusType::None)
                ? getBusEmoji(entry.info.bus) + " " : juce::String();
            ...
            switch (rec->status) {
                case TrackRecommendation::Status::Pending:
                    ...
                    entry.coachAdviceText = emoji + (rec->delta < -3.0f ? "\xF0\x9F\x94\xB4 " : "\xF0\x9F\x9F\xA1 ")
                        + direction + " " + juce::String(delta, 1) + " dB (PK " + juce::String(rec->beforeValue, 1) + "dB)";
                    break;
                case TrackRecommendation::Status::Applied:
                    entry.coachAdviceStatus = SuggestionStatus::Green;
                    entry.coachAdviceText = emoji + "\xE2\x9C\x85 Corregido! "
                        + juce::String(rec->beforeValue, 1) + " \xE2\x86\x92 " + juce::String(rec->expectedAfter, 1) + " dB";
                    break;
        ...
        else if (coachEngine_ != nullptr && entry.trackRole != TrackRole::Unknown)
        {
            ...
            const bool clippingRisk = ts.hasSignal()
                && (ts.health == TrackHealth::ClippingRisk
                    || gainAdv.currentPeak > -0.5f);

            if (clippingRisk) {
                entry.coachAdviceStatus = SuggestionStatus::Red;
                float pk = std::max(gainAdv.currentPeak, ts.getPeakCombinedDb());
                entry.coachAdviceText = emoji + "\xF0\x9F\x94\xB4 Pico "
                    + juce::String(pk, 1) + "dB \xE2\x80\x94 baja YA";
            }
                ...
                juce::String sevEmoji = off ? juce::String("\xF0\x9F\x94\xB4 ")
                                            : juce::String("\xF0\x9F\x9F\xA1 ");
                juce::String arrow = (gainAdv.suggestedDeltaDb > 0) ? "Sube" : "Baja";
                juce::String txt = emoji + sevEmoji + gainAdv.trackName + " \xE2\x80\x94 "
                    + arrow + " " + juce::String(std::abs(gainAdv.suggestedDeltaDb), 1)
                    + " dB (actual " + juce::String(gainAdv.currentPeak, 1)
                    + " / target " + juce::String(gainAdv.peakTarget, 1) + ")";
                ...
                juce::String sevEmoji = off ? juce::String("\xF0\x9F\x94\xB4 ")
                                            : juce::String("\xF0\x9F\x9F\xA1 ");
                juce::String kind = dynAdv.isOvercompressed()
                    ? juce::String("Sobre-comprimido")
                    : juce::String("Muy din\xC3\xA1mico");
                juce::String detail = kind + " (crest " + juce::String(dynAdv.currentCrest, 1)
                    + " vs " + juce::String(dynAdv.crestTarget, 1) + "). "
                    + dynAdv.suggestedAction;
                entry.coachAdviceText = emoji + sevEmoji + dynAdv.trackName
                    + " \xE2\x80\x94 " + detail;
            }
                ...
                juce::String what = tonalAdv.hasExcess() ? juce::String("Exceso en ")
                                                         : juce::String("Falta en ");
                juce::String detail = what + regionName + " (" + regionFreq + "). " + action;
                entry.coachAdviceText = emoji + sevEmoji + tonalAdv.trackName
                    + " \xE2\x80\x94 " + detail;
            }
            else {
                // Las 3 dimensiones OnTarget → pista saludable con target confirmado
                entry.coachAdviceStatus = SuggestionStatus::Green;
                entry.coachAdviceText = emoji + "\xF0\x9F\x9F\xA2 " + gainAdv.trackName
                    + " en target (" + juce::String(gainAdv.peakTarget, 1) + " dB)";
            }
        }
            ...
            if (ts.hasSignal() && ts.health != TrackHealth::Unknown)
            {
                ...
                switch (ts.health) {
                    case TrackHealth::ClippingRisk:
                        entry.coachAdviceStatus = SuggestionStatus::Red;
                        entry.coachAdviceText = emoji + "\xF0\x9F\x94\xB4 Pico " + juce::String(pk,1) + "dB \xE2\x80\x94 baja YA";
                        break;
                    case TrackHealth::Overcompressed:
                        entry.coachAdviceStatus = SuggestionStatus::Red;
                        entry.coachAdviceText = emoji + "\xF0\x9F\x94\xB4 Crest " + juce::String(ts.crestFactor,1) + "dB";
                        break;
                    ...
                    case TrackHealth::NeedsCompression:
                        entry.coachAdviceStatus = SuggestionStatus::Yellow;
                        entry.coachAdviceText = emoji + "\xF0\x9F\x9F\xA1 Crest " + juce::String(ts.crestFactor,1) + "dB";
                        break;
                    case TrackHealth::Clean:
                        entry.coachAdviceStatus = SuggestionStatus::Green;
                        entry.coachAdviceText = emoji + "\xF0\x9F\x9F\xA2 " + juce::String(ts.rmsCombined,1) + "dB";
                        break;
