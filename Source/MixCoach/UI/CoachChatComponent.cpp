...
namespace mixcoach {
    ...
    float ChatMessagesComponent::getBubbleHeight(const ChatBubble& bubble, float maxWidth) const
    {
        ...
        if (bubble.isSystem) {
            ...
            if (bubble.text.contains("**")) sysTextH *= 1.15f;
    ...
    void MixCoachPanel::resized()
    {
        ...
        if (showSuggestions) {
            ...
            for (auto& sug : suggestions_) {
                float textW =
                    juce::GlyphArrangement::getStringWidthInt(juce::Font(juce::FontOptions(8.0f)).boldened(), sug)
                    + 16.0f;
    ...
    void MixCoachPanel::paint(juce::Graphics& g)
    {
        ...
        JUCE_TRY
        {
            ...
            for (size_t i = 0; i < suggestionChipBounds_.size(); ++i) {
                ...
                juce::ColourGradient chipGrad(MixCoachTheme::accent().withAlpha(isHovered ? 0.18f : 0.08f),
                                              chipRectF.getX(),
                                              chipRectF.getY(),
                                              ...
                                              chipRectF.getBottom(),
                                              false);
                if (isHovered) chipGrad.addColour(0.5f, MixCoachTheme::accent().withAlpha(0.14f));
                ...
                if (isHovered) {
                    ...
                    juce::ColourGradient chipGlassGrad(juce::Colours::white.withAlpha(0.08f),
                                                       chipGlass.getX(),
                                                       chipGlass.getY(),
                                                       juce::Colour(0x00000000),
                                                       chipGlass.getX(),
            ...
            {
                systemToggleBounds_ = {chatViewport_.getX(),
                ...
                if (numSystem > 0 && systemToggleBounds_.getHeight() > 4) {
                    ...
                    if (showSystemMessages_) {
                        ...
                        juce::String label =
                            juce::String(juce::CharPointer_UTF8("\xF0\x9F\x9B\xA0 ")) + juce::String(numSystem)
                            + (numSystem == 1 ? " hidden event \xE2\x80\xA2 Show" : " hidden events \xE2\x80\xA2 Show");
            ...
            g.setColour(MixCoachTheme::textMuted().withAlpha(0.6f));
            g.drawText("GROUP BY:", innerArea.getX() + 2, toolbarY + 1, 56, 16, juce::Justification::centredLeft);
    ...
    {
        if (connected) {
            ollamaStatusLabel_.setText("\xF0\x9F\xA4\x96 " + modelName + ": \xE2\x9C\x93 connected",
                                       juce::dontSendNotification);
            ollamaStatusLabel_.setColour(juce::Label::textColourId, MixCoachTheme::info());
            ollamaRetryBtn_.setVisible(false);
        }
        else {
            ollamaStatusLabel_.setText("\xF0\x9F\xA4\x96 " + modelName + ": \xE2\x9C\x97 disconnected",
                                       juce::dontSendNotification);
