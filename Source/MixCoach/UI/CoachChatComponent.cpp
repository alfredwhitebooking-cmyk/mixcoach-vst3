...
void MixCoachPanel::resized()
{
        ...
        if (isPreFullUI(coachRoomState_)) {
            ...
            if (showSuggestions) {
                ...
                // ═══ FIX: setShowModeCards usa QuickReplyBar, NO ModeSelectionCard ═══
                // Antes revisaba modeCardMix_.isVisible() que siempre era false.
                bool modeCardsActive = (coachRoomState_ == CoachRoomState::Intention
                    && (modeCardMix_.isVisible() || quickReplyBar_.isVisible()));
                if (modeCardsActive) {
    ...
    void MixCoachPanel::performSprint1Layout()
    {
        ...
        if (quickReplyBar_.isVisible()) {
            ...
            if (isSetup) {
                ...
                int qrbX = 4;
                // ═══ FIX: Clampear posición Y para evitar que se salga de la pantalla ═══
                // Si el panel es pequeño (< 80px), la barra se posiciona en la parte inferior.
                int qrbY = juce::jmax(4, getHeight() - 80);
