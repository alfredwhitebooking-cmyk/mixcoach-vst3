...
static float pulseAlpha(float speedHz, float phaseOffset) noexcept
{
    ...
    return 0.5f + 0.5f * std::sin(t * juce::MathConstants<float>::twoPi * speedHz
...
void MessengerListComponent::paint(juce::Graphics& g)
{
    ...
    // ═══ Health summary bar ═══════════════════════════════════════════════
    {
        ...
        for (int idx = 0; idx < SlotRegistry::kMaxSlots; ++idx)
        {
            ...
            if (!messengers_[idx].hasSignal) { silent++; continue; }
        ...
        if (clean + warn + crit + silent > 0)
        {
            ...
            auto drawPill = [&](int count, juce::Colour colour, const char* label, int& xOff)
            {
                ...
                int w = juce::GlyphArrangement::getStringWidthInt(
                        interFont(9.0f).boldened(), text) + 14;
                auto pill = juce::Rectangle<int>(xOff, healthBar.getY() + MixCoachTheme::spacingSM / 2, w, healthBar.getHeight() - MixCoachTheme::spacingSM);
            ...
            if (coachEngine_ != nullptr)
            {
                ...
                if (prog.totalActive > 0 && prog.identified > 0)
                {
                    ...
                    auto idCol = allConfirmed
                        ? MixCoachTheme::success()
                        : MixCoachTheme::accentCyan();
                    juce::String idText = "\xF0\x9F\x8E\xAF " // 🎯
                        + juce::String(prog.confirmed) + "/"
                        + juce::String(prog.identified);
                    int idW = juce::GlyphArrangement::getStringWidthInt(
                            interFont(9.0f).boldened(), idText) + 14;
                    auto idPill = juce::Rectangle<int>(
                        xOff, healthBar.getY() + 3,
                        idW, healthBar.getHeight() - 6);
    ...
    // ═══ SPRINT 5: TrackFeed Banner (top events) ═══════════════════════════
    {
        ...
        // Colapso/expande button
        {
            ...
            g.drawFittedText(bannerCollapsed_ ? "▶" : "▼",
                             bannerCollapseBtn_, juce::Justification::centred, 1);
        }

        // TRACKFEED label
        {
            int labelX = bannerCollapseBtn_.getRight() + MixCoachTheme::spacingXS;
            auto labelArea = juce::Rectangle<int>(labelX, bannerArea.getY() + MixCoachTheme::spacingSM / 2, 70, bannerArea.getHeight() - MixCoachTheme::spacingSM);
            g.setFont(interFont(9.0f).boldened());
        ...
        if (!bannerCollapsed_ && !topEvents_.empty())
        {
            ...
            for (int ei = 0; ei < juce::jmin(maxShow, (int)topEvents_.size()); ++ei)
            {
                ...
                else if (ev.severity >= 0.4f) dotCol = MixCoachTheme::warning();
                else dotCol = MixCoachTheme::success();
                ...
                if (clickedEventIdx_ == ei)
                {
                    ...
                    g.drawEllipse((float)(eventX - 2), (float)(dotY - 2),
                                  (float)(dotSize + 4), (float)(dotSize + 4), 1.5f);
                }
                ...
                if (evMsg.length() > 28)
                    evMsg = evMsg.substring(0, 26) + "…";

                g.setFont(juce::Font(juce::FontOptions(MixCoachTheme::fontSizeExtraTiny)));
                auto textCol = (clickedEventIdx_ == ei)
                    ? juce::Colour(0xFFF1F1F6).withAlpha(1.0f)
                    : juce::Colour(0xFF8B8FA3).withAlpha(0.80f);
                g.setColour(textCol);
                int textW = juce::GlyphArrangement::getStringWidthInt(
                    juce::Font(juce::FontOptions(MixCoachTheme::fontSizeExtraTiny)), evMsg) + 6;
                if (eventX + textW > maxW) break;

                auto textArea = juce::Rectangle<int>(eventX, bannerArea.getY() + 3,
                                                      textW, bannerArea.getHeight() - 6);
                ...
                auto eventBounds = juce::Rectangle<int>(eventX - (dotSize + 4), bannerArea.getY(),
                                                         textW + dotSize + 4, bannerArea.getHeight());
            ...
            if ((int)topEvents_.size() > maxShow)
            {
                ...
                auto remArea = juce::Rectangle<int>(eventX, bannerArea.getY() + 3,
                                                     24, bannerArea.getHeight() - 6);
        ...
        else if (!bannerCollapsed_)
        {
            ...
            auto emptyArea = juce::Rectangle<int>(bannerCollapseBtn_.getRight() + 76, bannerArea.getY() + 3,
                                                    200, bannerArea.getHeight() - 6);
        ...
        else
        {
            ...
            if (!topEvents_.empty())
            {
                ...
                auto countArea = juce::Rectangle<int>(countX, bannerArea.getY() + 3,
                                                       100, bannerArea.getHeight() - 6);
    ...
    if (clickedEventIdx_ >= 0 && clickedEventIdx_ < (int)topEvents_.size())
    {
        ...
        if (ev.trackId >= 0 && ev.trackId < SlotRegistry::kMaxSlots)
        {
            ...
            if (entry.info.active)
            {
                ...
                if (trackName.isEmpty())
                    trackName = "Track " + juce::String(ev.trackId + 1);
            }
        }

        juce::String contextLabel;
        if (ev.context == "gain")         contextLabel = "Gain Staging";
        else if (ev.context == "dynamics") contextLabel = "Dynamics";
        else if (ev.context == "tonal")    contextLabel = "Tonal Balance";
        else                                contextLabel = "General";

        juce::String severityLabel;
        juce::Colour sevCol;
        if (ev.severity >= 0.7f)      { severityLabel = "HIGH"; sevCol = MixCoachTheme::error(); }
        else if (ev.severity >= 0.4f) { severityLabel = "MEDIUM"; sevCol = MixCoachTheme::warning(); }
        else                          { severityLabel = "LOW"; sevCol = MixCoachTheme::success(); }
        ...
        if (popupY + popupH > getHeight() - 4)
            popupY = clickedMousePos_.y - popupH - 8;
        ...
        g.fillRoundedRectangle(popupBounds.toFloat(), 6.0f);            g.setColour(sevCol.withAlpha(0.50f));
        ...
        if (std::abs(ev.deviation) > 0.001f || std::abs(ev.value) > 0.001f)
        {
            ...
            if (std::abs(ev.threshold) > 0.001f)
                metricsStr += " | Target: " + juce::String(ev.threshold, 1);
            if (std::abs(ev.deviation) > 0.001f)
                metricsStr += " | " + juce::String(ev.deviation > 0 ? "+" : "") + juce::String(ev.deviation, 1) + " dB";
                                             ...
                                             const MessengerEntry& entry, int index)
{
    ...
    juce::ColourGradient glassGrad(
        juce::Colours::white.withAlpha(isSelected ? 0.09f : 0.06f * fade),
        juce::Point<float>(0.0f, glassBar.getY()),
        juce::Colour(0x00000000),
        juce::Point<float>(0.0f, glassBar.getBottom()), false);
    ...
    if (hasRole) {
        ...
        if (entry.roleConfidence >= 0.75f)
            confIcon = "\xE2\x9C\x93"; // ✓
    ...
    juce::String lStr = peakL > kMeterMinDb + 1.0f
        ? "L " + juce::String(peakL, 1) + " dB"
        : "L --.- dB";
    g.drawFittedText(lStr, lArea, juce::Justification::centredRight, 1);

    g.setColour(MixCoachTheme::channelRight().withAlpha(fade * 0.9f));
    g.setFont(juce::Font(juce::FontOptions(8.5f)).boldened());
    juce::String rStr = peakR > kMeterMinDb + 1.0f
        ? "R " + juce::String(peakR, 1) + " dB"
        : "R --.- dB";
    ...
    if (entry.info.colour.getARGB() != 0xFF808080)
        circleColour = entry.info.colour;
    ...
    g.fillEllipse((float)(circleX - 1), (float)(circleY - 1),
                  (float)(circleSize + 2), (float)(circleSize + 2));
    g.setColour(circleColour.withAlpha(fade));
    g.fillEllipse((float)circleX, (float)circleY,
                  (float)circleSize, (float)circleSize);
    g.setColour(juce::Colours::white.withAlpha(0.20f * fade));
    g.fillEllipse((float)(circleX + 2), (float)(circleY + 1),
                  (float)(circleSize * 0.3f), (float)(circleSize * 0.25f));
    ...
    // ─── Status strip (left edge) ────────────────────────────────────────
    {
        ...
        if (entry.coachAdviceText.isNotEmpty()) {
            switch (entry.coachAdviceStatus) {
                case SuggestionStatus::Red:    statusColour = MixCoachTheme::error();   break;
                case SuggestionStatus::Yellow: statusColour = MixCoachTheme::warning(); break;
                case SuggestionStatus::Green:  statusColour = MixCoachTheme::success(); break;
                case SuggestionStatus::White:  statusColour = textMuted();               break;
                default: statusColour = juce::Colours::transparentBlack; break;
            }
    ...
    int nameX = circleX + circleSize + 8;  // se ajusta si hay health dot
    {
        ...
        switch (entry.trackHealth) {
            case TrackHealth::Clean:           healthColour = MixCoachTheme::success(); break;
            case TrackHealth::NeedsEQ:
            case TrackHealth::NeedsCompression:
            case TrackHealth::MaskingIssue:
            case TrackHealth::PhaseIssue:      healthColour = MixCoachTheme::warning(); break;
            case TrackHealth::Overcompressed:
            case TrackHealth::ClippingRisk:
            case TrackHealth::StereoCollapse:  healthColour = MixCoachTheme::error();   break;
            case TrackHealth::LowSignal:
            case TrackHealth::Silent:          healthColour = textMuted();               break;
            default:                           healthColour = textMuted().withAlpha(0.2f); break;
        }
        ...
        if (entry.hasSignal && entry.trackHealth != TrackHealth::Unknown)
        {
            ...
            g.fillEllipse((float)(hDotX - 1), (float)(hDotY - 1),
                          (float)(hDotSize + 2), (float)(hDotSize + 2));
            // Dot
            g.setColour(healthColour.withAlpha(0.80f * fade));
            g.fillEllipse((float)hDotX, (float)hDotY,
                          (float)hDotSize, (float)hDotSize);
            // Highlight
            g.setColour(juce::Colours::white.withAlpha(0.18f * fade));
            g.fillEllipse((float)(hDotX + 2), (float)(hDotY + 1),
                          (float)(hDotSize * 0.3f), (float)(hDotSize * 0.25f));
    ...
    if (entry.coachAdviceText.isNotEmpty())
    {
        ...
        switch (entry.coachAdviceStatus) {
            case SuggestionStatus::Red:    trackFeedColour = MixCoachTheme::error();   break;
            case SuggestionStatus::Yellow: trackFeedColour = MixCoachTheme::warning(); break;
            case SuggestionStatus::Green:  trackFeedColour = MixCoachTheme::success(); break;
            case SuggestionStatus::White:  trackFeedColour = textMuted();               break;
            default:                       trackFeedColour = textDim();                 break;
        }
        auto feedArea = juce::Rectangle<int>(nameX, y0 + 17, nameW, 11);
        g.setFont(interFont(8.5f).boldened());
        g.setColour(trackFeedColour.withAlpha(fade * 0.90f));
        g.drawFittedText(entry.coachAdviceText, feedArea, juce::Justification::centredLeft, 1);
    }

    // ─── PK|RMS + Crest metrics ──────────────────────────────────────────
    {
        auto metricsArea = juce::Rectangle<int>(nameX, y0 + 28, nameW, 11);
        juce::String pkStr2 = peakCombined > kMeterMinDb + 1.0f
            ? "PK " + juce::String(peakCombined, 1)
            : "PK ---";
        juce::String rmsStr2 = rmsDb > kMeterMinDb + 1.0f
            ? "RMS " + juce::String(rmsDb, 1)
            : "RMS ---";
        juce::String crestStr = (entry.hasSignal && entry.crestDb > 0.1f)
            ? "CR " + juce::String(entry.crestDb, 1)
            : "CR ---";

        // Color-code crest: green (4-12dB), yellow (12-18dB), red (>18dB or <4dB)
        juce::Colour crestCol = textDim().withAlpha(fade * 0.75f);
        if (entry.hasSignal && entry.crestDb > 0.1f)
        {
            if (entry.crestDb < 4.0f || entry.crestDb > 24.0f)
                crestCol = MixCoachTheme::error().withAlpha(fade * 0.80f);
            else if (entry.crestDb < 6.0f || entry.crestDb > 18.0f)
                crestCol = MixCoachTheme::warning().withAlpha(fade * 0.80f);
            else
                crestCol = MixCoachTheme::success().withAlpha(fade * 0.80f);
        }

        g.setFont(interFont(7.0f));
        juce::String metricsStr = pkStr2 + " | " + rmsStr2 + " | ";
        int crestStartX = metricsArea.getX() + juce::GlyphArrangement::getStringWidthInt(
            interFont(7.0f), metricsStr);

        g.setColour(textDim().withAlpha(fade * 0.75f));
        g.drawFittedText(metricsStr, metricsArea, juce::Justification::centredLeft, 1);

        // Draw crest with its own colour
        auto crestArea = juce::Rectangle<int>(metricsArea.getX() + crestStartX - metricsArea.getX(),
                                               metricsArea.getY(),
                                               metricsArea.getRight() - crestStartX,
                                               metricsArea.getHeight());
        g.setFont(interFont(7.0f).boldened());
        g.setColour(crestCol);
        g.drawFittedText(crestStr, crestArea, juce::Justification::centredLeft, 1);
    }

    // ─── Spectral mini-bar + stereo width (bottom edge) ────────────────
    {
        int barY = y0 + h - 4;
        int barH = 3;
        int leftEdge = x0 + circleSize + 14; // after colour circle + gap
        int rightEdge = routeX + 2;           // before bus pill
        int barW = juce::jmax(20, rightEdge - leftEdge);

        if (barW > 20 && entry.hasSignal)
        {
            int barX = leftEdge;

            // ─── 6-band spectral mini-bar ─── usando MixCoachTheme::specColour() ─

            // Find max energy across bands for normalization
            float maxEnergy = -100.0f;
            for (int r = 0; r < 6; ++r)
                maxEnergy = juce::jmax(maxEnergy, entry.bandLevelDb[r]);

            float bandW = (float)barW / 6.0f;
            for (int r = 0; r < 6; ++r)
            {
                float energy = entry.bandLevelDb[r];
                float fraction = 0.05f;
                if (maxEnergy > -90.0f && energy > -90.0f)
                {
                    float normalized = (energy + 60.0f) / 54.0f;
                    fraction = juce::jlimit(0.05f, 1.0f, normalized);
                }

                float bandX = (float)barX + (float)r * bandW;
                float bandH = (float)barH * fraction;
                float bandTop = (float)(barY + barH) - bandH;

                auto bandRect = juce::Rectangle<float>(bandX + 0.5f, bandTop,
                                                        bandW - 1.0f, bandH);
                g.setColour(MixCoachTheme::specColour(r).withAlpha(0.65f * fade));
                g.fillRect(bandRect);
            }

            // ─── Stereo width indicator (tiny bar at right edge) ───────
            ...
            // Background
            g.setColour(juce::Colour(0xFF0E0F18).withAlpha(fade));
            g.fillRect((float)swBarX, (float)swBarY, (float)swBarW, (float)barH);

            // Fill — color según zona
            juce::Colour swColour;
            if (entry.correlation < 0.0f)
                swColour = MixCoachTheme::error().withAlpha(fade * 0.70f);   // Phase issue
            else if (swNorm < 0.2f)
                swColour = juce::Colour(0xFF94A3B8).withAlpha(fade * 0.50f); // Near mono
            else if (swNorm < 0.4f)
                swColour = MixCoachTheme::accentCyan().withAlpha(fade * 0.65f); // Narrow stereo
            else
                swColour = MixCoachTheme::accent().withAlpha(fade * 0.70f);  // Wide

            g.setColour(swColour);
            g.fillRect((float)swBarX, (float)swBarY, swFill, (float)barH);
        }
    }
}
