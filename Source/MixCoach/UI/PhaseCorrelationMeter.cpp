...
namespace mixcoach {
    ...
    bool PhaseCorrelationMeter::advanceFrame(double sampleRateHz, bool allowRepaint)
    {
        ...
        juce::Colour col;
        if (std::abs(corr) < 0.3f) col = MixCoachTheme::error();
        else if (corr < 0.0f)
    ...
    void PhaseCorrelationMeter::paint(juce::Graphics& g)
    {
        ...
        juce::ColourGradient redGrad(MixCoachTheme::error().withAlpha(0.25f),
        ...
        auto yellowZone = barBounds.withLeft(leftX + totalW * 0.35f).withWidth(totalW * 0.30f);
        juce::ColourGradient yellowGrad(MixCoachTheme::warning().withAlpha(0.20f),
        ...
        auto greenZone = barBounds.withLeft(leftX + totalW * 0.65f).withWidth(totalW * 0.35f);
        juce::ColourGradient greenGrad(MixCoachTheme::success().withAlpha(0.25f),
        ...
        if (std::abs(corr) < 0.3f) indicatorColour = MixCoachTheme::error();
        else if (corr < 0.0f)
        ...
        // Glow del marker
        juce::ColourGradient markerGlow(indicatorColour.withAlpha(0.3f),
        ...
        if (hasPhaseDiagnostic_ && phaseDiagnostic_.description.isNotEmpty()) {
            ...
            // ─── Highlight zone en la barra según severidad ────────────────
            {
                ...
                auto highlightRect = juce::Rectangle<float>(barBounds.getX() + highlightPos * totalW,
                                                            barBounds.getY() - 2.0f,
            ...
            {
                auto warnArea =
                    juce::Rectangle<float>(barBounds.getX(), barBounds.getBottom() + 2.0f, barBounds.getWidth(), 16.0f);

                g.setFont(juce::Font(juce::FontOptions(8.0f)).boldened());
                g.setColour(colour.withAlpha(0.4f + severity * 0.4f));
                g.drawText(phaseDiagnostic_.description, warnArea.toNearestInt(), juce::Justification::centred);
            }
        }
    }
