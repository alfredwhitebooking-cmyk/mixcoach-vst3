...
void SpectrographComponent::changeListenerCallback(juce::ChangeBroadcaster* source)
{
    ...
    {
        float bandWidth = (diag.highFreqHz > 0.0f && diag.lowFreqHz > 0.0f)
            ? std::log2(diag.highFreqHz / diag.lowFreqHz) : 0.0f;
        if (bandWidth > 2.0f || bandWidth <= 0.0f)
            continue;

        float centerHz = std::sqrt(diag.lowFreqHz * diag.highFreqHz);
        juce::String label;
        if (centerHz >= 1000.0f)
            label = juce::String(centerHz / 1000.0f, 1) + "k";
        else
            label = juce::String(static_cast<int>(centerHz));

        bool alreadyExists = false;
        for (const auto& m : frequencyMarkers_) {
            float logRatio = (centerHz > 0.0f && m.frequencyHz > 0.0f)
                ? std::abs(std::log2(centerHz / m.frequencyHz)) : 999.0f;
        ...
        if (diag.trackName.isNotEmpty())
            desc = diag.trackName + ": " + desc;

        addFrequencyMarker(centerHz, label,
                           diag.severity, diag.isCritical,
                           diag.isPraise, 10.0f);
    }
...
void SpectrographComponent::paint(juce::Graphics& g)
{
    ...
    if (layout_.valid)
    {
        ...
        {
            auto dbColInComp = layout_.dbCol.translated((float) plotArea_.getX(),
                                                        (float) plotArea_.getY());
...
void SpectrographComponent::mouseMove(const juce::MouseEvent& e)
{
    ...
    if (!layout_.valid || plotArea_.isEmpty())
        return;

    auto plotInComp = layout_.plot.translated((float) plotArea_.getX(),
                                               (float) plotArea_.getY());
