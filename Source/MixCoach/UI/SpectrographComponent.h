...
public:
    ...
    [[nodiscard]] bool hasActiveDiagnostics() const noexcept
    {
        return !activeDiagnostics_.empty();
    }
    ...
    void setReferenceEnabled(bool enabled) { referenceEnabled_ = enabled; repaint(); }
...
private:
    ...
    enum DisplayMode { kPeak, kRms, kHybrid };
    ...
    int slopePresetIndex_ = 0;  // Precision metering: flat dBFS response by default. (Ctrl+Click para cambiar: 0/1.5/3.0/4.5)
...
public:
    ...
    void addFrequencyMarker(float freqHz, const juce::String& label,
                            float severity = 0.5f, bool isCritical = false,
                            bool isPraise = false, float durationSecs = 10.0f);
