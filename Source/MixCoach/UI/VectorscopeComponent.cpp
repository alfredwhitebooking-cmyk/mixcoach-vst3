...
juce::Rectangle<float> VectorscopeComponent::plotCircleArea() const
{
    ...
    return juce::Rectangle<float>(0.0f, 0.0f, squareSize, squareSize)
...
void VectorscopeComponent::drawGrid(juce::Graphics& g, juce::Rectangle<float> area)
{
    ...
    struct RingDef { float norm; float alpha; float width; };
    RingDef rings[] = {
        { 0.15f, 0.04f, 0.3f },
        { 0.30f, 0.06f, 0.4f },
        { 0.50f, 0.08f, 0.4f },
    ...
    for (auto& ring : rings) {
        ...
        juce::Colour ringCol = (ring.norm > 0.99f)
            ? juce::Colour(0xFF8899AA).withAlpha(ring.alpha)
    ...
    // ─── Tick marks cada 30° en el anillo exterior ────────────────────────
    {
        ...
        for (int deg = 0; deg < 360; deg += 30) {
            ...
            g.drawLine(cx + cosA * outerR, cy + sinA * outerR,
                       cx + cosA * innerR, cy + sinA * innerR, 0.6f);
        }
    }
    ...
    g.drawText("M", juce::Rectangle<float>(cx - 8.0f, area.getY() + 1.0f, 16.0f, 12.0f),
               juce::Justification::centred);
    // L - diagonal superior izquierda - azul claro
    g.setColour(juce::Colour(0xFF60A5FA).withAlpha(0.70f));
    g.drawText("L", juce::Rectangle<float>(cx - diag - 8.0f, cy - diag - 6.0f, 16.0f, 12.0f),
               juce::Justification::centred);
    // R - diagonal superior derecha - verde
    g.setColour(juce::Colour(0xFF34D399).withAlpha(0.70f));
    g.drawText("R", juce::Rectangle<float>(cx + diag - 8.0f, cy - diag - 6.0f, 16.0f, 12.0f),
               juce::Justification::centred);
    // S - extremos horizontales (eje lateral / anti-fase) - cyan
    g.setColour(juce::Colour(0xFF00B7FF).withAlpha(0.55f));
    g.drawText("S", juce::Rectangle<float>(area.getX() + 1.0f, cy - 6.0f, 14.0f, 12.0f),
               juce::Justification::centred);
    g.drawText("S", juce::Rectangle<float>(area.getRight() - 15.0f, cy - 6.0f, 14.0f, 12.0f),
               juce::Justification::centred);
}
...
void VectorscopeComponent::paint(juce::Graphics& g)
{
    ...
    if (stereoWidth_ > 0.01f) {
        ...
        else if (stereoWidth_ < 0.40f) wCol = juce::Colour(0xFF44BBFF);
        else if (stereoWidth_ < 0.70f) wCol = juce::Colour(0xFF44CC66);
        else                           wCol = juce::Colour(0xFFFF6633);
...
void VectorscopeComponent::setDisplayCorrelation(float correlation)
{
    ...
    if (std::abs(correlation_ - old) > 0.005f)
        repaint();
}
...
void VectorscopeComponent::drawPhaseOverlay(juce::Graphics& g, juce::Rectangle<float> circleArea)
{
    ...
    if (phaseDiagnostic_.isWarning || phaseDiagnostic_.description.isNotEmpty())
    {
        ...
        auto textArea = juce::Rectangle<float>(
            circleArea.getX() + 12.0f,
            textY,
            circleArea.getWidth() - 24.0f,
            18.0f
        );
        ...
        if (displayText.isEmpty()) {
            if (phaseDiagnostic_.isWarning)
                displayText = "\xE2\x9A\xA0 Phase Warning";
            else if (phaseDiagnostic_.isPraise)
                displayText = "\xE2\x9C\x93 Good Phase";
        }
        
        if (displayText.isNotEmpty())
            g.drawText(displayText, textArea, juce::Justification::centredTop);
    }
}
