...
{
    if (! crestSmooth_.advance(sampleRateHz))
        return false;
...
void CrestHistogram::drawArc(juce::Graphics& g, juce::Rectangle<float> gaugeBounds)
{
    ...
    juce::Path bgArc;
    bgArc.addArc(cx - midR, cy - midR, midR * 2.0f, midR * 2.0f,
    ...
    auto drawColoredArc = [&](float startNorm, float endNorm,
                               juce::Colour colour, float alpha)
    {
        ...
        juce::Path segPath;
        segPath.addArc(cx - midR, cy - midR, midR * 2.0f, midR * 2.0f,
    ...
    if (crestNorm > 0.01f)
    {
        ...
        if (crestNorm <= greenEnd)
            fillColour = MixCoachTheme::meterGreen();
        ...
        juce::Path fillArc;
        fillArc.addArc(cx - midR, cy - midR, midR * 2.0f, midR * 2.0f,
                       kArcStartAngle, fillEnd, true);
...
void CrestHistogram::drawScale(juce::Graphics& g, juce::Rectangle<float> gaugeBounds)
{
    ...
    struct ScaleMark { float value; bool isMajor; };
    ScaleMark marks[] = {
        { 0.0f, true }, { 5.0f, false }, { 10.0f, true },
        { 15.0f, false }, { 20.0f, true },
        { 25.0f, false }, { 30.0f, true }
    };
...
// ═══════════════════════════════════════════════════════════════════════════
void CrestHistogram::drawNeedle(juce::Graphics& g, juce::Rectangle<float> gaugeBounds,
                                 float angle)
{
    float cx = gaugeBounds.getCentreX();
    float cy = gaugeBounds.getBottom();
...
{
    auto infoArea = gaugeBounds.withTop(gaugeBounds.getBottom() + 4.0f)
                               .withHeight(36.0f).toNearestInt();
    ...
    if (crestVal < 10.0f)
        crestColour = MixCoachTheme::meterGreen();
