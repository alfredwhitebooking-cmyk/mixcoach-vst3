...
namespace mixcoach {
    ...
    {
        if (menuId <= 0 || menuId == 1) return TrackRole::Unknown;
        ...
        static const std::vector<std::vector<TrackRole>> kCatRoles = {
            ...
            {TrackRole::BassSub,
             TrackRole::Bass808,
             TrackRole::BassPick,
             TrackRole::BassFinger,
             TrackRole::BassSynth,
             TrackRole::BassBus},
            {TrackRole::GuitarAcoustic,
             TrackRole::GuitarElectric,
             TrackRole::GuitarRhythm,
             TrackRole::GuitarLead,
             TrackRole::GuitarBus},
            {TrackRole::SynthLead,
             TrackRole::SynthPad,
             TrackRole::SynthPluck,
             TrackRole::KeysPiano,
             TrackRole::KeysElectric,
             TrackRole::KeysOrgan,
             TrackRole::KeysBus,
             TrackRole::MelodyBus},
            {TrackRole::VozPrincipal, TrackRole::VozFondo, TrackRole::VozDouble, TrackRole::Adlibs, TrackRole::VozBus},
        ...
        if (catIdx >= 0 && catIdx < (int)kCatRoles.size() && itemIdx >= 0 && itemIdx < (int)kCatRoles[catIdx].size()) {
    ...
    juce::PopupMenu buildRoleMenuStatic()
    {
        ...
        std::vector<CatDef> cats = {
            {RoleCategory::Drums,
             "Drums",
             {{TrackRole::Kick, "Kick"},
              {TrackRole::Kick808, "Kick 808"},
              {TrackRole::Snare, "Snare"},
              {TrackRole::SnareTrap, "Snare Trap"},
              {TrackRole::HiHat, "Hi-Hat"},
              {TrackRole::HiHatOpen, "Hi-Hat Open"},
              {TrackRole::Ride, "Ride"},
              {TrackRole::Crash, "Crash"},
              {TrackRole::Tom, "Tom"},
              {TrackRole::TomFloor, "Tom Floor"},
              {TrackRole::Percussion, "Percussion"},
              {TrackRole::Clap, "Clap"},
              {TrackRole::DrumBus, "Drum Bus"}}},
            {RoleCategory::Bass,
             "Bass",
             {{TrackRole::BassSub, "Sub Bass"},
              {TrackRole::Bass808, "808 Bass"},
              {TrackRole::BassPick, "Bass (Pick)"},
              {TrackRole::BassFinger, "Bass (Finger)"},
              {TrackRole::BassSynth, "Synth Bass"},
              {TrackRole::BassBus, "Bass Bus"}}},
            {RoleCategory::Guitars,
             ...
             {{TrackRole::GuitarAcoustic, "Acoustic Guitar"},
              ...
              {TrackRole::GuitarLead, "Lead Guitar"},
              {TrackRole::GuitarBus, "Guitar Bus"}}},
            {RoleCategory::Keys,
             "Keys / Synths",
             {{TrackRole::SynthLead, "Synth Lead"},
              {TrackRole::SynthPad, "Synth Pad"},
              {TrackRole::SynthPluck, "Synth Pluck"},
              {TrackRole::KeysPiano, "Piano"},
              {TrackRole::KeysElectric, "Electric Piano"},
              {TrackRole::KeysOrgan, "Organ"},
              {TrackRole::KeysBus, "Keys Bus"},
              {TrackRole::MelodyBus, "Melody Bus"}}},
            {RoleCategory::Vocals,
             "Vocals",
             {{TrackRole::VozPrincipal, "Lead Vocal"},
              {TrackRole::VozFondo, "Backing Vocal"},
              {TrackRole::VozDouble, "Vocal Double"},
              {TrackRole::Adlibs, "Ad-libs"},
              {TrackRole::VozBus, "Vocal Bus"}}},
            {RoleCategory::FX,
             "FX / Ambience",
             {{TrackRole::FxRiser, "Riser"},
              {TrackRole::FxImpact, "Impact"},
              {TrackRole::FxAmbience, "Ambience"},
              {TrackRole::FxNoise, "Noise"}}},
            {RoleCategory::Melody,
             "Melodic",
             {{TrackRole::Strings, "Strings"}, {TrackRole::Brass, "Brass"}, {TrackRole::Winds, "Winds"}}}};
    ...
    void MessengerListComponent::syncRolesFromCoach()
    {
        ...
        for (int i = 0; i < SlotRegistry::kMaxSlots; ++i) {
            ...
            if (messengers_[i].trackRole == TrackRole::Unknown || messengers_[i].trackRole == TrackRole::Master)
