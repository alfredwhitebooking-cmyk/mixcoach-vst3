...
void CrestPanel::setValues(float peak, float rms)
{
    ...
    float c = (rms > -60.0f && peak > -60.0f)
        ? juce::jmax(0.0f, peak - rms)
        : 0.0f;
...
void CrestPanel::paint(juce::Graphics& g)
{
    ...
    // ─── Header: "CREST FACTOR" con glow morado ───────────────────────────
    {
        ...
        juce::ColourGradient glow(
            juce::Colour(0xFFC084FC).withAlpha(0.25f), underline.getX(), underline.getY(),
            juce::Colour(0xFFC084FC).withAlpha(0.01f), underline.getRight(), underline.getY(), false);
    ...
    auto sepArea = area.removeFromTop(juce::jmax(6, area.getHeight() * 3 / 100));
    {
        ...
        juce::ColourGradient sepGrad(
            juce::Colour(0xFFA855F7).withAlpha(0.15f), (float)sepArea.getX(), sy,
            juce::Colour(0xFFA855F7).withAlpha(0.02f), (float)sepArea.getRight(), sy, false);
...
void CrestPanel::drawGauge(juce::Graphics& g, juce::Rectangle<float> bounds)
{
    ...
    {
        auto gaugeBg = bounds;
        juce::ColourGradient radialBg(
            juce::Colour(0xFF0E1520), gaugeBg.getCentreX(), gaugeBg.getCentreY(),
            juce::Colour(0xFF06080E), gaugeBg.getX(), gaugeBg.getY(), true);
    ...
    {
        juce::Path bgArc;
        bgArc.addArc(cx - radius, cy - halfR, radius * 2.0f, halfR * 2.0f,
                     kGaugeStart, kGaugeEnd, true);
        // Outer glow
        g.setColour(juce::Colour(0xFF1A2A44).withAlpha(0.08f));
    ...
    for (int i = 0; i < kArcSegments; ++i) {
        ...
        juce::Path segArc;
        segArc.addArc(cx - radius, cy - halfR, radius * 2.0f, halfR * 2.0f,
                      aStart, aEnd, true);
    ...
    if (crestNorm > 0.01f) {
        ...
        juce::Path fillArc;
        fillArc.addArc(cx - radius, cy - halfR, radius * 2.0f, halfR * 2.0f,
                       kGaugeStart, fillEnd, true);

        // Color dinámico con transición suave
        juce::Colour fillCol;
        if (crestNorm < 0.30f)
            fillCol = juce::Colour(0xFF44CC66);
        else if (crestNorm < 0.50f)
            fillCol = juce::Colour(0xFF44CC66).interpolatedWith(juce::Colour(0xFFFFCC44), (crestNorm - 0.30f) / 0.20f);
        else if (crestNorm < 0.70f)
            fillCol = juce::Colour(0xFFFFCC44).interpolatedWith(juce::Colour(0xFFFF8833), (crestNorm - 0.50f) / 0.20f);
        else
            fillCol = juce::Colour(0xFFFF8833).interpolatedWith(juce::Colour(0xFFEE3333), (crestNorm - 0.70f) / 0.30f);
...
void CrestPanel::drawScale(juce::Graphics& g, float cx, float cy, float radius)
{
    ...
    for (auto& m : majorMarks) {
        ...
        g.drawText(juce::String(m.text),
                   juce::Rectangle<float>(lx - 11, ly - 5, 22, 10),
                   juce::Justification::centred);
    }

    // ─── Marcas secundarias entre principales ───────────────────────────
    struct MinorRange { float start, end, step; };
    ...
    g.drawText("dB", juce::Rectangle<float>(cx - 12, labelY, 24, 9),
               juce::Justification::centred);
}
...
void CrestPanel::drawNeedle(juce::Graphics& g, float cx, float cy,
                            float radius, float crestNorm)
{
    float halfR = radius;
    ...
    // ─── Cuerpo principal de la aguja (gradiente metálico cónico) ────────
    {
        ...
        juce::ColourGradient needleGrad(
            juce::Colour(0xFF8A9AAA), nx, ny,   // gris claro brillante (punta)
            juce::Colour(0xFF1A2233), cx, cy,   // gris oscuro profundo (base)
            true);
    ...
    float hOffY = perpY * 1.0f;
    {
        ...
        float hlBase = 1.5f;
        float hlTip  = 0.3f;
        highlightPath.startNewSubPath(cx + hOffX + perpX * hlBase * 0.5f,
                                      cy + hOffY + perpY * hlBase * 0.5f);
        highlightPath.lineTo(nx + hOffX + perpX * hlTip * 0.5f,
                             ny + hOffY + perpY * hlTip * 0.5f);
        highlightPath.lineTo(nx + hOffX - perpX * hlTip * 0.5f,
                             ny + hOffY - perpY * hlTip * 0.5f);
        highlightPath.lineTo(cx + hOffX - perpX * hlBase * 0.5f,
                             cy + hOffY - perpY * hlBase * 0.5f);
    ...
    // ─── Pivote mecánico premium ─────────────────────────────────────────
    {
        ...
        juce::ColourGradient pivotGrad(
            juce::Colour(0xFF6A7A8A), cx - 4.0f, cy - 4.0f,
            juce::Colour(0xFF2A3344), cx + 4.0f, cy + 4.0f, true);
...
void CrestPanel::drawMetricsTable(juce::Graphics& g, juce::Rectangle<float> bounds)
{
    ...
    if (cval > 24.0f)
        crestCol = juce::Colour(0xFFEE3333);
    else if (cval > 18.0f)
    ...
    for (int i = 0; i < numRows; ++i) {
        auto rowArea = juce::Rectangle<float>(
            bounds.getX(), bounds.getY() + i * rowH - 0.5f,
            bounds.getWidth(), rowH + 1.0f);

        // ─── Background highlight para CREST ─────────────────────────────
        if (rows[i].highlight) {
            auto bgBounds = rowArea.reduced(2, 1);
            juce::ColourGradient hlGrad(
                rows[i].accent.withAlpha(0.06f), bgBounds.getX(), bgBounds.getY(),
                rows[i].accent.withAlpha(0.01f), bgBounds.getRight(), bgBounds.getY(), false);
        ...
        juce::String valStr;
        if (rows[i].value > -60.0f)
            valStr = juce::String(rows[i].value, 1);
        else
            valStr = "--.-";
        ...
        if (i < numRows - 1) {
            ...
            juce::ColourGradient sepGrad(
                MixCoachTheme::divider().withAlpha(0.12f), bounds.getX() + 15, sy,
                MixCoachTheme::divider().withAlpha(0.04f), bounds.getRight() - 15, sy, false);
            g.setGradientFill(sepGrad);
