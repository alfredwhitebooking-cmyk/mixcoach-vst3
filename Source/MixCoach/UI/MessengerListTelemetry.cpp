...
// ═══════════════════════════════════════════════════════════════════════════
void MessengerListComponent::syncTelemetryFromRegistry(SlotRegistry& registry,
                                                       bool& anyDataOut,
    ...
    registry.forEachActive([&](const SlotInfo& info)
    {
        ...
        else
        {
            ...
            if (sharedData.getAudioMemoryV2().isInitialized())
            {
                ...
                if (nRead > 0)
                {
                    ...
                    float rmsL = (sumSqL > 0.0) ? juce::Decibels::gainToDecibels((float)std::sqrt(sumSqL / nRead)) : -100.0f;
                    float rmsR = (sumSqR > 0.0) ? juce::Decibels::gainToDecibels((float)std::sqrt(sumSqR / nRead)) : -100.0f;
            ...
            if (!hasSignal && sharedData.getAudioMemory().isInitialized())
            {
                ...
                if (nRead > 0)
                {
                    ...
                    rawPeakR = rawPeakL;
                    float rms = (sumSq > 0.0) ? juce::Decibels::gainToDecibels((float)std::sqrt(sumSq / nRead)) : -100.0f;
        ...
        if (!hasSignal && !haveCacheData)
        {
            ...
            if (nowMs - lastNoSignalLogMs_ > 10000)
            {
                ...
                LogHelper::writeToLog("[DIAG] Slot " + juce::String(idx)
                    + " sin senal: V2init=" + juce::String(sharedData.getAudioMemoryV2().isInitialized() ? 1 : 0)
        ...
        {
            if (rawPeakCombined > entry.barLevel)
                entry.barLevel = rawPeakCombined;
            if (rawRmsCombined > entry.rmsSmooth)
                entry.rmsSmooth = rawRmsCombined;
            ...
            if (ts.hasSignal() && ts.health != TrackHealth::Unknown && ts.health != TrackHealth::Silent)
            {
                ...
                switch (ts.health) {
                    ...
                    case TrackHealth::Clean:
                        ...
                        overrideExisting = (suggestion.status == SuggestionStatus::None || suggestion.status == SuggestionStatus::White);
                        break;
                    default:
...
void MessengerListComponent::smoothMeters()
{
    ...
    for (int idx = 0; idx < SlotRegistry::kMaxSlots; ++idx)
    {
        ...
        if (entry.rmsSmooth > entry.rmsAvg + 0.5f)
            entry.rmsSmooth += (entry.rmsAvg - entry.rmsSmooth) * 0.06f;
