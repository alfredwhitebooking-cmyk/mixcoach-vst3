...
struct MixCoachTheme
{
    ...
    static void fillGlassPanel(juce::Graphics& g, juce::Rectangle<float> bounds, float radius = 6.0f)
    {
        ...
        juce::ColourGradient topGrad(
            glassHighlight(), juce::Point<float>(0.0f, highlight.getY()),
            juce::Colours::transparentBlack, juce::Point<float>(0.0f, highlight.getBottom()), false);
    ...
    static void drawGradientMeter(juce::Graphics& g, juce::Rectangle<float> bounds,
                                  float level, bool vertical = true, float radius = 1.5f)
    {
        ...
        if (vertical)
        {
            meterGrad = juce::ColourGradient(
                meterGreen(),  bounds.getCentreX(), bounds.getBottom(),
                meterRed(),    bounds.getCentreX(), bounds.getY(), false);
        ...
        {
            meterGrad = juce::ColourGradient(
                meterGreen(),  bounds.getX(),        bounds.getCentreY(),
                meterRed(),    bounds.getRight(),   bounds.getCentreY(), false);
    ...
    {
        switch (category) {
            case 0:  return roleDrums();
            case 1:  return roleBass();
            case 2:  return roleGuitars();
            case 3:  return roleKeys();
            case 4:  return roleVocals();
            case 5:  return roleFX();
            case 6:  return roleMelody();
    ...
    {
        switch (band) {
            case 0:  return specSub();
            case 1:  return specBass();
            case 2:  return specLoMid();
            case 3:  return specHiMid();
            case 4:  return specPres();
            case 5:  return specAir();
            default: return accentGlow();
        }
    }
    ...
    static constexpr int cardHeight_list       = 40;   // Messenger track list
...
inline juce::Colour textBright() { return juce::Colour(0xFFF1F1F6); }
inline juce::Colour textDim()    { return juce::Colour(0xFF8B8FA3); }
inline juce::Colour textMuted()  { return juce::Colour(0xFF5C5F73); }
