...
namespace mixcoach {
    ...
    void ProfessionalAnalyzersComponent::paint(juce::Graphics& g)
    {
        ...
        // ─── Borde superior neón ───
        juce::ColourGradient topLine(MixCoachTheme::accent().withAlpha(0.2f),
    ...
    void ProfessionalAnalyzersComponent::updateFromAnalyzer(const AudioAnalyzer& analyzer)
    {
        ...
        for (int i = 0; i < numRead; ++i) vectorscope_->pushSample(leftBuf[i], rightBuf[i]);
    ...
    void VectorscopeSystem::timerCallback()
    {
        ...
        // ─── Age phosphor trail ──────────────────────────────────────────────
        for (auto& pt : phosphorTrail_) pt.age++;

        // Remove aged-out phosphor points
        while (!phosphorTrail_.empty() && phosphorTrail_.front().age > kPhosphorSteps) phosphorTrail_.pop_front();

        // ─── Update correlation label color ──────────────────────────────────
        juce::Colour corrColour;
        float sc = smoothCorrelation_;
        if (sc < -0.3f) corrColour = MixCoachTheme::error();
        else if (sc < 0.0f)
    ...
    void VectorscopeSystem::pushSample(float left, float right)
    {
        ...
        if (std::abs(left) < 0.001f && std::abs(right) < 0.001f) return;
        ...
        if (phosphorTrail_.size() > static_cast<size_t>(kMaxPhosphor)) phosphorTrail_.pop_front();
    }
    ...
    void VectorscopeSystem::drawBackground(juce::Graphics& g, juce::Rectangle<float> area)
    {
        ...
        juce::ColourGradient radialGrad(
            MixCoachTheme::bgCard().withAlpha(0.6f), cx, cy, MixCoachTheme::bgDarker(), cx + radius, cy, false);
    ...
    void VectorscopeSystem::drawGrid(juce::Graphics& g, juce::Rectangle<float> area)
    {
        ...
        g.drawText("L", juce::Rectangle<float>(area.getX() + 2, cy - 8, 14, 14), juce::Justification::centred);
        // R - Right side
        g.setColour(MixCoachTheme::channelRight().withAlpha(0.6f));
        g.drawText("R", juce::Rectangle<float>(area.getRight() - 16, cy - 8, 14, 14), juce::Justification::centred);
        // R - Top
        g.setColour(MixCoachTheme::channelRight().withAlpha(0.6f));
        g.drawText("R", juce::Rectangle<float>(cx - 8, area.getY() + 2, 16, 14), juce::Justification::centred);
        // L - Bottom
        g.setColour(MixCoachTheme::channelLeft().withAlpha(0.6f));
        g.drawText("L", juce::Rectangle<float>(cx - 8, area.getBottom() - 16, 16, 14), juce::Justification::centred);
        ...
        for (float angle = 0; angle < 360.0f; angle += 30.0f) {
            ...
            g.drawLine(cx + cosA * r1, cy + sinA * r1, cx + cosA * r2, cy + sinA * r2, 0.5f);
        }
    }
    ...
    void VectorscopeSystem::drawPhosphorPoints(juce::Graphics& g, juce::Rectangle<float> area)
    {
        ...
        if (sc > 0.5f) {
            ...
            mix         = (0.0f - sc) / 0.3f;
            traceColour = MixCoachTheme::warning().interpolatedWith(
                MixCoachTheme::warning().interpolatedWith(MixCoachTheme::error(), 0.5f), mix);
        }
        else {
            // Orange → Red (excessive phase issues)
            mix         = (-0.3f - sc) / 0.7f;
            traceColour = MixCoachTheme::warning()
                              .interpolatedWith(MixCoachTheme::error(), 0.5f)
                              .interpolatedWith(MixCoachTheme::error(), juce::jmin(1.0f, mix));
        }
        ...
        for (const auto& pt : phosphorTrail_) {
            ...
            if (sx < area.getX() || sx > area.getRight() || sy < area.getY() || sy > area.getBottom()) continue;
    ...
    void VectorscopeSystem::drawCorrelationIndicator(juce::Graphics& g, juce::Rectangle<float> area)
    {
        ...
        if (barBounds.getY() + barBounds.getHeight() > getHeight() - 22.0f) return;
        ...
        juce::ColourGradient corrGrad(MixCoachTheme::error().withAlpha(0.2f),
        ...
        if (sc < -0.3f) markerColour = MixCoachTheme::error();
            ...
            markerColour = MixCoachTheme::warning();
        else
            markerColour = MixCoachTheme::success();
    ...
    void PhaseCorrelationSystem::timerCallback()
    {
        ...
        valueLabel_.setText(prefix + juce::String(current_, 2), juce::dontSendNotification);
    ...
    juce::Colour PhaseCorrelationSystem::getCorrelationColour(float corr) const noexcept
    {
        ...
        if (corr < -0.3f) return MixCoachTheme::error();
        else if (corr < 0.0f)
            return MixCoachTheme::error().interpolatedWith(MixCoachTheme::warning(), (corr + 0.3f) / 0.3f);
        else if (corr < 0.5f)
            return MixCoachTheme::warning().interpolatedWith(MixCoachTheme::success(), corr / 0.5f);
        else
    ...
    {
        // ─── Background ──────────────────────────────────────────────────────
        juce::ColourGradient bgGrad(MixCoachTheme::bgDarker(),
        ...
        juce::ColourGradient phaseGrad(MixCoachTheme::error().withAlpha(0.15f),
        ...
        ZoneMarker markers[] = {{-1.0f, "-1"}, {-0.5f, "-0.5"}, {0.0f, "0"}, {0.5f, "+0.5"}, {1.0f, "+1"}};
        ...
        for (auto& m : markers) {
            ...
            g.drawVerticalLine(juce::roundToInt(mx), bounds.getY() + 4, bounds.getBottom() - 4);
            // Label
            g.setColour(MixCoachTheme::textMuted().withAlpha(0.5f));
        ...
        g.drawText("OUT OF PHASE",
                   juce::Rectangle<float>(bounds.getX() + 4, bounds.getY() + 4, bounds.getWidth() * 0.3f, 14),
                   juce::Justification::centredLeft);
        g.setColour(MixCoachTheme::success().withAlpha(0.4f));
        g.drawText("IN PHASE",
                   juce::Rectangle<float>(
                       bounds.getRight() - bounds.getWidth() * 0.25f, bounds.getY() + 4, bounds.getWidth() * 0.25f, 14),
                   juce::Justification::centredLeft);
        ...
        if (norm > 0.0f) {
            ...
            if (current_ < 0.0f) {
                ...
                fillBounds    = juce::Rectangle<float>(mx, bounds.getY() + 2, centerX - mx, bounds.getHeight() - 4);
            }
            else {
                float centerX = bounds.getX() + 0.5f * bounds.getWidth();
                fillBounds = juce::Rectangle<float>(centerX, bounds.getY() + 2, mx - centerX, bounds.getHeight() - 4);
            }
