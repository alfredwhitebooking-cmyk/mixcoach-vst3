...
void MessengerListComponent::mouseDown(const juce::MouseEvent& e)
{
        ...
        for (int ei = 0; ei < (int)bannerEventBounds_.size(); ++ei)
        {
            if (bannerEventBounds_[ei].contains(e.getPosition()))
            {
                ...
                if (clickedEventIdx_ >= 0)
                    clickedMousePos_ = e.getPosition();
                repaint();
                return;
            }
        }
    ...
    if (roleRect.contains(e.getPosition()) && coachEngine_ != nullptr)
    {
        ...
        bool hasRole = (messengers_[slot].trackRole != TrackRole::Unknown
                        && messengers_[slot].trackRole != TrackRole::Master);
        bool isPending = messengers_[slot].signalOrderPending
                         || messengers_[slot].roleInferredPending;
        if (hasRole && isPending) {
            // Check if click is on the badge (right 14px of the pill area)
            auto badgeRect = juce::Rectangle<int>(roleRect.getRight() - 14, roleRect.getY(), 14, roleRect.getHeight());
        ...
        menu.showMenuAsync(juce::PopupMenu::Options(),
            [this, currentSlot](int result)
            {
                ...
                if (coachEngine_ != nullptr)
                    coachEngine_->setTrackRoleWithLearning(currentSlot, newRole);

                repaint();

                if (onSlotSelected)
                    onSlotSelected(currentSlot);
            });
        return;
    }
...
void MessengerListComponent::mouseMove(const juce::MouseEvent& e)
{
    ...
    if (slot >= 0 && slot < SlotRegistry::kMaxSlots)
    {
        ...
        if (entry.hasSignal && entry.trackHealth != TrackHealth::Unknown)
        {
            ...
            if (y >= 0)
            {
                auto hDotBounds = juce::Rectangle<int>(hDotX - 2, y + (kCardHeight - hDotSize) / 2 - 2,
                                                        hDotSize + 4, hDotSize + 4);
...
{
    if (healthFilter_ == HealthFilter::None || slotIndex < 0 || slotIndex >= SlotRegistry::kMaxSlots)
        return false;

    if (!messengers_[slotIndex].info.active)
        return true;
    ...
    switch (healthFilter_) {
        case HealthFilter::Critical:
            return !(health == TrackHealth::ClippingRisk ||
                     health == TrackHealth::Overcompressed ||
                     health == TrackHealth::StereoCollapse);
        case HealthFilter::Warning:
            return !(health == TrackHealth::NeedsEQ ||
                     health == TrackHealth::NeedsCompression ||
                     health == TrackHealth::MaskingIssue ||
                     health == TrackHealth::PhaseIssue);
        case HealthFilter::Clean:
            return health != TrackHealth::Clean;
        case HealthFilter::Silent:
            return !(health == TrackHealth::LowSignal ||
                     health == TrackHealth::Silent ||
