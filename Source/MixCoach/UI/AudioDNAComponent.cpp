...
// ─── Health levels ─────────────────────────────────────────────────────────
enum HealthLevel : uint8_t { kGood = 0, kCaution = 1, kProblem = 2 };
...
{
    switch (h) {
        case kGood:    return 0.85f;
        case kCaution: return 0.50f;
        case kProblem: return 0.20f;
...
{
    switch (h) {
        case kGood:    return MixCoachTheme::success().withAlpha(alpha);
        case kCaution: return MixCoachTheme::warning().withAlpha(alpha);
        case kProblem: return MixCoachTheme::error().withAlpha(alpha);
        default:       return MixCoachTheme::textMuted().withAlpha(alpha);
    }
}
...
{
    switch (h) {
        case kGood:    return MixCoachTheme::success().withAlpha(0.15f);
        case kCaution: return MixCoachTheme::warning().withAlpha(0.12f);
        case kProblem: return MixCoachTheme::error().withAlpha(0.15f);
        default:       return MixCoachTheme::bgInput().withAlpha(0.5f);
    }
}
...
void computeBandHealth(const AudioDNAData& data, BandHealth bands[6])
{
    for (int b = 0; b < 6; ++b) {
        ...
        bh.composite = static_cast<HealthLevel>(
            (std::max)(static_cast<int>(bh.crestHealth),
                       static_cast<int>(bh.stereoHealth)));
        ...
        if (bh.stereoHealth == kProblem && bh.stereoVal < 0.05f)
            bh.diagnosis = "Mono";
...
void AudioDNAComponent::pushHealthDiagnostics()
{
    ...
    if (nowMs - lastDiagnosticPushMs_ < 2000)
        return;
    ...
    for (int b = 0; b < 6; ++b)
    {
        ...
        if (bh.composite == kGood)
        {
            ...
            d.description = juce::String(kBandLabels[b]) + ": " + juce::String(bh.diagnosis) + " ("
                          + juce::String(bh.crestVal, 1) + "dB crest, "
                          + juce::String(bh.stereoVal, 2) + " width)";
        ...
        if (diagLabel == "compressed" || diagLabel == "over" || diagLabel == "wild")
        {
            ...
            if (diagLabel == "wild")
                diagnosis = "Excessive dynamics in " + juce::String(kBandLabels[b]);
            else
...
void AudioDNAComponent::update(SlotRegistry& registry, SharedData& sharedData)
{
    ...
    registry.forEachActive([&](const SlotInfo& info) {
        ...
        if (telem.stereoWidthPerBand[4] > 0.8f || telem.stereoWidthPerBand[5] > 0.8f)
            newData.widePresAirCount++;
    });

    // Compute averages
    if (newData.activeTrackCount > 0)
        newData.avgTransientRatio /= newData.activeTrackCount;

    for (int b = 0; b < 6; ++b) {
        if (newData.crestBandCounts[b] > 0)
            newData.avgCrestPerBand[b] /= newData.crestBandCounts[b];
        if (newData.stereoWidthCounts[b] > 0)
            newData.avgStereoWidthPerBand[b] /= newData.stereoWidthCounts[b];
    }
...
void AudioDNAComponent::paint(juce::Graphics& g)
{
    ...
    // ─── Row 1: Band Labels ──────────────────────────────────────────
    {
        ...
        for (int b = 0; b < 6; ++b) {
            auto cell = juce::Rectangle<int>(
                gridArea.getX() + b * (colW + kColGap), y, colW, kLabelH);
    ...
    // ─── Row 2: Health Bar (colored pill with glow) ──────────────────
    {
        ...
        for (int b = 0; b < 6; ++b) {
            auto cell = juce::Rectangle<int>(
                gridArea.getX() + b * (colW + kColGap), y, colW, kHealthBarH);
            ...
            if (fillPct > 0.01f) {
                auto fill = cellF.withWidth(
                    juce::jmax(4.0f, cellF.getWidth() * fillPct));
    ...
    // ─── Row 3: Crest Value ──────────────────────────────────────────
    {
        ...
        for (int b = 0; b < 6; ++b) {
            auto cell = juce::Rectangle<int>(
                gridArea.getX() + b * (colW + kColGap), y, colW, kValueRowH);
            const auto& bh = bands[b];
            ...
            g.fillEllipse(iconArea.getCentreX() - dotR, iconArea.getCentreY() - dotR,
                          dotR * 2.0f, dotR * 2.0f);
            ...
            juce::String crestStr = bh.crestVal > 0.0f
                ? juce::String(bh.crestVal, 1) + "dB"
                : "--";
    ...
    // ─── Row 4: Stereo Width bar ────────────────────────────────────
    {
        ...
        for (int b = 0; b < 6; ++b) {
            ...
            if (wNorm > 0.01f) {
                auto fill = barArea.withWidth(
                    juce::jmax(2.0f, barArea.getWidth() * wNorm));
            ...
            juce::String wStr = bh.stereoVal > 0.01f
                ? juce::String(bh.stereoVal, 2)
                : "--";
    ...
    // ─── Row 5: Diagnosis label ─────────────────────────────────────
    {
        ...
        for (int b = 0; b < 6; ++b) {
            auto cell = juce::Rectangle<int>(
                gridArea.getX() + b * (colW + kColGap), y, colW, kDiagRowH);
            const auto& bh = bands[b];
    ...
    // ─── Summary Row ────────────────────────────────────────────────
    {
        ...
        if (remainingH > kSummaryH) {
            ...
            if (problemBands > 0)
                summary += "  |  " + juce::String(problemBands) + " bands need attention";
            ...
            if (wideSummary.isNotEmpty())
                summary += "  |  Wide: " + wideSummary.trim();
...
void AudioDNAComponent::drawHeader(juce::Graphics& g, juce::Rectangle<int> area)
{
    ...
    g.setColour(MixCoachTheme::textDim().withAlpha(0.7f));
    g.drawText(juce::String(data_.activeTrackCount) + " tracks",
               rightArea, juce::Justification::centredRight);
    ...
    juce::ColourGradient glow(
        MixCoachTheme::accent().withAlpha(0.12f), b.getX() + b.getWidth() * 0.3f, 0.0f,
        MixCoachTheme::accent().withAlpha(0.0f),  b.getX() + b.getWidth(), 0.0f, false);
...
juce::Colour AudioDNAComponent::bandColour(int bandIndex, float value, float maxVal)
{
    ...
    return healthColour(
        t < 0.35f ? kProblem : (t < 0.65f ? kCaution : kGood),
        1.0f);
}
