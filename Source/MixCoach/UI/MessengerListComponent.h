...
namespace mixcoach { class TrackFeedCore; }
...
public:
    ...
    void setHealthFilter(HealthFilter filter) noexcept { healthFilter_ = filter; repaint(); }
    /** Limpia el filtro de salud activo. */
    void clearHealthFilter() noexcept { healthFilter_ = HealthFilter::None; repaint(); }
...
private:
    ...
    mutable juce::Rectangle<int> bannerCollapseBtn_;  // Bounds del botón collapse/expand (mutable, se actualiza en paint())

    // ═══ SPRINT 7: Banner event clickability ═══════════════════════
    mutable std::vector<juce::Rectangle<int>> bannerEventBounds_;  // Hit bounds for each event in banner (mutable, set in paint)
