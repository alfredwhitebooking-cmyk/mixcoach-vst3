    ...
    for (const auto& group : busGroups_) {
        for (const auto& node : group.tracks) {
            ...
            if (idx >= 0 && idx < SlotRegistry::kMaxSlots) {
                if (levelSmooth_[idx].advance(60.0))
                    changed = true;
            }
        }
    }
...
// ═══════════════════════════════════════════════════════════════════════════
void MixMapComponent::updateData(
    ...
    const std::array<TrackRole, SlotRegistry::kMaxSlots>& trackRoles)
{
    ...
    static constexpr BusType kBusOrder[] = {
        BusType::Drums, BusType::Bass, BusType::Guitars,
        BusType::Keys, BusType::Vocals, BusType::FX,
        BusType::Melody, BusType::None
    };
    ...
    for (auto bus : kBusOrder) {
        ...
        registry.forEachActive([&](const SlotInfo& info) {
            ...
            node.roleConfidence = (node.role != TrackRole::Unknown && node.role != TrackRole::Master)
                ? 0.85f : 0.0f;

            // Set smooth level target (clamp between -60 and 0 for meter range)
            if (node.hasSignal) {
                levelSmooth_[info.slotIndex].setTargetValue(
                    juce::jlimit(-60.0f, 0.0f, node.peakCombined));
            } else {
        ...
        if (!group.tracks.empty()) {
            ...
            std::sort(group.tracks.begin(), group.tracks.end(),
                [](const TrackNode& a, const TrackNode& b) {
    ...
    for (const auto& g : busGroups_)
        totalTracks_ += (int)g.tracks.size();
...
void MixMapComponent::paint(juce::Graphics& g)
{
    ...
    // ─── Draw confirm map button at the bottom ────────────────────────
    {
        ...
        juce::ColourGradient btnGrad(
            btnColour.withAlpha(mapConfirmed_ ? 0.10f : 0.14f),
            (float)btnArea.getCentreX(), (float)btnArea.getY(),
            btnColour.withAlpha(mapConfirmed_ ? 0.05f : 0.07f),
            (float)btnArea.getCentreX(), (float)btnArea.getBottom(), false);
        if (!mapConfirmed_)
            btnGrad.addColour(0.5f, btnColour.withAlpha(0.12f));
...
// ═══════════════════════════════════════════════════════════════════════════
void MixMapComponent::drawBusHeader(juce::Graphics& g, const BusGroup& group,
                                     juce::Rectangle<int> area, int /*index*/)
{
    const float cr = kCr;
    ...
    juce::ColourGradient bgGrad(
        group.colour.withAlpha(0.20f), (float)area.getX(), (float)area.getY(),
        group.colour.withAlpha(0.08f), (float)area.getX(), (float)area.getBottom(), false);
    ...
    g.fillRoundedRectangle(
        juce::Rectangle<float>((float)area.getX() + 1.0f, (float)area.getY() + 2.0f,
                               3.0f, (float)area.getHeight() - 4.0f),
        1.5f);
    ...
    juce::Path arrowHead;
    arrowHead.addTriangle((float)arrowX2, (float)arrowY,
                          (float)arrowX2 - 6.0f, (float)arrowY - 4.0f,
                          (float)arrowX2 - 6.0f, (float)arrowY + 4.0f);
...
// ═══════════════════════════════════════════════════════════════════════════
void MixMapComponent::drawTrackRow(juce::Graphics& g, const TrackNode& node,
                                    juce::Rectangle<int> area, bool even)
{
    // ─── Selection highlight ────────────────────────────────────────────
    ...
    if (isSelected) {
        ...
        juce::ColourGradient selGrad(
            MixCoachTheme::accent().withAlpha(0.12f),
            (float)area.getX(), (float)area.getY(),
            MixCoachTheme::accent().withAlpha(0.04f),
            (float)area.getX(), (float)area.getBottom(), false);
    ...
    // ═══ 1. Color dot (rol) ═════════════════════════════════════════════
    auto dotArea = juce::Rectangle<int>(cx, area.getCentreY() - kDotSize / 2,
                                         kDotSize, kDotSize);
    ...
    if (roleStr.isNotEmpty() && roleStr != "Unknown" && !displayName.containsIgnoreCase(roleStr)) {
        ...
        if (node.roleConfidence >= 0.75f)
            displayName += " \xE2\x9C\x93"; // ✅
    ...
    // ═══ 4. Frequency bar ═══════════════════════════════════════════════
    auto freqArea = juce::Rectangle<int>(cx, area.getCentreY() - kFreqBarH / 2,
                                          kFreqBarW, kFreqBarH);
    ...
    juce::Colour dotCol = (node.bus != BusType::None)
        ? getBusColour(static_cast<int>(node.bus)).withAlpha(0.40f)
    ...
    g.drawHorizontalLine(area.getCentreY(),
                          (float)dotIndArea.getRight() + 2.0f,
                          (float)(inner.getRight() - 4));
}

// ═══════════════════════════════════════════════════════════════════════════
//  drawLevelBar — Barra de nivel animada (Peak meter) con gradiente verde→amarillo→rojo
// ═══════════════════════════════════════════════════════════════════════════
void MixMapComponent::drawLevelBar(juce::Graphics& g, juce::Rectangle<int> area,
                                    float levelDb) const
{
    ...
    if (norm > 0.005f)
    {
        ...
        if (norm < 0.50f)
            barColour = MixCoachTheme::success().interpolatedWith(
                MixCoachTheme::warning(), norm / 0.50f);
        else if (norm < 0.80f)
            barColour = MixCoachTheme::warning().interpolatedWith(
                MixCoachTheme::error(), (norm - 0.50f) / 0.30f);
        else
            barColour = MixCoachTheme::error();

        // ─── Gradiente vertical (glow en la parte superior) ───────────
        juce::ColourGradient barGrad(
            barColour.withAlpha(0.85f),
            fillBounds.getX(), fillBounds.getY(),
            barColour.withAlpha(0.50f),
            fillBounds.getX(), fillBounds.getBottom(), false);
        ...
        if (fillW > 6.0f)
        {
            ...
            juce::ColourGradient shineGrad(
                ...
                shine.getX(), shine.getBottom(), false);
...
// ═══════════════════════════════════════════════════════════════════════════
void MixMapComponent::drawFreqBar(juce::Graphics& g, juce::Rectangle<int> area,
                                    const float regionEnergy[6]) const
{
    int numBands = 6;
    ...
    for (int r = 0; r < 6; ++r)
        maxEnergy = juce::jmax(maxEnergy, regionEnergy[r]);

    // Colores de región: Sub=Bass, Bass=Bass, LoMid=Mid, HiMid=Mid, Pres=High, Air=High
    static const juce::Colour kRegionColors[6] = {
        MixCoachTheme::specSub(),
    ...
    for (int r = 0; r < numBands; ++r) {
        ...
        juce::ColourGradient barGrad(
            kRegionColors[r].withAlpha(0.7f * fraction + 0.1f),
            barRect.getX(), barRect.getY(),
            kRegionColors[r].withAlpha(0.3f * fraction),
            barRect.getX(), barRect.getBottom(), false);
        g.setGradientFill(barGrad);
...
// ═══════════════════════════════════════════════════════════════════════════
void MixMapComponent::drawStereoBadge(juce::Graphics& g, juce::Rectangle<int> area,
                                       StereoPos pos) const
{
    auto rect = area.toFloat();
...
MixMapComponent::StereoPos
MixMapComponent::classifyStereo(float correlation, float avgWidth) noexcept
{
    if (correlation < 0.0f)
        return StereoPos::PhaseIssue;
    if (avgWidth < 0.1f)
        return StereoPos::Mono;
    if (correlation > 0.85f && avgWidth < 0.2f)
        return StereoPos::Center;
    if (correlation > 0.6f && avgWidth < 0.3f)
        return StereoPos::Narrow;
    if (avgWidth < 0.5f)
        return StereoPos::Wide;
    return StereoPos::Spread;
}

MixMapComponent::FreqRange
MixMapComponent::classifyFrequency(const float regionEnergy[6]) noexcept
{
    ...
    if (std::abs(maxIdx - secondIdx) >= 3)
        return FreqRange::Full;
...
{
    switch (pos) {
        case StereoPos::Mono:       return "MONO";
        case StereoPos::Center:     return "CENTER";
        case StereoPos::Narrow:     return "NARROW";
        case StereoPos::Wide:       return "WIDE";
        case StereoPos::Spread:     return "SPREAD";
        case StereoPos::PhaseIssue: return juce::String::fromUTF8("\xE2\x9A\xA0PHASE"); // ⚠PHASE
    }
    return "";
}
...
{
    switch (pos) {
        case StereoPos::Mono:       return juce::Colour(0xFF94A3B8); // gray
        case StereoPos::Center:     return juce::Colour(0xFF10B981); // green
        case StereoPos::Narrow:     return juce::Colour(0xFF3B82F6); // blue
        case StereoPos::Wide:       return juce::Colour(0xFFF59E0B); // amber
        case StereoPos::Spread:     return juce::Colour(0xFF8B5CF6); // purple
        case StereoPos::PhaseIssue: return juce::Colour(0xFFEF4444); // red
    }
...
{
    switch (range) {
        case FreqRange::SubBass: return "SUB-BASS";
        case FreqRange::BassMid: return "BASS-MID";
        case FreqRange::Mid:     return "MID";
        case FreqRange::MidHigh: return "MID-HIGH";
        case FreqRange::High:    return "HIGH";
        case FreqRange::Full:    return "FULL";
        case FreqRange::Silent:  return "—";
    }
    return "";
}
...
{
    switch (range) {
        case FreqRange::SubBass: return juce::Colour(0xFF8B5CF6); // violet
        case FreqRange::BassMid: return juce::Colour(0xFF3B82F6); // blue
        case FreqRange::Mid:     return juce::Colour(0xFF10B981); // green
        case FreqRange::MidHigh: return juce::Colour(0xFFF59E0B); // amber
        case FreqRange::High:    return juce::Colour(0xFFF97316); // orange
        case FreqRange::Full:    return juce::Colour(0xFFEC4899); // pink
        case FreqRange::Silent:  return juce::Colour(0xFF6B7280); // gray
    }
    return juce::Colours::grey;
}
...
void MixMapComponent::drawTrackTooltip(juce::Graphics& g, juce::Point<int> mousePos) const
{
    ...
    if (roleStr.isNotEmpty() && roleStr != "Unknown")
        header += " — " + roleStr;
    lines.add(header);
    lines.add(""); // spacer
    ...
    if (node.hasSignal) {
        ...
        if (crest > 0.0f && crest < 60.0f)
            lines.add("Crest: " + juce::String(crest, 1) + " dB");
    } else {
        lines.add("Sin señal");
    }
    ...
    // Stereo
    StereoPos stereo = classifyStereo(node.correlation, node.avgStereoWidth);
    lines.add("Est\xC3\xA9reo: " + stereoLabel(stereo)
              + " (\xCF\x81=" + juce::String(node.correlation, 2) + ")");

    // Role confidence
    if (node.roleConfidence > 0.0f) {
        juce::String confidenceLabel;
        if (node.roleConfidence >= 0.9f)
            confidenceLabel = "\xE2\x9C\x85 Alta";
        else if (node.roleConfidence >= 0.6f)
            confidenceLabel = "\xE2\x9A\xA0\xEF\xB8\x8F Media";
        else
            confidenceLabel = "\xE2\x9D\x8C Baja";
        lines.add("Confianza rol: " + confidenceLabel
                  + " (" + juce::String(node.roleConfidence * 100.0f, 0) + "%)");
    }
    ...
    if (slotIdx >= 0 && slotIdx < (int)trackIssueBadges_.size())
    {
        ...
        if (badge.hasIssues && badge.issueCount > 0)
        {
            ...
            if (badge.shortType.isNotEmpty())
                lines.add("Tipo: " + badge.shortType);
        }
    ...
    if (needsBus)
    {
        ...
        if (hasSuggestedBus)
        {
            ...
            lines.add("   \xE2\x86\xA8 Sugerido: "
                      + juce::String(busNames[static_cast<int>(suggestedBus)]));
        }
        else
        {
    ...
    else
    {
        ...
        busLine = "\xF0\x9F\x94\x80 Enrutamiento: "
                  + juce::String(busNames[static_cast<int>(node.bus)])
                  + " \xE2\x86\x92 Master";
        lines.add(busLine);
        ...
        {
            lines.add("   \xE2\x86\xA8 Rol sugiere: "
                      + juce::String(busNames[static_cast<int>(suggestedBus)]));
        }
    }
    ...
    if (tipX + tipW > parentSize.getRight() - 4)
        tipX = mousePos.x - tipW - 6;
    if (tipX < 4) tipX = 4;

    int tipY = mousePos.y - 10;
    if (tipY + tipH > parentSize.getBottom() - 4)
        tipY = parentSize.getBottom() - tipH - 4;
    if (tipY < 4) tipY = 4;
    ...
    g.fillRoundedRectangle(
        juce::Rectangle<float>(tipBounds.getX() + 1.0f, tipBounds.getY() + 4.0f,
                               2.5f, tipBounds.getHeight() - 8.0f),
        1.5f);
    ...
    for (int i = 0; i < lines.size(); ++i)
    {
        ...
        auto textBounds = juce::Rectangle<float>(
            tipBounds.getX() + padX + 4.0f, (float)textY,
            tipBounds.getWidth() - padX * 2 - 4.0f, (float)lineH);
...
void MixMapComponent::drawMasterSection(juce::Graphics& g, juce::Rectangle<int> area)
{
    ...
    juce::ColourGradient bgGrad(
        masterCol.withAlpha(0.18f), (float)area.getX(), (float)area.getY(),
        masterCol.withAlpha(0.06f), (float)area.getX(), (float)area.getBottom(), false);
...
// ═══════════════════════════════════════════════════════════════════════════
void MixMapComponent::drawRoutingArrow(juce::Graphics& g, juce::Point<float> from,
                                        juce::Point<float> to, juce::Colour colour,
                                        float alpha) const
{
    // ─── Main line ──────────────────────────────────────────────────────
    ...
    juce::Path arrowHead;
    arrowHead.addTriangle(to.x, to.y,
