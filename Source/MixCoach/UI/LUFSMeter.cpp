...
namespace mixcoach {
    ...
    LUFSMeter::LUFSMeter()
    {
        titleLabel_.setText(juce::CharPointer_UTF8("\xF0\x9F\x94\x8A Loudness (LUFS)"), juce::dontSendNotification);
    ...
    void LUFSMeter::paint(juce::Graphics& g)
    {
        ...
        drawBar(g,
                barBounds.removeFromLeft(barWidth).toFloat().reduced(2, 0),
                integrated_.getCurrent(),
                "I",
                "LUFS",
                MixCoachTheme::lufsIntegrated(),
                kTargetIntegrated);
        drawBar(g,
                barBounds.removeFromLeft(barWidth).toFloat().reduced(2, 0),
                shortTerm_.getCurrent(),
                "S",
                "LUFS",
                MixCoachTheme::lufsShort(),
                kTargetStreaming);
        drawBar(g,
                barBounds.removeFromLeft(barWidth).toFloat().reduced(2, 0),
                momentary_.getCurrent(),
                "M",
                "LU",
                MixCoachTheme::lufsMomentary(),
                kTargetBroadcast);
        drawBar(g,
                barBounds.removeFromLeft(barWidth).toFloat().reduced(2, 0),
                truePeak_.getCurrent(),
                "TP",
                "dBTP",
    ...
    void LUFSMeter::drawBar(juce::Graphics& g,
                            ...
                            float targetLine)
    {
        ...
        juce::ColourGradient bgGrad(MixCoachTheme::bgDarker().darker(0.3f),
        ...
        if (barFill.getHeight() > 1.0f) {
            juce::ColourGradient barGrad(colour.withAlpha(0.95f),
            ...
            juce::ColourGradient glow(juce::Colours::white.withAlpha(0.2f),
        ...
        g.setFont(juce::Font(juce::FontOptions(MixCoachTheme::fontSizeTiny)).boldened());
        auto valStr  = (unit == "LU" || unit == "dBTP") ? juce::String(value, 1) + " " + unit : juce::String(value, 1);
