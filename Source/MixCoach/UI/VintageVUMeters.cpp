...
namespace mixcoach {
    ...
    void VintageVUMeters::drawStaticDialFace(
        juce::Graphics& g, juce::Rectangle<float> bounds, float cx, float cy, float radius)
    {
        scaleArcCx_ = cx;
        scaleArcCy_ = cy;
        scaleArcR_  = radius;
        ...
        // ── 1. Fondo ámbar retroiluminado ──────────────────────────────────────
        {
            ...
            juce::ColourGradient amber(
                juce::Colour(0xFFFFF3C8), cx, glowCy, juce::Colour(0xFF8A5E1C), cx, glowCy + glowR, true);
        ...
        // Viñeta interna
        {
            juce::ColourGradient vign(juce::Colours::transparentBlack,
                                      cx,
                                      cy - radius * 0.4f,
                                      juce::Colour(0x60100800),
                                      bounds.getX(),
                                      bounds.getY(),
                                      true);
        ...
        // Sombra en bordes
        {
            juce::ColourGradient leftShadow(juce::Colour(0x30000000),
                                            bounds.getX(),
                                            bounds.getCentreY(),
                                            juce::Colours::transparentBlack,
                                            bounds.getX() + bounds.getWidth() * 0.25f,
                                            bounds.getCentreY(),
                                            false);
            g.setGradientFill(leftShadow);
            g.fillRect(bounds);

            juce::ColourGradient rightShadow(juce::Colour(0x30000000),
                                             bounds.getRight(),
                                             bounds.getCentreY(),
                                             juce::Colours::transparentBlack,
                                             bounds.getRight() - bounds.getWidth() * 0.25f,
                                             bounds.getCentreY(),
                                             false);
        ...
        for (const auto& m : scaleMarks) {
            ...
            if (m.fontSz > 0.0f) {
                ...
                g.drawText(
                    lbl, juce::Rectangle<float>(tx - 15.0f, ty - 8.0f, 30.0f, 16.0f), juce::Justification::centred);
            }
        }
        ...
        {
            juce::ColourGradient glass(juce::Colours::white.withAlpha(0.13f),
                                       bounds.getCentreX(),
                                       bounds.getY(),
                                       juce::Colours::transparentWhite,
                                       bounds.getCentreX(),
                                       bounds.getY() + bounds.getHeight() * 0.45f,
                                       false);
    ...
    void VintageVUMeters::drawNeedle(juce::Graphics& g, float cx, float cy, float radius, float level, float peakHold)
    {
    ...
    void VintageVUMeters::computeCells(juce::Rectangle<int> bounds, juce::Rectangle<float> (&cells)[4]) const noexcept
    {
        auto area = bounds;
        ...
        cells[3]          = {
            b.getX() + halfW + gap * 0.5f, b.getY() + halfH + gap * 0.5f, halfW - gap * 0.5f, halfH - gap * 0.5f};
    }

    void VintageVUMeters::drawBrushedPanel(juce::Graphics& g, juce::Rectangle<float> bounds)
    {
        juce::ColourGradient base(juce::Colour(0xFF1E1E20),
                                  bounds.getCentreX(),
                                  bounds.getY(),
        ...
        for (int y = y0; y < y1; ++y) {
            ...
            juce::Colour c       = (a >= 0.0f) ? juce::Colours::white.withAlpha(0.014f * a)
        ...
        {
            juce::ColourGradient topLit(juce::Colours::white.withAlpha(0.06f),
                                        bounds.getCentreX(),
                                        bounds.getY(),
                                        juce::Colours::transparentWhite,
                                        bounds.getCentreX(),
    ...
    void VintageVUMeters::drawBevelFrame(juce::Graphics& g, juce::Rectangle<float> cell)
    {
        ...
        {
            juce::ColourGradient frameGrad(juce::Colour(0xFF030303),
                                           cell.getCentreX(),
                                           cell.getY(),
                                           juce::Colour(0xFF252527),
                                           cell.getCentreX(),
                                           cell.getBottom(),
                                           false);
        ...
        g.drawLine(cell.getX() + corner, cell.getY() + 1.5f, cell.getRight() - corner, cell.getY() + 1.5f, 1.2f);

        g.setColour(juce::Colours::black.withAlpha(0.7f));
        g.drawLine(
            cell.getX() + corner, cell.getBottom() - 1.5f, cell.getRight() - corner, cell.getBottom() - 1.5f, 1.2f);
    ...
    void VintageVUMeters::drawHub(juce::Graphics& g, float cx, float cy, float faceH)
    {
        ...
        {
            juce::ColourGradient capGrad(juce::Colour(0xFF303030),
                                         cx - r * 0.3f,
                                         cy - r * 0.55f,
        ...
        {
            juce::ColourGradient brass(juce::Colour(0xFFEDD06E),
                                       cx - br * 0.35f,
                                       cy - br * 0.35f,
                                       juce::Colour(0xFF7A5B1A),
                                       cx + br * 0.5f,
                                       cy + br * 0.5f,
                                       false);
    ...
    void VintageVUMeters::rebuildPanelCache(juce::Rectangle<int> bounds)
    {
        ...
        for (int i = 0; i < 4; ++i) paintMeterStatic(cg, cells[i], i);
    ...
    void VintageVUMeters::paint(juce::Graphics& g)
    {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            drawNeedle(g, geo.cx, geo.cy, geo.radius, channels_[i].level.getCurrent(), channels_[i].peakHold);
            g.restoreState();
        }
    }
