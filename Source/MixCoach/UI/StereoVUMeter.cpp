                              ...
                              float leftPeak, float rightPeak)
{
    ...
    leftLabel_.setText("L: " + (leftPeak >= 0.0f ? juce::String("+") : "") + juce::String(leftPeak, 1) + " dB", juce::dontSendNotification);
    rightLabel_.setText("R: " + (rightPeak >= 0.0f ? juce::String("+") : "") + juce::String(rightPeak, 1) + " dB", juce::dontSendNotification);
    
    // Cambiar color de la etiqueta si hay clipping (Saturación)
    leftLabel_.setColour(juce::Label::textColourId, leftPeak >= 0.0f ? MixCoachTheme::error() : MixCoachTheme::textDim());
    rightLabel_.setColour(juce::Label::textColourId, rightPeak >= 0.0f ? MixCoachTheme::error() : MixCoachTheme::textDim());
...
void StereoVUMeter::paint(juce::Graphics& g)
{
    ...
    drawChannelMeter(g, content.removeFromLeft((int)halfWidth).toFloat().reduced(4, 0),
                                     ...
                                     bool isLeftChannel)
{
    ...
    constexpr float kScaleVals[] = { 6.0f, 0.0f, -3.0f, -6.0f, -12.0f, -18.0f, -24.0f, -30.0f, -40.0f, -50.0f, -60.0f };
    ...
    for (int i = 0; i < kNumScale; ++i) 
    {
        ...
        g.setColour(kScaleVals[i] > 0.0f ? MixCoachTheme::error().withAlpha(0.5f) : MixCoachTheme::textMuted().withAlpha(0.4f));
        float textX = isLeftChannel ? meterBar.getX() + 3.0f : meterBar.getRight() - 17.0f;
        g.drawText(juce::String((int)kScaleVals[i]), juce::Rectangle<float>(textX, glY - 4.5f, 14.0f, 9.0f),
    ...
    if (rmsNorm > 0.005f) 
    {
        ...
        juce::ColourGradient barGrad(
            MixCoachTheme::meterGreen(),  meterBar.getCentreX(), meterBar.getBottom(),
            MixCoachTheme::meterRed(),    meterBar.getCentreX(), meterBar.getY(), false);
    ...
    if (peak > -60.0f) 
    {
        ...
        if (isLeftChannel) 
        {
            ...
            tri.addTriangle(triLeft, peakY,
                            triLeft - triSize, peakY - triSize + 0.5f,
                            triLeft - triSize, peakY + triSize - 0.5f);
        }
        else 
        {
            ...
            tri.addTriangle(triRight, peakY,
                            triRight + triSize, peakY - triSize + 0.5f,
                            triRight + triSize, peakY + triSize - 0.5f);
        }
