...
void MixCoachBgWorker::run() { editor_.backgroundRunLoop(); }
    ...
    // ─── Scope de mutex para SlotRegistry ────────────────────────────
    {
        ...
        if (bgBackupScanRequested_.exchange(false))
        {
            ...
            if (found > 0) {
                ...
                bgHasNewResults_.store(true);
                LogHelper::writeToLog("[MixCoachEditor] BG: forceFullSync encontro " + juce::String(found) + " slots");
            }
        }
        ...
        if (initialSyncDone)
        {
            if (sharedData_->getAudioMemoryV2().isInitialized())
            {
                try
                {
                    ...
                    sharedData_->getSlotRegistry().forEachActive([&](const SlotInfo& info) {
                        ...
                        if (nRead > 0)
                        {
                            ...
                            result.peakLeft = (peakL > 1e-10f) ? juce::Decibels::gainToDecibels(peakL) : -100.0f;
                            result.peakRight = (peakR > 1e-10f) ? juce::Decibels::gainToDecibels(peakR) : -100.0f;
                            result.rmsLeft = (sumSqL > 0.0) ? juce::Decibels::gainToDecibels((float)std::sqrt(sumSqL / nRead)) : -100.0f;
                            result.rmsRight = (sumSqR > 0.0) ? juce::Decibels::gainToDecibels((float)std::sqrt(sumSqR / nRead)) : -100.0f;
                            if (nRead > 1) {
                                ...
                                for (int i = 0; i < nRead; ++i) {
                                    ...
                                    num += dl * dr; denL += dl * dl; denR += dr * dr;
                                }
                                result.correlation = (denL > 1e-15 && denR > 1e-15)
                                    ? (float)(num / (std::sqrt(denL) * std::sqrt(denR))) : 0.0f;
                            ...
                            // ═══ Envelope follower: attack/release/sustain estimation ══
                            {
                                ...
                                env.envFloor = (envCount > 0) 
                                    ? juce::jmin(envSum / envCount * 0.3f, env.envFloor * 0.97f + 0.03f * (envSum / envCount))
                                    : env.envFloor;
                                
                                if (peakLinear > 1e-6f && env.envFloor > 1e-10f && env.envPeak > env.envFloor * 2.0f) {
                                    ...
                                    if (normEnv > 0.3f && normEnv < 0.6f && normPrev > normEnv) {
                                        ...
                                        if (fallPerSample > 1e-6f) {
                                            ...
                                            env.releaseSamples = env.releaseSamples * 0.95f + releaseEst * 0.05f;
                                        }
                                    }
                                ...
                                if (env.cycleCount > 2) {
                                    ...
                                    result.sustainLevelDb = (env.sustainLevel > 1e-10f) 
                                        ? 20.0f * std::log10(env.sustainLevel) : -100.0f;
                                } else {
                                ...
                                if (nRead >= 1024)
                                {
                                    ...
                                    for (int ch = 0; ch < numChunks; ++ch)
                                    {
                                        ...
                                        for (int i = 0; i < fftSize; ++i) {
                                            ...
                                            mfft[i*2] = m * slotHann_[i]; mfft[i*2+1] = 0.0f;
                                            sfft[i*2] = s * slotHann_[i]; sfft[i*2+1] = 0.0f;
                                            lfft[i*2] = left[idx] * slotHann_[i]; lfft[i*2+1] = 0.0f;
                                            rfft[i*2] = right[idx] * slotHann_[i]; rfft[i*2+1] = 0.0f;
                                        }
                                        ...
                                        for (int b = 0; b < kNumSpectralBands; ++b) {
                                            ...
                                            for (int bin = startBin; bin < endBin; ++bin) {
                                                ...
                                                if (mag > 1e-10f)
                                                    sumMag += 20.0 * std::log10((double)mag);
                                                else
                                                    sumMag += -100.0;
                                                cnt++;
                                            }
                                        ...
                                        for (int r = 0; r < kNumRegions; ++r) {
                                            ...
                                            for (int b = startBand; b < endBand; ++b) {
                                                ...
                                                for (int bin = bStart; bin < bEnd; ++bin) {
                                                    ...
                                                    lSum += lMag; rSum += rMag; wc++;
                                                    mSum += mMag; sSum += sMag; msCount++;
                                                }
                                                mSumAcc += mSum; sSumAcc += sSum; msCountAcc += msCount;
                                            }
                                    ...
                                    for (int r = 0; r < kNumRegions; ++r) {
                                        if (regionWinCount[r] > 0) {
                                            ...
                                            result.crestPerBand[r] = (avgAvg > 1e-10f) ? avgPeak / avgAvg : 1.0f;
                                            if (result.crestPerBand[r] > 1.0f)
                                                result.crestPerBand[r] = 20.0f * std::log10(result.crestPerBand[r]);
                                            else
                                                result.crestPerBand[r] = 0.0f;
                                            result.stereoWidthPerBand[r] = regionWidthSum[r] / regionWinCount[r];
                ...
                {
                    LogHelper::writeToLog("[MixCoachEditor] BG: Telemetry exception: " + juce::String(e.what()));
                }
                catch (...)
                {
    ...
    if (bgRefAnalysisRequested_.exchange(false))
    {
        ...
        if (refPath.isNotEmpty())
        {
            ...
            if (coach != nullptr)
            {
                ...
                {
                    LogHelper::writeToLog("[MixCoachEditor] BG: analizando referencia desde buffer: " + refPath);
                ...
                {
                    LogHelper::writeToLog("[MixCoachEditor] BG: analizando referencia desde disco (fallback): " + refPath);
